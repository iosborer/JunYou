...
@interface TFJunYou_MeetingObject()
...
@implementation TFJunYou_MeetingObject
...
- (void)alertView:(UIAlertView *)alertView clickedButtonAtIndex:(NSInteger)buttonIndex{
    if (buttonIndex == 1) {
        if (alertView.tag == 10000) { //SIP掉线
            [TFJunYou_MyTools showTipView:Localized(@"JX_Connection")];
            [self connect];
...
-(void)showConnectFailed{
    [TFJunYou_MyTools showTipView:[NSString stringWithFormat:@"%@%@",Localized(@"JXMeeting_connect"),Localized(@"JX_Failed")]];
    _showConnResult = NO;
}

-(void)showConnectSuccess{
    [TFJunYou_MyTools showTipView:[NSString stringWithFormat:@"%@%@",Localized(@"JXMeeting_connect"),Localized(@"JX_Success")]];
    _showConnResult = NO;
}
...
-(void)newMsgCome:(NSNotification *)notifacation
{
    ...
    TFJunYou_MeetingObject *meetingvc=[[TFJunYou_MeetingObject alloc] init];
    if([msg.toUserId isEqualToString:MY_USER_ID]){
        if([msg.type intValue] == kWCMessageTypeVideoChatAsk || [msg.type intValue] == kWCMessageTypeAudioChatAsk){
            if(meetingvc.isMeeting && ![self.roomNum isEqualToString:msg.fromUserId]){ //如果有别的通话 不弹出界面
            ...
            dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(0.5 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
                ...
                if(meetingvc.isMeeting && ![self.roomNum isEqualToString:msg.fromUserId]){
                    if (!msg.isMultipleRelay) {
                        [self sendAVBusy:msg];
                    }
                    return; //如果有别的通话 不弹出界面
...
- (void)sendAVBusy:(TFJunYou_MessageObject *)meetingMsg {
    TFJunYou_MessageObject *msg=[[TFJunYou_MessageObject alloc]init];
    msg.timeSend     = [NSDate date];
    msg.fromUserId   = MY_USER_ID;
    msg.fromUserName = MY_USER_NAME;
...
-(void)checkMediaServer:(NSTimer*)sender{
    _checkCount++;
    if(self.isConnected){
        TFJunYou_MessageObject* msg = (TFJunYou_MessageObject*)[sender userInfo];
        [self sendReadyMsg:msg];//如果连接成功，则发ready消息
        [_timer invalidate];
        _checkCount = 0;
    }
...
-(void)sendReadyMsg:(TFJunYou_MessageObject*)msg{//发送准备好音视频通话的消息
...
-(void)doAudioVideoMeeting:(acceptCallViewController*)vc{
    ...
    if([_msg.type intValue] == kWCMessageTypeAudioMeetingInvite || [_msg.type intValue] == kWCMessageTypeVideoMeetingInvite){
        ...
        [meetingVc sendAccept:kWCMessageTypeAudioChatAccept toUserId:_msg.fromUserId toUserName:_msg.fromUserName objectId:_msg.objectId];
    }else if ([_msg.type intValue] == kWCMessageTypeVideoChatAsk) {
        
        avVC.roomNum = _msg.fromUserId;
        avVC.meetUrl = self.meetUrl;
        [meetingVc sendAccept:kWCMessageTypeVideoChatAccept toUserId:_msg.fromUserId toUserName:_msg.fromUserName objectId:_msg.objectId];
