...
#import "TFJunYou_TalkViewController.h"
#import "UIImage+Color.h"
#import "TFJunYou_TalkCell.h"
#import "TFJunYou_TalkModel.h"
#import "TFJunYou_TalkDetailView.h"

@interface TFJunYou_TalkViewController ()<UITableViewDelegate, UITableViewDataSource,UICollectionViewDelegate, UICollectionViewDataSource,UICollectionViewDelegateFlowLayout>
...
@property (nonatomic, strong) TFJunYou_TalkDetailView *talkView;

@end

@implementation TFJunYou_TalkViewController
...
- (void)viewDidLoad {
    ...
    TFJunYou_TalkModel *model = [[TFJunYou_TalkModel alloc] init];
...
- (TFJunYou_TalkDetailView *)talkView {
 
    if (!_talkView) {
        _talkView = [[TFJunYou_TalkDetailView alloc] initWithFrame:self.view.bounds];
...
- (void)customView {
    ...
    UIImageView *backImageView = [[UIImageView alloc] initWithFrame:CGRectMake(15, 24, TFJunYou__SCREEN_WIDTH - 30, TFJunYou__SCREEN_HEIGHT - 48)];
    backImageView.image = [UIImage imageNamed:@"Talk_BackImage"];
    backImageView.userInteractionEnabled = YES;
    [self.tableBody addSubview:backImageView];
    
    
    UIButton *close = [[UIButton alloc] initWithFrame:CGRectMake(TFJunYou__SCREEN_WIDTH - 47, 24, 32, 32)];
...
//    _headImage = [[UIImageView alloc] initWithFrame:CGRectMake((TFJunYou__SCREEN_WIDTH - 150) /2, THE_DEVICE_HAVE_HEAD ? 40 : 20, 150, 150)];
//    _headImage.image = [UIImage imageNamed:@"Talk_BackBtnImage"];
//    _headImage.layer.cornerRadius = _headImage.frame.size.width / 2;
//    _headImage.layer.masksToBounds = YES;
//    [self.tableBody addSubview:_headImage];
    ...
    [_collectionView registerClass:[TFJunYou_TalkCell class] forCellWithReuseIdentifier:NSStringFromClass([TFJunYou_TalkCell class])];
...
//    UIView *tipView = [[UIView alloc] initWithFrame:CGRectMake(0, CGRectGetMaxY(_name.frame), TFJunYou__SCREEN_WIDTH, 40)];
...
- (void)talkStart:(UIButton *)btn {
    ...
    if (!self.isSpeak) {
        ...
        NSInteger index = 0;
        TFJunYou_TalkModel *talkModel = [[TFJunYou_TalkModel alloc] init];
        for (NSInteger i = 0; i < self.talkArray.count; i ++) {
            TFJunYou_TalkModel *model = self.talkArray[i];
...
- (void)sendOnlineMsg:(TFJunYou_MessageObject *)msg1 {
...
- (void)talkClose {
    ...
    TFJunYou_TalkModel *model = self.talkArray.firstObject;
    model.lastTime = 0;
    model.talkTime = 0;
}
...
- (void)newMsgCome:(NSNotification *)notif {
    ...
    if ([msg.type intValue] == kWCMessageTypeTalkJoin || [msg.type intValue] == kWCMessageTypeTalkOnline) {
        ...
        BOOL flag = NO;
        for (TFJunYou_TalkModel *model in self.talkArray) {
            if ([model.userId isEqualToString:msg.fromUserId]) {
                flag = YES;
                break;
            }
...
//
//    TFJunYou_TalkCell *cell = [tableView dequeueReusableCellWithIdentifier:@"TFJunYou_TalkCell"];
//
//    if (!cell) {
//        cell = [[TFJunYou_TalkCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:@"TFJunYou_TalkCell"];
//    }
//    TFJunYou_TalkModel *model = [self.talkArray objectAtIndex:indexPath.row];
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath{
    ...
    TFJunYou_TalkCell *cell = [collectionView dequeueReusableCellWithReuseIdentifier:NSStringFromClass([TFJunYou_TalkCell class]) forIndexPath:indexPath];
