...
@interface acceptCallViewController ()<TFJunYou_ActionSheetVCDelegate>
...
@property (nonatomic, strong) TFJunYou_ActionSheetVC *callSendMsgView;
...
- (id)init
{
    ...
    if (self) {
        ...
        if ([user.offlineNoPushMsg intValue] != 1) {
            _player = [[TFJunYou_AudioPlayer alloc]init];
...
- (void) customView {
    ...
    _labelRemoteParty.frame = CGRectMake(0, 129, TFJunYou__SCREEN_WIDTH, 25);
    ...
    _labelStatus.frame = CGRectMake(0, CGRectGetMaxY(_labelRemoteParty.frame) + 20, TFJunYou__SCREEN_WIDTH, 18);
...
//    _viewTop.frame = CGRectMake(0, 40, TFJunYou__SCREEN_WIDTH, 67);
...
//    _viewBottom.frame = CGRectMake(0,CGRectGetMaxY(_labelStatus.frame), TFJunYou__SCREEN_WIDTH, TFJunYou__SCREEN_HEIGHT - CGRectGetMaxY(_labelStatus.frame));
//    _viewBottom.userInteractionEnabled = YES;
//    [self.tableBody addSubview:_viewBottom];
    
//    CGFloat margX = 20;
//    CGFloat margWidth = (TFJunYou__SCREEN_WIDTH-(4*Button_Width+margX*2))/3;
    
    CGFloat y = (TFJunYou__SCREEN_HEIGHT - CGRectGetMaxY(_labelStatus.frame)-63*2-175)/2;
    // 发消息
    ...
    _buttonChangeaccept.frame = CGRectMake(TFJunYou__SCREEN_WIDTH-Button_Width-48, _buttonSendmsg.frame.origin.y, Button_Width, Button_Height);
    if (self.isGroup) {
    ...
    _animationImgV = [[UIImageView alloc] initWithFrame:CGRectMake((TFJunYou__SCREEN_WIDTH-145)/2, CGRectGetMaxY(_buttonSendmsg.frame)+15, 145, 145)];
    ...
    _buttonAccept.frame = CGRectMake(TFJunYou__SCREEN_WIDTH-Button_Width-48, _buttonHangup.frame.origin.y, Button_Width, Button_Height);
...
-(TFJunYou_CustomButton *)createBottomButtonWithImage:(NSString *)Image SelectedImg:(NSString *)selectedImage selector:(SEL)selector btnWidth:(CGFloat)btnWidth imageWidth:(CGFloat)imageWidth{
    TFJunYou_CustomButton * button = [TFJunYou_CustomButton buttonWithType:UIButtonTypeCustom];
...
- (void)onSendmsg{
    ...
    self.callSendMsgView = [[TFJunYou_ActionSheetVC alloc] initWithImages:nil names:self.msgArray];
...
- (void)sendMsg:(NSString *)Msg{
    [self onCancel];
    TFJunYou_MessageObject *message = [[TFJunYou_MessageObject alloc] init];
