//
//  TFJunYou_AVCallViewController.m
//  TFJunYouChat
//
...
@interface TFJunYou_AVCallViewController ()<UIAlertViewDelegate, TFJunYou_TalkViewControllerDelegate>
...
@property (nonatomic, strong) TFJunYou_SelectFriendsVC *selVC;
@property (nonatomic, strong) TFJunYou_TalkViewController *talkVC;

@property (nonatomic, assign) BOOL isMuted;

@end

@implementation TFJunYou_AVCallViewController
...
- (void)viewDidLoad {
    ...
    [self actionQuit];
    [TFJunYou_MyTools showTipView:@"模拟器不支持音视频通话"];
    return;
...
//                              }];
        ...
        _recorderBtn = [[UIButton alloc] initWithFrame:CGRectMake(TFJunYou__SCREEN_WIDTH - 20 - 38,  30, 30, 30)];
...
- (void)createTalkView {
    ...
    _talkVC = [[TFJunYou_TalkViewController alloc] init];
...
-(void)onInvite{
    ...
    TFJunYou_UserObject *user = [[TFJunYou_UserObject sharedInstance] getUserById:self.roomNum];
    ...
    _selVC = [TFJunYou_SelectFriendsVC alloc];
    _selVC.isAddWindow = YES;
    _selVC.isNewRoom = NO;
    _selVC.isShowMySelf = NO;
    if (self.isGroup && [self.roomNum isEqualToString:MY_USER_ID]) {
        
        _selVC.type = TFJunYou_SelectFriendTypeSelFriends;
    }else {
        _selVC.type = TFJunYou_SelectFriendTypeSelMembers;
    }
...
//
//        } else {
//            [TFJunYou_MyTools showTipView:Localized(@"JX_NotScreenRecording")];
//        }
//    }else {
...
//                });
//
//                [TFJunYou_MyTools showTipView:@"录屏失败，请重新录制"];
//
////                [self stopRecord];
//            }
//        });
...
////
////                NSString *str = [TFJunYou_FileInfo getUUIDFileName:@"mp4"];
...
- (void)creatLocalVideoView {
    ...
    if (self.isAudio || !device) {
        ...
        UIImageView *headImage = [[UIImageView alloc] initWithFrame:CGRectMake(TFJunYou__SCREEN_WIDTH / 2 - 50, TFJunYou__SCREEN_HEIGHT / 2 - 110, 100, 100)];
    ...
    UILabel *label = [[UILabel alloc] initWithFrame:CGRectMake(0, TFJunYou__SCREEN_HEIGHT / 2, TFJunYou__SCREEN_WIDTH, 20)];
...
- (void)createSuspensionView {
    ...
    g_subWindow.frame = CGRectMake(TFJunYou__SCREEN_WIDTH - 80 - 10, 50, _suspensionBtn.frame.size.width, _suspensionBtn.frame.size.height);
