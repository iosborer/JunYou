...
//        SDWebImageManager *manager = [SDWebImageManager sharedManager];
        ...
        [manager diskImageExistsWithKey:[url absoluteString] completion:^(BOOL isInCache) {
            if (!isInCache) {//没有缓存
                HUD = [MBProgressHUD showHUDAddedTo:self animated:YES];
                HUD.mode = MBProgressHUDModeDeterminate;
                if ([photoUrl rangeOfString:@".webp"].location != NSNotFound) {
                    NSData *data = [NSData dataWithContentsOfURL:url];
                    UIImage *img = [UIImage imageWithData:data];
                    self.webPimageView.frame=[self caculateOriginImageSizeWith:img];
                    self.webPimageView.image = img;
                    [HUD hide:YES];
                }else {
                    
                    [self.imageView sd_setImageWithURL:url placeholderImage:[UIImage imageNamed:@"ic-zanwu@3x"] options:0 progress:^(NSInteger receivedSize, NSInteger expectedSize, NSURL * _Nullable targetURL) {
                        HUD.progress = ((float)receivedSize)/expectedSize;
                    } completed:^(UIImage * _Nullable image, NSError * _Nullable error, SDImageCacheType cacheType, NSURL * _Nullable imageURL) {
                        self.imageView.frame=[self caculateOriginImageSizeWith:image];
                        NSLog(@"图片加载完成");
...
-(UIImageView *)webPimageView{
    
    if (_webPimageView==nil) {
        _webPimageView = [[UIImageView alloc] init];
        _webPimageView.contentMode = UIViewContentModeScaleAspectFit;
        _webPimageView.userInteractionEnabled=YES;
        ...
        [_webPimageView addGestureRecognizer:singleTap];
        [_webPimageView addGestureRecognizer:doubleTap];
        [_webPimageView addGestureRecognizer:twoFingerTap];
        [_webPimageView addGestureRecognizer:longPress];
        
        [singleTap requireGestureRecognizerToFail:doubleTap];//如果双击了，则不响应单击事件
        
        [self.scrollView addSubview:_webPimageView];
    }
...
-(CGRect)caculateOriginImageSizeWith:(UIImage *)image{
    ...
    CGFloat originImageHeight=[self imageCompressForWidth:image targetWidth:TFJunYou__SCREEN_WIDTH].size.height;
    if (originImageHeight>=TFJunYou__SCREEN_HEIGHT) {
        originImageHeight=TFJunYou__SCREEN_HEIGHT;
    }
    
    CGRect frame=CGRectMake(0, (TFJunYou__SCREEN_HEIGHT-originImageHeight)*0.5, TFJunYou__SCREEN_WIDTH, originImageHeight);
