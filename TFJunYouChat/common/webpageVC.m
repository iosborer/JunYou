...
#import "TFJunYou_LinksShareVC.h"
...
@interface webpageVC ()<ImageSelectorViewDelegate,UITextFieldDelegate,UIScrollViewDelegate,AVAudioRecorderDelegate,AVAudioPlayerDelegate,WKNavigationDelegate>
...
@property (nonatomic, strong) TFJunYou_LinksShareVC *linksShareVC;
@property (nonatomic, strong) ImageSelectorViewController *SelectorVC;
@property (nonatomic, strong) TFJunYou_addMsgVC* TFJunYou_addMsgVC;
@property (nonatomic, strong) TFJunYou_ReportUserVC * reportVC;
@property (nonatomic, strong) TFJunYou_RelayVC *relayVC;
...
// 进度条
@property (nonatomic, strong) TFJunYou_WebviewProgress *progressLine;
...
- (void)viewDidLoad {
    ...
    if (self.isSend) {
        ...
        //            self.sendBtn.frame = CGRectMake(TFJunYou__SCREEN_WIDTH - btnSize.width - 10, TFJunYou__SCREEN_TOP - 34, btnSize.width + 10, 30);
        ...
        btn.frame = CGRectMake(TFJunYou__SCREEN_WIDTH - 18-15, TFJunYou__SCREEN_TOP - 18-15, 18, 18);
...
//    webView.scalesPageToFit = YES;
    ...
    self.progressLine = [[TFJunYou_WebviewProgress alloc] initWithFrame:CGRectMake(0, TFJunYou__SCREEN_TOP, 0, 3)];
...
- (void)onMore {
    ...
    self.linksShareVC = [[TFJunYou_LinksShareVC alloc] init];
...
//                               @"urlSchemes" : [self.shareDic objectForKey:@"urlSchemes"]
        ...
        self.relayVC = [[TFJunYou_RelayVC alloc] init];
...
//        [self.view addSubview:_SelectorVC.view];
        ...
        _relayVC = [[TFJunYou_RelayVC alloc] init];
        NSMutableArray *array = [NSMutableArray arrayWithObject:msg];
        //    vc.msg = msg;
...
- (void)onShare {
    ...
    if (self.shareDic) {
        ...
        _TFJunYou_addMsgVC = [[TFJunYou_addMsgVC alloc] init];
        //在发布信息后调用，并使其刷新
        _TFJunYou_addMsgVC.block = ^{
        };
        _TFJunYou_addMsgVC.shareUr = [self.shareDic objectForKey:@"url"];
        _TFJunYou_addMsgVC.shareTitle = [self.shareDic objectForKey:@"title"];
        _TFJunYou_addMsgVC.shareIcon = [self.shareDic objectForKey:@"imageUrl"];
        _TFJunYou_addMsgVC.dataType = weibo_dataType_share;
        _TFJunYou_addMsgVC.delegate = self;
        [self.view addSubview:_TFJunYou_addMsgVC.view];
    }else{
        _TFJunYou_addMsgVC = [[TFJunYou_addMsgVC alloc] init];
        _TFJunYou_addMsgVC.dataType = 2;
        _TFJunYou_addMsgVC.urlShare = self.url;
        //    [g_navigation pushViewController:vc animated:YES];
        [self.view addSubview:_TFJunYou_addMsgVC.view];
    }
}
...
- (void)onReport {
    [self.linksShareVC hideShareView];
    _reportVC = [[TFJunYou_ReportUserVC alloc] init];
...
- (void)onSearch {
    ...
    self.keyboardView = [[UIView alloc] initWithFrame:CGRectMake(0, TFJunYou__SCREEN_HEIGHT-TFJunYou__SCREEN_BOTTOM, TFJunYou__SCREEN_WIDTH, TFJunYou__SCREEN_BOTTOM)];
    ...
    self.textField = [[UITextField alloc] initWithFrame:CGRectMake(CGRectGetMaxX(shutBtn.frame)+10, 5, TFJunYou__SCREEN_WIDTH-50-100, 39)];
    ...
    self.numLabel = [[UILabel alloc] initWithFrame:CGRectMake(TFJunYou__SCREEN_WIDTH-50-100-45, 12, 40, 15)];
...
- (void)onTextType {
    ...
    UIView *view = [[UIView alloc] initWithFrame:CGRectMake(0, self.view.frame.size.height - TFJunYou__SCREEN_BOTTOM - 80, TFJunYou__SCREEN_WIDTH, TFJunYou__SCREEN_BOTTOM + 80)];
    view.backgroundColor = [UIColor whiteColor];
    [self.baseView addSubview:view];
    
    //滑块设置
    _slider = [[UISlider alloc] initWithFrame:CGRectMake(20, 50, TFJunYou__SCREEN_WIDTH - 40, 20)];
...
- (void)onFloatWindow {
    ...
    g_subWindow.frame = CGRectMake(TFJunYou__SCREEN_WIDTH - 80 - 10, 50, 60, 60);
...
- (void)hideFloatWindow {
    [UIView animateWithDuration:0.5 animations:^{
        self.view.frame = CGRectMake(TFJunYou__SCREEN_WIDTH, 0, self.view.frame.size.width, 0);
...
- (void)panAction:(UIPanGestureRecognizer *)pan {
    ...
    if (pan.state == UIGestureRecognizerStateEnded || pan.state == UIGestureRecognizerStateCancelled) {
        if (frame.origin.x <= TFJunYou__SCREEN_WIDTH / 2) {
            frame.origin.x = 10;
        }else {
            frame.origin.x = TFJunYou__SCREEN_WIDTH - frame.size.width - 10;
        }
        if (frame.origin.y < 0) {
            frame.origin.y = 10;
        }
        if ((frame.origin.y + frame.size.height) > TFJunYou__SCREEN_HEIGHT) {
            frame.origin.y = TFJunYou__SCREEN_HEIGHT - frame.size.height - 10;
        }
        [UIView animateWithDuration:0.5 animations:^{
...
//    self.keywordsNum = (int)[self.webView highlightAllOccurencesOfString:self.textStr index:self.clickTime];
    ...
    [self.webView highlightAllOccurencesOfString:self.textStr index:self.clickTime searchResultBlock:^(NSInteger searchCount) {
        weakSelf.keywordsNum = (int)searchCount;
        weakSelf.numLabel.text = [NSString stringWithFormat:@"%d/%d",weakSelf.clickTime,weakSelf.keywordsNum];
    }];
...
- (BOOL)textFieldShouldReturn:(UITextField *)textField {
    ...
    if (textField.returnKeyType==UIReturnKeySearch) {
        ...
        [self.webView highlightAllOccurencesOfString:textField.text index:1 searchResultBlock:^(NSInteger searchCount) {
            weakSelf.keywordsNum = (int)searchCount;
            weakSelf.clickTime = weakSelf.keywordsNum != 0 ? 1 : 0;
            weakSelf.numLabel.text = [NSString stringWithFormat:@"%d/%d",weakSelf.clickTime,weakSelf.keywordsNum];
        }];
    }
    return YES;
...
- (void)keyboardWillChangeFrame:(NSNotification *)notification{
    ...
    [UIView animateWithDuration:duration animations:^{
        self.keyboardView.frame = CGRectMake(self.keyboardView.frame.origin.x, TFJunYou__SCREEN_HEIGHT-TFJunYou__SCREEN_BOTTOM+transformY, self.keyboardView.frame.size.width, self.keyboardView.frame.size.height);
    }];
}
#pragma mark --键盘收回
- (void)keyboardDidHide:(NSNotification *)notification{
    CGFloat duration = [notification.userInfo[UIKeyboardAnimationDurationUserInfoKey] floatValue];
    [UIView animateWithDuration:duration animations:^{
        self.keyboardView.frame = CGRectMake(self.keyboardView.frame.origin.x, TFJunYou__SCREEN_HEIGHT-TFJunYou__SCREEN_BOTTOM, self.keyboardView.frame.size.width, self.keyboardView.frame.size.height);
    }];
}
...
- (void)webView:(WKWebView *)webView didStartProvisionalNavigation:(WKNavigation *)navigation {
    ...
    NSString* s = webView.URL.absoluteString;
...
//        TFJunYou_WebAuthView *view = [[TFJunYou_WebAuthView alloc] init];
...
//    NSString *string = [self.webView stringByEvaluatingJavaScriptFromString:@"document.title"];
    ...
    /**
    [self.webView evaluateJavaScript:@"document.title" completionHandler:^(NSString *string, NSError * _Nullable error) {
        NSLog(@"title===%@",string);
            if (self.titleString.length > 0) {
                self.title = self.titleString;
...
//            self.url = [self.webView.request.URL absoluteString];
        ...
        //    [self.webView stringByEvaluatingJavaScriptFromString:jsGetImages];
            //注入js方法
            [self.webView evaluateJavaScript:jsGetImages completionHandler:^(id _Nullable obj, NSError * _Nullable error) {
...
//- (void)skPayVC:(TFJunYou_SkPayVC *)skPayVC payBtnAction:(NSDictionary *)payDic {
//    if ([g_server.myself.isPayPassword boolValue]) {
//        self.verVC = [TFJunYou_VerifyPayVC alloc];
//        self.verVC.type = TFJunYou_VerifyTypeSkPay;
...
//        payPswVC.enterType = TFJunYou_VerifyTypeSkPay;
...
//        TFJunYou_SkPayVC *vc = [[TFJunYou_SkPayVC alloc] init];
//        vc.payDic = [dict copy];
//        vc.delegate = self;
