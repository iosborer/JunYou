...
@interface ImageBrowserViewController ()<UIScrollViewDelegate,PhotoViewDelegate,TFJunYou_ActionSheetVCDelegate,KKImageEditorDelegate,UINavigationControllerDelegate>{
...
- (void)setupActionSheet {
    ...
    TFJunYou_ActionSheetVC *actionVC = [[TFJunYou_ActionSheetVC alloc] initWithImages:@[] names:names];
...
-(UIScrollView *)scrollView{
    ...
    if (_scrollView==nil) {
        _scrollView=[[UIScrollView alloc]initWithFrame:CGRectMake(0, 0, TFJunYou__SCREEN_WIDTH, TFJunYou__SCREEN_HEIGHT)];
...
-(void)setImagesArray:(NSArray *)imagesArray{
    ...
    //设置contentSize
    self.scrollView.contentSize = CGSizeMake(TFJunYou__SCREEN_WIDTH * self.imagesArray.count, 0);
    
    for (int i = 0; i < self.imagesArray.count; i++) {
        [_subViewArray addObject:[NSNull class]];
    }
    
    self.scrollView.contentOffset = CGPointMake(TFJunYou__SCREEN_WIDTH*self.index, 0);//此句代码需放在[_subViewArray addObject:[NSNull class]]之后，因为其主动调用scrollView的代理方法，否则会出现数组越界
