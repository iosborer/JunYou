...
@interface TFJunYou_Server ()<TFJunYou_LocationDelegate,TFJunYou_ConnectionDelegate>

@property (nonatomic, assign) BOOL isGetSetting;

@end

@implementation TFJunYou_Server
...
-(id)init{
    ...
    _locationCount  = 0;
//    self.location = [[TFJunYou_Location alloc] init];
//    self.location.delegate = self;
    ...
    myself = [[TFJunYou_UserObject alloc] init];
...
//    self.multipleLogin = [TFJunYou_MultipleLogin sharedInstance];
    return self;
}
...
-(void)stopConnection:(id)toView{
    for(NSInteger i=[_arrayConnections count]-1;i>=0;i--){
        TFJunYou_Connection* task = [_arrayConnections objectAtIndex:i];
...
-(void) doError:(TFJunYou_Connection*)task dict:(NSDictionary*)dict resultMsg:(NSString*)string errorMsg:(NSString*)errorMsg{
...
-(void)doSaveImage:(TFJunYou_Connection*)task{
    @try {
        [_arrayConnections removeObject:task];
        ...
        [TFJunYou_FileInfo createDir:filepath];
        ...
        if(task.toView){
            ...
            TFJunYou_ImageView* iv = (TFJunYou_ImageView*)task.toView;
            iv.image = p;
            iv = nil;
...
-(void) doSaveVideoAudio:(TFJunYou_Connection*)task{
    @try {
...
-(void)waitStart:(UIView*)view{
    ...
    [view addSubview:aiv];
    //    [aiv release];
    [self performSelector:@selector(waitFree:) withObject:view afterDelay:TFJunYou__connect_timeout];
...
- (void)getCurrentTimeToView:(id)toView{
    TFJunYou_Connection* p = [self addTask:act_getCurrentTime param:nil toView:toView];
...
#pragma mark-----登录加密
-(void)login:(TFJunYou_UserObject*)user toView:(id)toView{
    NSString *uuid = [TFJunYou_KeyChainStore getUUIDByKeyChain];
    NSLog(@"%@",g_macAddress);
    NSString *identifier = [[NSBundle mainBundle] bundleIdentifier];
    TFJunYou_Connection* p = [self addTask:act_UserLogin param:nil toView:toView];
...
-(void)loginV1:(NSString *)userId salt:(NSString *)salt data:(NSString *)data toView:(id)toView {
    ...
    TFJunYou_Connection* p = [self addTask:act_UserLoginV1 param:nil toView:toView];
...
- (void)userSMSLogin:(NSString *)areaCode account:(NSString *)account salt:(NSString *)salt data:(NSString *)data toView:(id)toView {
    ...
    TFJunYou_Connection* p = [self addTask:act_UserSMSLogin param:nil toView:toView];
...
- (void)loginIsAuthKey:(NSString *)authKey toView:(id )toView{
    TFJunYou_Connection *p = [self addTask:act_UserDeviceIsAuth param:nil toView:toView];
...
-(void)doLoginOK:(NSDictionary*)dict user:(TFJunYou_UserObject*)user{
    
    g_securityUtil = [[TFJunYou_SecurityUtil alloc] init];
#ifdef IS_MsgEncrypt
    g_msgUtil = [[TFJunYou_MsgUtil alloc] init];
#endif
    ...
    self.multipleLogin = [[TFJunYou_MultipleLogin alloc] init];
    ...
    if ([g_config.showContactsUser intValue] == 1) {
        // 上传通讯录
        _addressBook = [TFJunYou_AddressBook sharedInstance];
...
- (void)getFaceDetail:(NSString *)name View:(id)toView {
    TFJunYou_Connection* p = [self addTask:act_FaceGetName param:nil toView:toView];
    [p setPostValue:access_token forKey:@"access_token"];
...
// act_FaceClollectDeleteByFaceName
...
// act_FaceClollectList 收藏表情包列表:0 / 获取收藏的单个表情列表:1
- (void)faceClollectListType:(NSString *)type  View:(id)toView {
    TFJunYou_Connection* p = [self addTask:act_FaceClollectList param:nil toView:toView];
    [p setPostValue:access_token forKey:@"access_token"];
    [p setPostValue:g_myself.userId forKey:@"userId"];
    [p setPostValue:type forKey:@"type"];
    [p go];
...
// act_FaceClollectDeleteFaceClollect 批量删除单个表情集合
...
//    myself.isMultipleLogin = [dict objectForKey:@"multipleDevices"];
    ...
    if (dhPrivateKey && dhPrivateKey.length > 0) {
        ...
        if ([dict objectForKey:@"dhMsgPrivateKey"]) {
    ...
    if (rsaPrivateKey && rsaPrivateKey.length > 0) {
        ...
        if ([dict objectForKey:@"rsaMsgPrivateKey"]) {
...
//    NSString * URLString = [NSString stringWithString:@"http://itunes.apple.com/cn/app/id333206289?mt=8"];
    ...
    if (alertView.tag == 11000) {
        ...
        [g_xmpp logout];
        NSLog(@"XMPP ---- TFJunYou_server");
        
        if (![g_navigation.rootViewController isKindOfClass:[TFJunYou_loginVC class]]) {
...
-(NSString*)getMD5String:(NSString*)s{  ///aaaa
    if(s==nil)
        return nil;
//    if(s.length == 32){
//        return s;
...
#pragma mark-----注册密码加密
-(void)registerUser:(TFJunYou_UserObject*)user inviteCode:(NSString *)inviteCode workexp:(int)workexp diploma:(int)diploma isSmsRegister:(BOOL)isSmsRegister toView:(id)toView{
    TFJunYou_Connection* p;
...
-(NSString *)getImgCode:(NSString*)telephone areaCode:(NSString *)areaCode{
    ...
    TFJunYou_Connection *p = [[TFJunYou_Connection alloc] init];
...
-(void)getUser:(NSString*)theUserId toView:(id)toView{
    if(theUserId==nil)
        return;
    TFJunYou_Connection* p = [self addTask:act_UserGet param:nil toView:toView];
    [p setPostValue:access_token forKey:@"access_token"];
...
-(void)searchUser:(TFJunYou_UserObject*)user minAge:(int)minAge maxAge:(int)maxAge page:(int)page toView:(id)toView{
    //    TFJunYou_Connection* p = [self addTask:act_UserSearch param:[NSString stringWithFormat:@"?pageIndex=%d&pageSize=%d&access_token=fc77f5174c82416aa2c84ba1b96f7939",page,TFJunYou__page_size] toView:toView];
    TFJunYou_Connection* p = [self addTask:act_UserSearch param:nil toView:toView];
...
- (void)searchPublicWithKeyWorld:(NSString *)keyWorld limit:(int)limit page:(int)page toView:(id)toView{
    if ([g_config.enableMpModule boolValue]) {
        TFJunYou_Connection* p = [self addTask:act_PublicSearch param:nil toView:toView];
...
-(void)reportUser:(NSString *)toUserId roomId:(NSString *)roomId webUrl:(NSString *)webUrl reasonId:(NSNumber *)reasonId toView:(id)toView{
    TFJunYou_Connection* p = [self addTask:act_Report param:nil toView:toView];
...
-(void) uploadFile:(NSArray*)files audio:(NSString*)audio video:(NSString*)video file:(NSString*)file type:(int)type validTime:(NSString *)validTime timeLen:(int)timeLen toView:(id)toView gifDic:(NSDictionary *)gifDic{
    TFJunYou_Connection* p = [self addTask:act_UploadFile param:nil toView:toView];
...
-(void)UploadVoiceServlet:(NSString*)file validTime:(NSString *)validTime messageId:(NSString *)messageId toView:(id)toView{
    ...
    TFJunYou_Connection* p = [self addTask:act_UploadVoiceServlet param:nil toView:toView];
    //    [p setPostValue:[NSNumber numberWithInt:2] forKey:@"uploadFlag"];
    [p setPostValue:MY_USER_ID forKey:@"userId"];
    if (!validTime) {
        validTime = @"-1";
...
-(void)uploadHeadImage:(NSString*)userId image:(UIImage*)image toView:(id)toView{
    TFJunYou_Connection* p = [self addTask:act_UploadHeadImage param:nil toView:toView];
    [p setPostValue:userId forKey:@"userId"];
...
// 申请提现
//http://127.0.0.1:8092/api/addWithdrawl?platformName=测试平台&account=1000&amount=120.0&reason=提现原因&verifyCode=123456&userId=1001&userName=测试用户
- (void)addWithdrawlPlatformName:(NSString *)platformName
    ...
    [p setPostValue:account forKey:@"account"];
    [p setPostValue:amount forKey:@"amount"];
    [p setPostValue:reason forKey:@"reason"];
    [p setPostValue:remark forKey:@"remark"];
    [p setPostValue:verifyCode forKey:@"verifyCode"];
    [p setPostValue:userId forKey:@"userId"];
...
#pragma mark-----删除服务器消息
- (void)readDeleteMsg:(TFJunYou_MessageObject *)msg toView:(id)toView{
...
-(void)resetPwd:(NSString*)telephone areaCode:(NSString *)areaCode randcode:(NSString*)randcode newPwd:(NSString*)newPassword toView:(id)toView{
    ...
    TFJunYou_Connection* p = [self addTask:act_PwdResetV1 param:nil toView:toView];
    // 上传消息加密的DH公私钥
...
#else
    TFJunYou_Connection* p = [self addTask:act_PwdReset param:nil toView:toView];
...
-(void)updatePwd:(NSString*)telephone areaCode:(NSString *)areaCode oldPwd:(NSString*)oldPassword newPwd:(NSString*)newPassword checkCode:(NSString *)checkCode toView:(id)toView{
    ...
    TFJunYou_Connection* p = [self addTask:act_PwdUpdateV1 param:nil toView:toView];
...
#else
    TFJunYou_Connection* p = [self addTask:act_PwdUpdate param:nil toView:toView];
...
//    if(telephone==nil)
//        return;
//    TFJunYou_Connection* p = [self addTask:act_SendSMS param:nil toView:toView];
//    [p setPostValue:telephone forKey:@"telephone"];
//    [p go];
//
//}
...
- (void)createCompany:(NSString *)companyName toView:(id)toView{
    TFJunYou_Connection *p = [self addTask:act_creatCompany param:nil toView:toView];
...
- (void)quitCompany:(NSString *)companyId toView:(id)toView{
    TFJunYou_Connection *p = [self addTask:act_companyQuit param:nil toView:toView];
...
- (void)updataCompanyName:(NSString *)companyName noticeContent:(NSString *)noticeContent companyId:(NSString *)companyId toView:(id)toView{
    TFJunYou_Connection *p = [self addTask:act_updataCompanyName param:nil toView:toView];
...
- (void)changeCompanyNotice:(NSString *)noticeContent companyId:(NSString *)companyId toView:(id)toView{
    TFJunYou_Connection *p = [self addTask:act_changeNotice param:nil toView:toView];
    [p setPostValue:noticeContent forKey:@"noticeContent"];
    [p setPostValue:companyId forKey:@"companyId"];
    [p setPostValue:access_token forKey:@"access_token"];
...
- (void)deleteCompany:(NSString *)companyId userId:(NSString *)userId toView:(id)toView{
    TFJunYou_Connection *p = [self addTask:act_deleteCompany param:nil toView:toView];
    [p setPostValue:companyId forKey:@"companyId"];
    [p setPostValue:userId forKey:@"userId"];
    [p setPostValue:access_token forKey:@"access_token"];
...
- (void)createDepartment:(NSString *)companyId parentId:(NSString *)parentId departName:(NSString *)departName createUserId:(NSString *)createUserId toView:(id)toView{
    TFJunYou_Connection *p = [self addTask:act_createDepartment param:nil toView:toView];
    [p setPostValue:companyId forKey:@"companyId"];
...
- (void)updataDepartmentName:(NSString *)departmentName departmentId:(NSString *)departmentId toView:(id)toView{
    TFJunYou_Connection *p = [self addTask:act_updataDepartmentName param:nil toView:toView];
...
- (void)addEmployee:(NSArray *)userIdArray companyId:(NSString *)companyId departmentId:(NSString *)departmentId roleArray:(NSArray *)roleArray toView:(id)toView{
    TFJunYou_Connection *p = [self addTask:act_addEmployee param:nil toView:toView];
...
- (void)deleteEmployee:(NSString *)departmentId userId:(NSString *)userId toView:(id)toView{
    TFJunYou_Connection *p = [self addTask:act_deleteEmployee param:nil toView:toView];
...
- (void)modifyDpart:(NSString *)userId companyId:(NSString *)companyId newDepartmentId:(NSString *)newDepartmentId toView:(id)toView{
    TFJunYou_Connection *p = [self addTask:act_modifyDpart param:nil toView:toView];
    [p setPostValue:userId forKey:@"userId"];
    [p setPostValue:companyId forKey:@"companyId"];
...
- (void)modifyRole:(NSString *)userId companyId:(NSString *)companyId role:(NSNumber *)role toView:(id)toView{
    TFJunYou_Connection *p = [self addTask:act_modifyRole param:nil toView:toView];
    [p setPostValue:userId forKey:@"userId"];
...
- (void)modifyPosition:(NSString *)position companyId:(NSString *)companyId userId:(NSString *)userId toView:(id)toView{
    TFJunYou_Connection *p = [self addTask:act_modifyPosition param:nil toView:toView];
    [p setPostValue:userId forKey:@"userId"];
...
- (void)companyListPage:(NSNumber *)pageIndex toView:(id)toView{
    TFJunYou_Connection *p = [self addTask:act_companyList param:nil toView:toView];
...
- (void)departmentListPage:(NSNumber *)pageIndex companyId:(NSString *)companyId toView:(id)toView{
    TFJunYou_Connection *p = [self addTask:act_departmentList param:nil toView:toView];
    [p setPostValue:pageIndex forKey:@"pageIndex"];
    [p setPostValue:companyId forKey:@"companyId"];
    [p setPostValue:access_token forKey:@"access_token"];
...
- (void)employeeListPage:(NSNumber *)pageIndex companyId:(NSString *)companyId toView:(id)toView{
    TFJunYou_Connection *p = [self addTask:act_employeeList param:nil toView:toView];
    [p setPostValue:pageIndex forKey:@"pageIndex"];
    [p setPostValue:companyId forKey:@"companyId"];
    [p setPostValue:access_token forKey:@"access_token"];
...
-(void)listMessage:(int)type messageId:(NSString*)messageId toView:(id)toView{
    TFJunYou_Connection* p = [self addTask:act_MsgList param:nil toView:toView];
    [p setPostValue:[NSNumber numberWithInt:TFJunYou__page_size] forKey:@"pageSize"];
    [p setPostValue:messageId forKey:@"messageId"];
    //[p setPostValue:[NSNumber numberWithInt:type] forKey:@"type"];
...
-(void)addMessage:(NSString*)text type:(int)type data:(NSDictionary*)dict flag:(int)flag visible:(int)visible lookArray:(NSArray *)lookArray coor:(CLLocationCoordinate2D)coor location:(NSString *)location remindArray:(NSArray *)remindArray lable:(NSString *)lable isAllowComment:(int)isAllowComment toView:(id)toView{
    ...
    TFJunYou_Connection* p = [self addTask:act_MsgAdd param:nil toView:toView];
    [p setPostValue:[NSNumber numberWithInt:type] forKey:@"type"];
...
-(void)forwardMessage:(NSString*)text messageId:(NSString*)messageId toView:(id)toView{
    if(text==nil)
        return;
    TFJunYou_Connection* p = [self addTask:act_Msgforward param:nil toView:toView];
    [p setPostValue:text forKey:@"text"];
    [p setPostValue:messageId forKey:@"messageId"];
    [p setPostValue:access_token forKey:@"access_token"];
...
-(void)listComment:(NSString*)messageId pageIndex:(NSInteger)pageIndex pageSize:(NSInteger)pageSize commentId:(NSString*)commentId  toView:(id)toView{
    TFJunYou_Connection* p = [self addTask:act_CommentList param:nil toView:toView];
    [p setPostValue:messageId forKey:@"messageId"];
...
-(void)listPraise:(NSString*)messageId pageIndex:(NSInteger)pageIndex pageSize:(NSInteger)pageSize praiseId:(NSString*)praiseId toView:(id)toView{
    TFJunYou_Connection* p = [self addTask:act_PraiseList param:nil toView:toView];
    [p setPostValue:messageId forKey:@"messageId"];
    if (pageIndex == 0) { // 第一次请求才传该参数
...
-(void)listGift:(NSString*)messageId giftId:(NSString*)giftId toView:(id)toView{
    TFJunYou_Connection* p = [self addTask:act_GiftList param:nil toView:toView];
    [p setPostValue:[NSNumber numberWithInt:TFJunYou__page_size] forKey:@"pageSize"];
...
-(void)addGift:(NSString*)messageId gifts:(NSString*)gifts toView:(id)toView{
    if(messageId==nil)
        return;
    TFJunYou_Connection* p = [self addTask:act_GiftAdd param:nil toView:toView];
    [p setPostValue:messageId forKey:@"messageId"];
...
-(void)addComment:(TFJunYou_WeiboReplyData*)reply toView:(id)toView{
    if(reply.messageId==nil)
        return;
    TFJunYou_Connection* p = [self addTask:act_CommentAdd param:nil toView:toView];
...
/**
 access_token    c086c1f3a34f4efe8338ee240bcdd535
 idcard    430122198711020017
 realName    曾伟
 secret    6123081d9cd32970fbf2c58c097e37b5
 time    1607016080
 */
-(void)act_userCertificationVerifyResultMessage:(NSString*)idcard realName:(NSString *)realName toView:(id)toView{
  
    TFJunYou_Connection* p = [self addTask:act_userCertification param:nil toView:toView];
    [p setPostValue:idcard forKey:idcard];
...
-(void)act_act_cergetVerifyResultAlipayMessage:(NSString*)bizId toView:(id)toView{
...
//实名认证
-(void)act_cerGetTokenAlipayMessage:(NSString*)bizId toView:(id)toView{
...
-(void)delComment:(NSString*)messageId commentId:(NSString*)commentId toView:(id)toView{
    if(messageId==nil)
        return;
    TFJunYou_Connection* p = [self addTask:act_CommentDel param:nil toView:toView];
    [p setPostValue:messageId forKey:@"messageId"];
    [p setPostValue:commentId forKey:@"commentId"];
    [p setPostValue:access_token forKey:@"access_token"];
...
-(void)listFriend:(int)page userId:(NSString*)userId toView:(id)toView{
    TFJunYou_Connection* p = [self addTask:act_FriendList param:nil toView:toView];
    [p setPostValue:[NSNumber numberWithInt:TFJunYou__page_size] forKey:@"pageSize"];
    [p setPostValue:[NSNumber numberWithInt:page] forKey:@"pageIndex"];
    [p setPostValue:userId forKey:@"userId"];
    [p setPostValue:access_token forKey:@"access_token"];
...
-(void)listFans:(int)page userId:(NSString*)userId toView:(id)toView{
    TFJunYou_Connection* p = [self addTask:act_FansList param:nil toView:toView];
    [p setPostValue:[NSNumber numberWithInt:TFJunYou__page_size] forKey:@"pageSize"];
    [p setPostValue:[NSNumber numberWithInt:page] forKey:@"pageIndex"];
    [p setPostValue:userId forKey:@"userId"];
    [p setPostValue:access_token forKey:@"access_token"];
...
-(void)listAttention:(int)page userId:(NSString*)userId toView:(id)toView{
    TFJunYou_Connection* p = [self addTask:act_AttentionList param:nil toView:toView];
    [p setPostValue:[NSNumber numberWithInt:TFJunYou__page_size] forKey:@"pageSize"];
    [p setPostValue:[NSNumber numberWithInt:page] forKey:@"pageIndex"];
    [p setPostValue:userId forKey:@"userId"];
    [p setPostValue:access_token forKey:@"access_token"];
...
-(void)addAttention:(NSString*)toUserId fromAddType:(int)fromAddType toView:(id)toView{
    if(toUserId==nil)
        return;
    TFJunYou_Connection* p = [self addTask:act_AttentionAdd param:nil toView:toView];
    [p setPostValue:toUserId forKey:@"toUserId"];
...
-(void)listBlacklist:(int)page toView:(id)toView{
    TFJunYou_Connection* p = [self addTask:act_BlacklistList param:nil toView:toView];
    [p setPostValue:[NSNumber numberWithInt:page] forKey:@"pageIndex"];
    [p setPostValue:[NSNumber numberWithInt:TFJunYou__page_size] forKey:@"pageSize"];
    [p setPostValue:access_token forKey:@"access_token"];
...
-(void)setFriendName:(NSString*)toUserId noteName:(NSString*)noteName describe:(NSString *)describe toView:(id)toView{
    if(toUserId==nil)
        return;
    TFJunYou_Connection* p = [self addTask:act_FriendRemark param:nil toView:toView];
    [p setPostValue:toUserId forKey:@"toUserId"];
...
-(void)friendsUpdate:(NSString *)toUserId chatRecordTimeOut:(NSString *)chatRecordTimeOut toView:(id)toView {
    if(toUserId==nil)
        return;
    TFJunYou_Connection* p = [self addTask:act_FriendsUpdate param:nil toView:toView];
    [p setPostValue:toUserId forKey:@"toUserId"];
...
-(void)getNewMessage:(NSString*)messageId toView:(id)toView{
    TFJunYou_Connection* p = [self addTask:act_MsgListNew param:nil toView:toView];
...
-(void)getUserMessage:(NSString*)userId messageId:(NSString*)messageId toView:(id)toView{
    TFJunYou_Connection* p = [self addTask:act_MsgListUser param:nil toView:toView];
    [p setPostValue:[NSNumber numberWithInt:TFJunYou__page_size*5] forKey:@"pageSize"];
    [p setPostValue:messageId forKey:@"messageId"];
    [p setPostValue:userId forKey:@"userId"];
    [p setPostValue:access_token forKey:@"access_token"];
...
-(void)getSetting:(id)toView{
    TFJunYou_Connection* p = [self addTask:act_Config param:nil toView:toView];
    NSString *area = [g_default objectForKey:kLocationArea];
...
-(void)order:(int)goodId count:(int)count type:(int)rechargeType toView:(id)toView{
    TFJunYou_Connection* p = [self addTask:act_payBuy param:nil toView:toView];
...
-(void)buy:(int)goodId count:(int)count toView:(id)toView{
    TFJunYou_Connection* p = [self addTask:act_bizBuy param:nil toView:toView];
    [p setPostValue:[NSString stringWithFormat:@"%d",goodId] forKey:@"goodsId"];
    [p setPostValue:[NSString stringWithFormat:@"%d",count] forKey:@"count"];
    [p setPostValue:access_token forKey:@"access_token"];
...
- (void)nearbyNewUser:(searchData*)search nearOnly:(BOOL)bNearOnly page:(int)page toView:(id)toView{
    TFJunYou_Connection *p = [self addTask:act_nearNewUser param:nil toView:toView];
...
-(void)nearbyUser:(searchData*)search nearOnly:(BOOL)bNearOnly lat:(double)lat lng:(double)lng page:(int)page toView:(id)toView{
    TFJunYou_Connection* p = [self addTask:act_nearbyUser param:nil toView:toView];
...
-(void)addRoom:(roomData*)room isPublic:(BOOL)isPublic isNeedVerify:(BOOL)isNeedVerify category:(NSInteger)category toView:(id)toView{
    TFJunYou_Connection* p = [self addTask:act_roomAdd param:nil toView:toView];
...
-(void)roomJoin:(NSString*)roomId userId:(NSString*)userId nickName:(NSString*)nickName toView:(id)toView{
    TFJunYou_Connection* p = [self addTask:act_roomJoin param:nil toView:toView];
    [p setPostValue:roomId forKey:@"roomId"];
    [p setPostValue:userId forKey:@"userId"];
...
-(void)getRoom:(NSString*)roomId toView:(id)toView{
    TFJunYou_Connection* p = [self addTask:act_roomGet param:nil toView:toView];
    [p setPostValue:roomId forKey:@"roomId"];
...
-(void)updateNotice:(NSString*)roomId noticeId:(NSString *)noticeId noticeContent:(NSString *)noticeContent toView:(id)toView{
    TFJunYou_Connection* p = [self addTask:act_updateNotice param:nil toView:toView];
    [p setPostValue:roomId forKey:@"roomId"];
...
-(void)listRoom:(int)page roomName:(NSString *)roomName toView:(id)toView{
    TFJunYou_Connection* p = [self addTask:act_roomList param:nil toView:toView];
    [p setPostValue:[NSNumber numberWithInt:page] forKey:@"pageIndex"];
...
-(void)listHisRoom:(int)page pageSize:(int)pageSize toView:(id)toView{
    ...
    //    TFJunYou_Connection* p = [self addTask:act_roomListHis param:[NSString stringWithFormat:@"?pageSize=%d&pageIndex=%d",TFJunYou__page_size,page] toView:toView];
    TFJunYou_Connection* p = [self addTask:act_roomListHis param:nil toView:toView];
    [p setPostValue:[NSNumber numberWithInt:pageSize] forKey:@"pageSize"];
    [p setPostValue:[NSNumber numberWithInt:page] forKey:@"pageIndex"];
...
-(void)listRoomMember:(NSString*)roomId page:(int)page toView:(id)toView{
    TFJunYou_Connection* p = [self addTask:act_roomMemberList param:nil toView:toView];
    [p setPostValue:[NSNumber numberWithInt:TFJunYou__page_size] forKey:@"pageSize"];
    [p setPostValue:[NSNumber numberWithInt:page] forKey:@"pageIndex"];
    [p setPostValue:roomId forKey:@"roomId"];
    [p setPostValue:access_token forKey:@"access_token"];
...
-(void)getRoomMember:(NSString*)roomId userId:(long)userId toView:(id)toView{
    TFJunYou_Connection* p = [self addTask:act_roomMemberGet param:nil toView:toView];
    [p setPostValue:roomId forKey:@"roomId"];
...
-(void)delRoomMember:(NSString*)roomId userId:(long)userId toView:(id)toView{
    TFJunYou_Connection* p = [self addTask:act_roomMemberDel param:nil toView:toView];
    [p setPostValue:roomId forKey:@"roomId"];
    [p setPostValue:[NSNumber numberWithLong:userId] forKey:@"userId"];
    [p setPostValue:access_token forKey:@"access_token"];
...
-(void)setRoomMember:(NSString*)roomId member:(memberData*)member toView:(id)toView{
    if(!member)
        return;
    TFJunYou_Connection* p = [self addTask:act_roomMemberSet param:nil toView:toView];
    [p setPostValue:roomId forKey:@"roomId"];
...
-(void)setRoomAdmin:(NSString*)roomId userId:(NSString*)userId type:(int)type  toView:(id)toView{
    TFJunYou_Connection* p = [self addTask:act_roomSetAdmin param:nil toView:toView];
    [p setPostValue:roomId forKey:@"roomId"];
...
-(void)setRoomInvisibleGuardian:(NSString*)roomId userId:(NSString*)userId type:(int)type toView:(id)toView{
    TFJunYou_Connection* p = [self addTask:act_roomSetInvisibleGuardian param:nil toView:toView];
    [p setPostValue:roomId forKey:@"roomId"];
    [p setPostValue:userId forKey:@"touserId"];
    [p setPostValue:[NSNumber numberWithInt:type] forKey:@"type"];
    [p setPostValue:access_token forKey:@"access_token"];
...
- (void)roomTransfer:(NSString *)roomId toUserId:(NSString *)toUserId toView:(id)toView {
    TFJunYou_Connection* p = [self addTask:act_roomTransfer param:nil toView:toView];
    [p setPostValue:roomId forKey:@"roomId"];
    [p setPostValue:toUserId forKey:@"toUserId"];
    [p setPostValue:access_token forKey:@"access_token"];
...
#ifdef IS_MsgEncrypt
    TFJunYou_UserObject *roomObj = [[TFJunYou_UserObject sharedInstance] getUserByRoomId:roomId];
...
-(void)addRoomMemberV1:(NSString*)roomId userArray:(NSArray*)array fromUserId:(NSString *)fromUserId toView:(id)toView{
    NSString * text;
    if([array count]<=0)
        return;
    SBJsonWriter * OderJsonwriter = [SBJsonWriter new];
    ...
    TFJunYou_Connection* p = [self addTask:act_roomMemberSetV1 param:nil toView:toView];
...
-(void)roomShareAddRoomId:(NSString *)roomId url:(NSString *)fileUrl fileName:(NSString *)fileName size:(NSNumber *)size type:(NSInteger)type toView:(id)toView{
    TFJunYou_Connection* p = [self addTask:act_shareAdd param:nil toView:toView];
    [p setPostValue:roomId forKey:@"roomId"];
    [p setPostValue:g_myself.userId forKey:@"userId"];
...
-(void)roomShareListRoomId:(NSString *)roomId userId:(NSString *)userId pageSize:(int)pageSize pageIndex:(int)pageIndex toView:(id)toView{
    TFJunYou_Connection* p = [self addTask:act_shareList param:nil toView:toView];
    [p setPostValue:[NSNumber numberWithInt:TFJunYou__page_size] forKey:@"pageSize"];
    [p setPostValue:[NSNumber numberWithInt:pageIndex] forKey:@"pageIndex"];
    [p setPostValue:roomId forKey:@"roomId"];
    if (userId.length > 0) {
...
-(void)roomShareGetRoomId:(NSString *)roomId shareId:(NSString *)shareId toView:(id)toView{
    TFJunYou_Connection* p = [self addTask:act_shareGet param:nil toView:toView];
    [p setPostValue:roomId forKey:@"roomId"];
...
-(void)roomShareDeleteRoomId:(NSString *)roomId shareId:(NSString *)shareId toView:(id)toView{
    TFJunYou_Connection* p = [self addTask:act_shareDelete param:nil toView:toView];
    [p setPostValue:roomId forKey:@"roomId"];
    [p setPostValue:shareId forKey:@"shareId"];
    [p setPostValue:g_myself.userId forKey:@"userId"];
    [p setPostValue:access_token forKey:@"access_token"];
...
- (void)roomMemberGetMemberListByPageWithRoomId:(NSString *)roomId joinTime:(long)joinTime toView:(id)toView {
    ...
    TFJunYou_Connection* p = [self addTask:act_roomMemberGetMemberListByPage param:nil toView:toView];
    [p setPostValue:roomId forKey:@"roomId"];
...
-(void)setPushChannelId:(NSString*)channelId{
    if(channelId==nil)
        return;
    NSString *identifier = [[NSBundle mainBundle] bundleIdentifier];
    TFJunYou_Connection* p = [self addTask:act_setPushChannelId param:nil toView:nil];
...
-(void)updateResume:(NSString*)resumeId nodeName:(NSString*)nodeName text:(NSString*)text toView:(id)toView{
    TFJunYou_Connection* p = [self addTask:act_resumeUpdate param:nil toView:toView];
...
- (void)tigaseMsgsWithReceiver:(NSString *)receiver StartTime:(long)startTime EndTime:(long)endTime PageIndex:(int)pageIndex toView:(id)toView; {
    TFJunYou_Connection* p = [self addTask:act_tigaseMsgs param:nil toView:toView];
    [p setPostValue:[NSNumber numberWithInt:pageIndex] forKey:@"pageIndex"];
...
- (void)tigaseMucMsgsWithRoomId:(NSString *)roomId StartTime:(long)startTime EndTime:(long)endTime PageIndex:(int)pageIndex PageSize:(int)pageSize toView:(id)toView {
    TFJunYou_Connection* p = [self addTask:act_tigaseMucMsgs param:nil toView:toView];
    [p setPostValue:[NSNumber numberWithInt:pageIndex] forKey:@"pageIndex"];
    [p setPostValue:[NSNumber numberWithInt:pageSize] forKey:@"pageSize"];
    [p setPostValue:[NSNumber numberWithLong:startTime] forKey:@"startTime"];
    [p setPostValue:[NSNumber numberWithLong:endTime] forKey:@"endTime"];
...
- (void)getHelperList:(int)pageIndex pageSize:(int)pageSize toView:(id)toView {
    TFJunYou_Connection* p = [self addTask:act_getHelperList param:nil toView:toView];
    [p setPostValue:[NSNumber numberWithInt:pageIndex] forKey:@"pageIndex"];
    [p setPostValue:[NSNumber numberWithInt:pageSize] forKey:@"pageSize"];
    [p setPostValue:access_token forKey:@"access_token"];
...
- (void)addGroupHelper:(NSString *)roomId roomJid:(NSString *)roomJid helperId:(NSString *)helperId toView:(id)toView {
    TFJunYou_Connection* p = [self addTask:act_addGroupHelper param:nil toView:toView];
    [p setPostValue:roomId forKey:@"roomId"];
    [p setPostValue:roomJid forKey:@"roomJid"];
...
- (void)addAutoResponse:(NSString *)roomId helperId:(NSString *)helperId keyword:(NSString *)keyword value:(NSString *)value toView:(id)toView {
    TFJunYou_Connection* p = [self addTask:act_addAutoResponse param:nil toView:toView];
    [p setPostValue:roomId forKey:@"roomId"];
...
- (void)deleteAutoResponse:(NSString *)groupHelperId keyWordId:(NSString *)keyWordId toView:(id)toView {
    TFJunYou_Connection* p = [self addTask:act_deleteAutoResponse param:nil toView:toView];
    [p setPostValue:groupHelperId forKey:@"groupHelperId"];
...
- (void)requestWithUrl:(NSString *)url toView:(id)toView {
    TFJunYou_Connection* p = [self addTask:url param:nil toView:toView];
    [p go];
}
...
- (void)userCourseAddWithMessageIds:(NSString *)messageIds CourseName:(NSString *)courseName RoomJid:(NSString *)roomJid toView:(id)toView {
    TFJunYou_Connection* p = [self addTask:act_userCourseAdd param:nil toView:toView];
    if (roomJid.length > 0) {
        [p setPostValue:roomJid forKey:@"roomJid"];
    }
    [p setPostValue:access_token forKey:@"access_token"];
...
// 登录后绑定微信号
- (void)userBindWXAccount:(TFJunYou_UserObject*)user type:(NSInteger)type openId:(NSString *)openId isLogin:(BOOL)isLogin toView:(id)toView {
...
- (void)getWxOpenId:(NSString *)code toView:(id)toView {
    TFJunYou_Connection* p = [self addTask:act_GetWxOpenId param:nil toView:toView];
    [p setPostValue:code forKey:@"code"];
    [p go];
}

- (void)wxSdkLogin:(TFJunYou_UserObject *)user type:(NSInteger)type openId:(NSString *)openId toView:(id)toView {
    
    
    NSString *identifier = [[NSBundle mainBundle] bundleIdentifier];
    
    TFJunYou_Connection* p = [self addTask:act_sdkLogin param:nil toView:toView];
    [p setPostValue:[NSNumber numberWithInteger:type] forKey:@"type"];
    [p setPostValue:openId forKey:@"loginInfo"];
...
- (void)wxSdkLoginV1:(NSString *)salt data:(NSString *)data toView:(id)toView {
    ...
    TFJunYou_Connection* p = [self addTask:act_sdkLoginV1 param:nil toView:toView];
    [p setPostValue:salt forKey:@"salt"];
    [p setPostValue:data forKey:@"data"];
    [p setPostValue:@"ios" forKey:@"deviceId"];
    [p go];
...
// 第三方绑定手机号
-(void)thirdLogin:(TFJunYou_UserObject*)user type:(NSInteger)type openId:(NSString *)openId isLogin:(BOOL)isLogin toView:(id)toView{
    //  type  第三方登录类型  1: QQ  2: 微信
    NSString *identifier = [[NSBundle mainBundle] bundleIdentifier];
    TFJunYou_Connection* p = [self addTask:act_thirdLogin param:nil toView:toView];
...
-(void)thirdLoginV1:(NSString*)userId salt:(NSString *)salt data:(NSString *)data toView:(id)toView {
    TFJunYou_Connection* p = [self addTask:act_thirdLoginV1 param:nil toView:toView];
    [p setPostValue:userId forKey:@"userId"];
    [p setPostValue:salt forKey:@"salt"];
    [p setPostValue:data forKey:@"data"];
    [p setPostValue:@"ios" forKey:@"deviceId"];
...
- (void)openCodeAuthorCheckAppId:(NSString *)appId state:(NSString *)state callbackUrl:(NSString *)callbackUrl toView:(id)toView {
    TFJunYou_Connection* p = [self addTask:act_openCodeAuthorCheck param:nil toView:toView];
    [p setPostValue:appId forKey:@"appId"];
...
- (void)roomLocationQueryWithIsQuery:(int)isQuery password:(NSString *)password toView:(id)toView{
    ...
    TFJunYou_Connection* p = [self addTask:act_RoomLocationQuery param:nil toView:toView];
    [p setPostValue:[NSNumber numberWithDouble:longitude] forKey:@"longitude"];
    [p setPostValue:[NSNumber numberWithDouble:latitude] forKey:@"latitude"];
...
- (void)payGetOrderInfoWithAppId:(NSString *)appId prepayId:(NSString *)prepayId toView:(id)toView{
    ...
    TFJunYou_Connection* p = [self addTask:act_PayGetOrderInfo param:nil toView:toView];
    [p setPostValue:appId forKey:@"appId"];
...
- (void)payPasswordPaymentWithAppId:(NSString *)appId prepayId:(NSString *)prepayId sign:(NSString *)sign time:(NSString *)time secret:(NSString *)secret toView:(id)toView {
    ...
    TFJunYou_Connection* p = [self addTask:act_PayPasswordPayment param:nil toView:toView];
    [p setPostValue:appId forKey:@"appId"];
    [p setPostValue:prepayId forKey:@"prepayId"];
...
- (void)userQrCodeLoginWithQRCodeKey:(NSString *)qrCodeKey type:(NSString *)type toView:(id)toView {
    ...
    TFJunYou_Connection* p = [self addTask:act_UserQrCodeLogin param:nil toView:toView];
...
- (void)authkeysGetLoginPrivateKeyWithUserId:(NSString *)userId salt:(NSString *)salt mac:(NSString *)mac toView:(id)toView {
    ...
    TFJunYou_Connection* p = [self addTask:act_AuthkeysGetLoginPrivateKey param:nil toView:toView];
    [p setPostValue:userId forKey:@"userId"];
    [p setPostValue:salt forKey:@"salt"];
    [p setPostValue:mac forKey:@"mac"];
    [p go];
...
- (void)authkeysUploadLoginKeyWithUserId:(NSString *)userId salt:(NSString *)salt privateKey:(NSString *)privateKey publicKey:(NSString *)publicKey mac:(NSString *)mac toView:(id)toView {
    ...
    TFJunYou_Connection* p = [self addTask:act_AuthkeysUploadLoginKey param:nil toView:toView];
    [p setPostValue:userId forKey:@"userId"];
    [p setPostValue:salt forKey:@"salt"];
    [p setPostValue:privateKey forKey:@"privateKey"];
    [p setPostValue:publicKey forKey:@"publicKey"];
...
- (void)authGetLoginCodeWithAreaCode:(NSString *)areaCode account:(NSString *)account salt:(NSString *)salt deviceId:(NSString *)deviceId mac:(NSString *)mac toView:(id)toView  {
    ...
    TFJunYou_Connection* p = [self addTask:act_AuthGetLoginCode param:nil toView:toView];
    [p setPostValue:areaCode forKey:@"areaCode"];
    [p setPostValue:salt forKey:@"salt"];
    [p setPostValue:account forKey:@"account"];
