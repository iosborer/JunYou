...
#pragma -mark 获取限定行数后内容的高度
    if(_numberOfTotalLines<=_numberOfLimitLines||_numberOfLimitLines==0){
        ...
        float line_y = (float) origins[_numberOfLimitLines -1].y;  
        CGFloat ascent;
        CGFloat descent;
        CGFloat leading;
        CTLineGetTypographicBounds(line1, &ascent, &descent, &leading);
...
#pragma -mark  解析表情图片
    if([self.images count]>0){
        ...
        for (id lineObj in lines) { 
            ...
            for (id runObj in (__bridge NSArray *)CTLineGetGlyphRuns(line1)) { 
                CTRunRef run = (__bridge CTRunRef)runObj;
                CFRange runRange = CTRunGetStringRange(run);
                if ( runRange.location==imgLocation.location) { 
                    CGRect runBounds;
                    runBounds.size.width =iconSize; 
    ...
    if([self.links count]>0){
        ...
        for (id lineObj in lines) { 
            ...
            for (id runObj in (__bridge NSArray *)CTLineGetGlyphRuns(line1)) { 
                CTRunRef run = (__bridge CTRunRef)runObj;
                CFRange runRange = CTRunGetStringRange(run);
                if ( runRange.location>=linkRange.location&&runRange.location<(linkRange.location+linkRange.length)) { 
                    CGRect runBounds;
                    CGFloat ascent;
                    CGFloat descent;
                    runBounds.size.width = CTRunGetTypographicBounds(run, CFRangeMake(0, 0), &ascent, &descent, NULL); 
                    runBounds.size.height = ascent + descent;
                    CGFloat xOffset = CTLineGetOffsetForStringIndex(line1, CTRunGetStringRange(run).location, NULL); 
