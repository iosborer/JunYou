...
#define imgVWidth (TFJunYou__SCREEN_WIDTH - 25) / 4
#define imgVHeight 131
@interface TFJunYou_SelectCoverVC ()<RITLPhotosViewControllerDelegate>
@property (nonatomic,strong)NSMutableArray *imageArray;
@property (nonatomic,strong)NSString *video;
@end
@implementation TFJunYou_SelectCoverVC
...
- (void)viewDidLoad {
    ...
    UIButton *btn = [[UIButton alloc] initWithFrame:CGRectMake(self_width-7-65, TFJunYou__SCREEN_TOP - 38, 65, 30)];
    if (THESIMPLESTYLE) {
...
- (void) createImages {
    ...
    dispatch_async(dispatch_get_global_queue(0, 0), ^{
        double integer = [TFJunYou_FileInfo getVideoTimeFromVideo:self.video];
        NSInteger num = 0;
        double interval;
        if (integer < 16) {
            interval = integer/16;
        ...
        for (int i = 0; i< num; i++) {
            UIImage *image = [TFJunYou_FileInfo getImagesFromVideo:self.video withTimeInterval:interval*(i+1)];
            if (image) {
        ...
        dispatch_async(dispatch_get_main_queue(), ^{
            ...
            self.tableBody.contentSize = CGSizeMake(TFJunYou__SCREEN_WIDTH, (imgVHeight + 5) * row - 5);
            [_wait stop];
        });
    });
}
