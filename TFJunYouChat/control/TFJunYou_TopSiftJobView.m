...
@interface TFJunYou_TopSiftJobView (){
...
@implementation TFJunYou_TopSiftJobView
...
- (void)moveBottomSlideLine:(CGFloat)offsetX{
    ...
    frame.origin.x = offsetX/(TFJunYou__SCREEN_WIDTH*2)*self.itemBtnWidth*2;
...
- (void)resetItemBtnWith:(CGFloat)offsetX{
    for (UIView *view in self.subviews) {
        if (view.tag >= 100 && view.tag <= 102) {
            ...
            if (view.tag == offsetX/TFJunYou__SCREEN_WIDTH+100) {
                btn.selected = YES;
            }else{
                btn.selected = NO;
            }
