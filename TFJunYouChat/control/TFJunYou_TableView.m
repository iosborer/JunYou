...
@implementation TFJunYou_TableView
...
-(void)dealloc{
    NSLog(@"TFJunYou_TableView.dealloc");
    [self clearPool];
    _pool = nil;
//    [super dealloc];
}
...
- (void)touchesBegan:(NSSet *)touches withEvent:(UIEvent *)event
{
    ...
    if ([_touchDelegate conformsToProtocol:@protocol(TFJunYou_TableViewDelegate)] &&
...
- (void)showEmptyImage:(EmptyType)emptyType{
    if(!_empty){
        _empty = [[UIImageView alloc]initWithFrame:CGRectMake((TFJunYou__SCREEN_WIDTH-56)/2, self.frame.size.height/5, 56, 108)];
...
//        _empty.backgroundColor = [UIColor magentaColor];
    ...
    if (!_tipLabel) {
        _tipLabel = [[UILabel alloc] initWithFrame:CGRectMake(_empty.frame.origin.x, _empty.frame.origin.y + _empty.frame.size.height + 10, TFJunYou__SCREEN_WIDTH, 30)];
...
//        _tipLabel.backgroundColor = [UIColor cyanColor];
    ...
    if (!_tipBtn) {
        ...
        _tipBtn.center = CGPointMake(TFJunYou__SCREEN_WIDTH/2, _tipBtn.center.y);
