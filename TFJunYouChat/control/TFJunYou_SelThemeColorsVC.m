...
@interface TFJunYou_SelThemeColorsVC ()
@property (nonatomic, strong) UIView *baseView;

@end

@implementation TFJunYou_SelThemeColorsVC
...
- (void)viewDidLoad {
    ...
    UILabel *tintLab = [[UILabel alloc] initWithFrame:CGRectMake(15, 11, 160, 10)];
    tintLab.text = @"来挑选符合你眼缘的颜色吧";
    tintLab.textColor = HEXCOLOR(0x999999);
    tintLab.font = SYSFONT(13);
    [self.tableBody addSubview:tintLab];
    
    self.baseView = [[UIView alloc] initWithFrame:CGRectMake(0, CGRectGetMaxY(tintLab.frame)+5, TFJunYou__SCREEN_WIDTH, TFJunYou__SCREEN_HEIGHT - CGRectGetMaxY(tintLab.frame)-TFJunYou__SCREEN_TOP)];
    ...
    resaveBtn.frame = CGRectMake(TFJunYou__SCREEN_WIDTH - 51 - 15, TFJunYou__SCREEN_TOP - 8 - 29, 51, 29);
...
- (void)setupColorViews {
    ...
    UIButton *btn;
    int inset = 8;
    CGFloat w = (TFJunYou__SCREEN_WIDTH-30 - inset*3)/4;
...
- (UIButton *)creatButtonWithFrame:(CGRect)frame color:(UIColor *)color title:(NSString *)title index:(NSInteger)index {
    ...
    UILabel *label = [[UILabel alloc] initWithFrame:CGRectMake(0, CGRectGetMaxY(colorView.frame)+10, frame.size.width, 14)];
    label.text = title;
