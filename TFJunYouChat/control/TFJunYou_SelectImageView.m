...
#define SELECTIMAGE_WIDTH 40 // 图片宽度
//动态间距
#define DWIDTH (TFJunYou__SCREEN_WIDTH - 200)/5.0
//动态间距
#define DHEIGHT (218 - 110)/3.0


@implementation TFJunYou_SelectImageView 
@synthesize delegate;
...
- (id)initWithFrame:(CGRect)frame
{
    ...
    if (self) {
        ...
        UIView* line = [[UIView alloc]initWithFrame:CGRectMake(0,0,TFJunYou__SCREEN_WIDTH * 2,LINE_WH)];
        ...
        int inset = (TFJunYou__SCREEN_WIDTH-SELECTIMAGE_WIDTH*4)/5;
...
//            X = m >8 ? SELECTIMAGE_WIDTH *n + (n+1)*inset+TFJunYou__SCREEN_WIDTH : SELECTIMAGE_WIDTH *n + (n+1)*inset;
//            Y = m > 4 && m <=8 ? h+margeY*2 : margeY;
//            button = [self createButtonWithImage:@"im_group_button_normal" highlight:@"im_group_button_press" target:delegate selector:self.onGroupHelper title:Localized(@"JX_GroupAssistants")];
//            button.frame = CGRectMake(X, Y, SELECTIMAGE_WIDTH, h);
//        }
        if (m > 8) {
            self.scrollView.contentSize = CGSizeMake(TFJunYou__SCREEN_WIDTH * 2, 0);
            self.scrollView.pagingEnabled = YES;
            
            
            _pageControl = [[UIPageControl alloc]initWithFrame:CGRectMake(100, self.frame.size.height-(THE_DEVICE_HAVE_HEAD ? 65 : 30), TFJunYou__SCREEN_WIDTH-200, 30)];
...
- (void)setHelpers:(NSArray *)helpers {
    ...
    if (!self.isWin && helpers.count <= 0) {
        UIImageView *img = [[UIImageView alloc] initWithFrame:CGRectMake((TFJunYou__SCREEN_WIDTH-60)/2, 44+20, 60, 60)];
        img.image = [UIImage imageNamed:@"group_helper_notData"];
        [self.helperScrollV addSubview:img];
        
        UILabel *label = [[UILabel alloc] initWithFrame:CGRectMake(0, CGRectGetMaxY(img.frame)+10, TFJunYou__SCREEN_WIDTH, 20)];
    ...
    // 创建群助手
    CGFloat width = TFJunYou__SCREEN_WIDTH/2;
    ...
    for (int i = 0; i < count; i++) {
        TFJunYou_GroupHeplerModel *model;
        if (i < helpers.count) {
            model = helpers[i];
        }
        UIView *view;
        if (lastView == nil) {
            ...
            int ii = ((int)maxX%(int)TFJunYou__SCREEN_WIDTH);
            if (ii <= 0) {
                int x = (int)((int)(CGRectGetMaxX(lastView.frame)+1)/(int)TFJunYou__SCREEN_WIDTH);
                if (CGRectGetMaxY(lastView.frame)+height > height*3) {
                    view = [self createButtonWithFrame:CGRectMake(x*TFJunYou__SCREEN_WIDTH, 0, width, height) iconUrl:model.helperModel.iconUrl title:model.helperModel.name index:i];
                }else {
                    view = [self createButtonWithFrame:CGRectMake((x-1)*TFJunYou__SCREEN_WIDTH, CGRectGetMaxY(lastView.frame), width, height) iconUrl:model.helperModel.iconUrl title:model.helperModel.name index:i];
                }
            }else {
    ...
    if (m > 6) {
        ...
        self.helperScrollV.contentSize = CGSizeMake(TFJunYou__SCREEN_WIDTH * num, 0);
