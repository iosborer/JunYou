...
@interface TFJunYou_TextView ()<UITextViewDelegate>

@property (nonatomic, copy) NSString *placeHolderStr;

@end

@implementation TFJunYou_TextView
...
-(void)dealloc{
//    NSLog(@"TFJunYou_TextView.dealloc");
    if(!self.disableAutoSize){
...
- (void)setPlaceHolder:(NSString *)value {
    ...
    NSUInteger maxChars = [TFJunYou_TextView maxCharactersPerLine];
...
- (NSUInteger)numberOfLinesOfText {
    return [TFJunYou_TextView numberOfLinesForMessage:self.text];
}
...
+ (NSUInteger)numberOfLinesForMessage:(NSString *)text {
    return (text.length / [TFJunYou_TextView maxCharactersPerLine]) + 1;
}
...
//    
//    [self.superview setFrame:CGRectMake(0, TFJunYou__SCREEN_HEIGHT+deltaY-self.superview.frame.size.height, self.superview.frame.size.width, self.superview.frame.size.height)];
}
