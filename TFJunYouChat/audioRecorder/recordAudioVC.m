...
void audioRouteChangeListenerCallback1 ( void   *inUserData,  AudioSessionPropertyID  inPropertyID,  UInt32 inPropertyValueSize,  const void  *inPropertyValue ){
    if(inUserData == nil)
        return;
    recordViewController *TFJunYou_recorder = (recordViewController *) inUserData;
    if(!TFJunYou_recorder.view.userInteractionEnabled)
        return;
    ...
    {
        [TFJunYou_recorder pause:nil];
        
        BOOL b = [TFJunYou_recorder getHeadsetMode];
        TFJunYou_recorder.isHeadsetTrue = b;
        [TFJunYou_recorder headsetChanged:b];
    }
...
-(void)build{
    _iv = [[TFJunYou_ImageView alloc] initWithFrame:CGRectMake(0, 0, TFJunYou__SCREEN_WIDTH, TFJunYou__SCREEN_HEIGHT-200)];
    _iv.userInteractionEnabled = YES;
    [self.view addSubview:_iv];
//    [_iv release];
    
    _effectType = [[UIScrollView alloc] initWithFrame:CGRectMake(0, _iv.frame.size.height, TFJunYou__SCREEN_WIDTH, 80)];
    _effectType.backgroundColor = HEXCOLOR(0x111111);
    [self.view addSubview:_effectType];
//    [_effectType release];

    UIView* bottom = [[UIView alloc] initWithFrame:CGRectMake(0, _iv.frame.size.height+80, TFJunYou__SCREEN_WIDTH, 120)];
...
//    [bottom release];
    ...
    for(int i=0;i<n;i++){
        TFJunYou_ImageView* iv = [[TFJunYou_ImageView alloc]initWithFrame:CGRectMake(i*70+5, 2, 65, 65)];
        iv.delegate = self;
...
//        [iv release];
    ...
    _volume = [[TFJunYou_ImageView alloc]initWithFrame:CGRectMake(0, 0, 106/2, 192/2)];
    _volume.center = _iv.center;
    _volume.image = [UIImage imageNamed:@"pub_recorder"];
    [_iv addSubview:_volume];
//    [_volume release];
    
    _input = [[TFJunYou_ImageView alloc]initWithFrame:CGRectMake(9, 1, 34, 66)];//20,1,66,132
    _input.image = [UIImage imageNamed:@"pub_microphone_volume"];
    [_volume addSubview:_input];
//    [_input release];

    _btnPlay = [[TFJunYou_ImageView alloc]initWithFrame:CGRectMake(0, 0, 60, 60)];
...
//    [_btnPlay release];
    _player = [[TFJunYou_AudioPlayer alloc]initWithParent:_btnPlay];
    _player.isOpenProximityMonitoring = NO;
    
    _btnBack = [[TFJunYou_ImageView alloc]initWithFrame:CGRectMake(40, 30, 40, 40)];
...
//    [_btnBack release];
    ...
    _btnDel = [[TFJunYou_ImageView alloc]initWithFrame:CGRectMake(40, 30, 40, 40)];
...
//    [_btnDel release];
    ...
    _btnEnter = [[TFJunYou_ImageView alloc]initWithFrame:CGRectMake(TFJunYou__SCREEN_WIDTH-80, 30, 60, 60)];
...
//    [_btnEnter release];
    ...
    _btnRecord = [[TFJunYou_ImageView alloc]initWithFrame:CGRectMake((TFJunYou__SCREEN_WIDTH-68)/2, 20, 68, 69)];
...
//    [_btnRecord release];
    ...
    _lb = [[UILabel alloc]initWithFrame:CGRectMake(0, TFJunYou__SCREEN_HEIGHT-20, TFJunYou__SCREEN_WIDTH, 20)];
    _lb.text = @"";
...
//    NSLog(@"%d,%d,%@",_mixRecorder.isPlayer,_mixRecorder.isMixSave,_mixRecorder.importAudioFile);
    ...
    _mixRecorder.outputAudioFile = [TFJunYou_FileInfo getUUIDFileName:@"mp3"];
