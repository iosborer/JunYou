//
//  TFJunYou_AudioRecorderViewController.m
//  TFJunYouChat
//
...
@interface TFJunYou_AudioRecorderViewController ()

@end

@implementation TFJunYou_AudioRecorderViewController{
    UIButton *_recordBtn;
...
-(void)dealloc{
    [self resume];
    NSLog(@"TFJunYou_AudioRecorderViewController.dealloc");
...
-(void)initSubViews{
    ...
    _recordBtn.frame = CGRectMake((TFJunYou__SCREEN_WIDTH -98)/2, (TFJunYou__SCREEN_HEIGHT-33-18-22-98), 98, 98);
...
//    [self.view addSubview:_playBtn];
    ...
    _confirmBtn.frame = CGRectMake((TFJunYou__SCREEN_WIDTH-_cancelBtn.frame.size.width -_cancelBtn.frame.origin.x), _cancelBtn.frame.origin.y, _cancelBtn.frame.size.width, _cancelBtn.frame.size.height);
    ...
    //提示语
    _noticeLabel = [[UILabel alloc] initWithFrame:CGRectMake(TFJunYou__SCREEN_WIDTH*(1-1/1.4)/2, 150, TFJunYou__SCREEN_WIDTH/1.2, TFJunYou__SCREEN_HEIGHT/3)];
...
//    [_noticeLabel release];
    ...
    //时间
    _timeBGView = [[UIImageView alloc] initWithFrame:CGRectMake((TFJunYou__SCREEN_WIDTH-200)/2, (TFJunYou__SCREEN_HEIGHT -_recordBtn.frame.size.height -100-2), 200, 2)];
...
//    [_timeLabel release];
    ...
    //返回上页
    _backBtn = [[UIButton alloc] initWithFrame:CGRectMake(0, TFJunYou__SCREEN_TOP - 46, 46, 46)];
...
-(void)confirmAudio:(UIButton *)sender{
    ...
    if (delegate && [delegate respondsToSelector:@selector(TFJunYou_audioRecorderDidFinish:TimeLen:)] ) {
        [delegate TFJunYou_audioRecorderDidFinish:_lastRecordFile TimeLen:_timeCount];
    }
    [super actionQuit];
}
