...
@interface TFJunYou_DeviceLockVC ()<NumLockViewControllerDelegate>

@property (nonatomic, strong) UISwitch *setSwitch;
@property (nonatomic, assign) BOOL isSetSwitch;

@property (nonatomic, strong) TFJunYou_ImageView *setIV;
@property (nonatomic, strong) NumLockViewController * numLockVC;

@end

@implementation TFJunYou_DeviceLockVC
...
- (void)viewDidLoad {
    ...
    int y = 0;
    TFJunYou_ImageView *iv = [self createButton:Localized(@"JX_Unlocking") drawTop:NO drawBottom:YES must:NO click:@selector(switchAction:) ParentView:self.tableBody];
    ...
    _setIV.frame = CGRectMake(0, y, TFJunYou__SCREEN_WIDTH, HEIGHT);
...
-(TFJunYou_ImageView*)createButton:(NSString*)title drawTop:(BOOL)drawTop drawBottom:(BOOL)drawBottom must:(BOOL)must click:(SEL)click ParentView:(UIView *)parent{
    ...
    //分割线
    if(drawTop){
        UIView* line = [[UIView alloc] initWithFrame:CGRectMake(15,0,TFJunYou__SCREEN_WIDTH-30,LINE_WH)];
    ...
    if(drawBottom){
        UIView* line = [[UIView alloc]initWithFrame:CGRectMake(15,HEIGHT-LINE_WH,TFJunYou__SCREEN_WIDTH-30,LINE_WH)];
        line.backgroundColor = THE_LINE_COLOR;
        [btn addSubview:line];
        //        [line release];
    }
