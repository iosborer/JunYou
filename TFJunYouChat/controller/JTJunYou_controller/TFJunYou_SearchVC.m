...
//  TFJunYouChat
//
//  Created by lifengye on 2020/8/6.
//  Copyright © 2019 zengwOS. All rights reserved.
//

#import "TFJunYou_SearchVC.h"
#import "TFJunYou_RoomMember.h"
#import "TFJunYou_ChatViewController.h"
#import "TFJunYou_SearchListVC.h"
#import "TFJunYou_SearchShowCell.h"
#import "TFJunYou_SearchMoreCell.h"
#import "TFJunYou_SearchRecordCell.h"
#import "TFJunYou_ClearAllRecordCell.h"
#import "TFJunYou_XMPP.h"
#import "TFJunYou_RoomPool.h"
@interface TFJunYou_SearchVC ()<UISearchBarDelegate,UITableViewDataSource,UITableViewDelegate,TFJunYou_SearchListDelegate, TFJunYou_SearchRecordCellDelegate>
{
    CGRect bounds;
...
@property (nonatomic,strong)TFJunYou_SearchListVC *searchListVC;
@property (nonatomic,assign)BOOL isShow;

@end

@implementation TFJunYou_SearchVC
- (instancetype)initWithTable:(NSString *)table{
    ...
    if (self) {
        ...
        self.searchRecordArray = [[TFJunYou_UserObject sharedInstance] fetchAllSearchRecordWithTable:self.tableName];
        _isShow = NO;
    }
    return self;
}
...
#pragma mark -- 界面控件
- (void)creatView{
    UIView *stastusBarView = [[UIView alloc] initWithFrame:CGRectMake(0, 0, TFJunYou__SCREEN_WIDTH, SCREEN_BAR)];
    if (THESIMPLESTYLE) {
    ...
    //基础view
    self.baseView = [[UIView alloc] initWithFrame:CGRectMake(0, SCREEN_BAR, TFJunYou__SCREEN_WIDTH, TFJunYou__SCREEN_HEIGHT-SCREEN_BAR)];
    self.baseView.backgroundColor = [UIColor whiteColor];
    [self.view addSubview:self.baseView];
    //导航栏
    UINavigationBar *bar = [[UINavigationBar alloc] initWithFrame:CGRectMake(0, 0, TFJunYou__SCREEN_WIDTH, 44)];
    if (THESIMPLESTYLE) {
    ...
    bar.translucent = NO;
    //搜索框
//    self.searchbar = [[UISearchBar alloc] initWithFrame:CGRectMake(10, 5, TFJunYou__SCREEN_WIDTH-20, 33)];
    self.searchbar = [[UISearchBar alloc] initWithFrame:CGRectMake(40, 5, TFJunYou__SCREEN_WIDTH-55, 33)];
...
//    }
    ...
    //关键字查询信息的tableView
    self.tableview = [[UITableView alloc] initWithFrame:CGRectMake(0, 44, TFJunYou__SCREEN_WIDTH, TFJunYou__SCREEN_HEIGHT-SCREEN_VIEW)];
    self.tableview.tag = 1;
    self.tableview.dataSource = self;
    self.tableview.delegate = self;
    self.tableview.backgroundColor = HEXCOLOR(0xf2f2f2);
    UIView *tableHead = [[UIView alloc] initWithFrame:CGRectMake(0, 0, TFJunYou__SCREEN_WIDTH, 8)];
    tableHead.backgroundColor = HEXCOLOR(0xf2f2f2);
    self.tableview.tableHeaderView = tableHead;
    UIView *view = [[UIView alloc] initWithFrame:CGRectMake(0, 0, TFJunYou__SCREEN_WIDTH, 8)];
    ...
    //指定内容查找
    self.searchRecordView = [[UITableView alloc] initWithFrame:CGRectMake(0, 44, TFJunYou__SCREEN_WIDTH, TFJunYou__SCREEN_HEIGHT-SCREEN_VIEW)];
    ...
    UIView *view2 = [[UIView alloc] initWithFrame:CGRectMake(0, 0, TFJunYou__SCREEN_WIDTH, 8)];
...
//读取某用户或群组对象
- (TFJunYou_UserObject *)readFromDictionary:(NSMutableDictionary *)dic withArray:(NSMutableArray *)array withNumber1:(NSInteger )i number2:(NSInteger )j{
    NSMutableArray* array1 = [dic objectForKey:array[i]];
    TFJunYou_UserObject *user = array1[j];
    return user;
}
//添加显示的模块数和模块名
- (void)displaySection{
...
- (void)getContactsWithText:(NSString *)text{
    NSMutableArray *array = [[TFJunYou_UserObject sharedInstance] searchContacts:text];
...
- (void)getGroupWithText:(NSString *)text{
    NSMutableArray *array1 = [[TFJunYou_UserObject sharedInstance] searchGroup:text withContent:@"userNickname"];
    NSMutableArray *array2 = [[TFJunYou_UserObject sharedInstance] searchGroupMember:text];
    ...
    if (array1.count != 0) {
        ...
        for (TFJunYou_UserObject *user2 in self.groupArray) {
            [roomArray addObject:user2.roomId];
        }
        for (memberData *member in array2) {
           TFJunYou_UserObject *user = [[TFJunYou_UserObject sharedInstance] groupForRoomId:member.roomId];
...
- (void)getPublicWithText:(NSString *)text{
    NSMutableArray *array = [[TFJunYou_UserObject sharedInstance] searchPublic:text];
...
- (void)getChatRecordWithText:(NSString *)text{
    ...
    self.msgDictionary = [[TFJunYou_UserObject sharedInstance] searchChatRecordWithText:text];
    array = [[TFJunYou_UserObject sharedInstance] getAllContactsAndGroup];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    if (tableView.tag == 1) {
        ...
        if (indexPath.row == 1 || indexPath.row == 2 || indexPath.row == 3) {
            TFJunYou_SearchShowCell *cell = [[TFJunYou_SearchShowCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:@"chatCell" withNewStyle:TFJunYou_SearchShowCellStyleUser];
            TFJunYou_UserObject *user = [self readFromDictionary:self.sectionDictonary withArray:self.sectionArray withNumber1:indexPath.section number2:indexPath.row-1];
        ...
        if (indexPath.row == 4) {
            TFJunYou_SearchMoreCell *cell = [[TFJunYou_SearchMoreCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:@"moreSearchCell"];
    ...
    if (indexPath.row == 0) {
        TFJunYou_ClearAllRecordCell *cell = [[TFJunYou_ClearAllRecordCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:@"historycell" withTitle:@"history"];
    ...
    if (indexPath.row == _searchRecordArray.count + 1) {
        TFJunYou_ClearAllRecordCell *cell = [[TFJunYou_ClearAllRecordCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:@"deleteallcell"];
    ...
    TFJunYou_SearchRecordCell *cell = [tableView dequeueReusableCellWithIdentifier:@"searchRecirdCell"];
    if (!cell) {
        cell = [[TFJunYou_SearchRecordCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:@"searchRecirdCell"];
    }
    cell.delegate = self;
...
- (void)tableView:(UITableView *)tableView willDisplayCell:(UITableViewCell *)cell forRowAtIndexPath:(NSIndexPath *)indexPath{
    if (tableView.tag == 1) {
        ...
        if (indexPath.row == [tableView numberOfRowsInSection:indexPath.section] - 1) {
            ...
            if ([cell isKindOfClass:[TFJunYou_SearchShowCell class]]) {
                [((TFJunYou_SearchShowCell *)cell) cutSelectedView];
            }
        }
        layer.path = pathRef;
        CFRelease(pathRef);
...
- (UIView *)tableView:(UITableView *)tableView viewForFooterInSection:(NSInteger)section{
    if (tableView.tag == 1) {
        ...
        if (!footer) {
            ...
            UIView *view = [[UIView alloc] initWithFrame:CGRectMake(0, 0, TFJunYou__SCREEN_WIDTH, 10)];
            view.backgroundColor = HEXCOLOR(0xf2f2f2);
    ...
    if (tableView.tag == 1) {
        if (indexPath.row == 1 || indexPath.row == 2 || indexPath.row == 3){
            if ([self compareSectionSring:CONTACT withIndexPath:indexPath] || [self compareSectionSring:PUBLIC withIndexPath:indexPath]){
                TFJunYou_UserObject *user = [self readFromDictionary:self.sectionDictonary withArray:self.sectionArray withNumber1:indexPath.section number2:indexPath.row-1];
                TFJunYou_ChatViewController *chatView = [[TFJunYou_ChatViewController alloc] init];
            ...
            if ([self compareSectionSring:GROUP withIndexPath:indexPath]) {
                ...
                TFJunYou_ChatViewController *chatView = [TFJunYou_ChatViewController alloc];
                if ([user.groupStatus intValue] == 0) {
                    chatView.chatRoom  = [[TFJunYou_XMPP sharedInstance].roomPool joinRoom:user.userId title:user.userNickname lastDate:nil isNew:NO];
                }
            ...
            if ([self compareSectionSring:RECORD withIndexPath:indexPath]) {
                self.searchListVC = [[TFJunYou_SearchListVC alloc] initWithLable:RECORD withSearchText:self.searchbar.text withUserArray:nil withGroupDictionary:nil withMsgDictionary:self.msgDictionary  isChatRecord:YES enterChatRecord:YES withPeople:self.recordArray[indexPath.row-1]];
        ...
        if (indexPath.row == 4) {
            if ([self compareSectionSring:CONTACT withIndexPath:indexPath]) {
                self.searchListVC = [[TFJunYou_SearchListVC alloc] initWithLable:CONTACT withSearchText:self.searchbar.text withUserArray:self.contactsArray withGroupDictionary:nil withMsgDictionary:nil isChatRecord:NO enterChatRecord:NO withPeople:nil];
                [self showListView:indexPath];
            }
            if ([self compareSectionSring:GROUP withIndexPath:indexPath]) {
                self.searchListVC = [[TFJunYou_SearchListVC alloc] initWithLable:GROUP withSearchText:self.searchbar.text withUserArray:self.groupArray withGroupDictionary:self.groupDictionary withMsgDictionary:nil  isChatRecord:NO enterChatRecord:NO withPeople:nil];
                [self showListView:indexPath];
            }
            if ([self compareSectionSring:PUBLIC withIndexPath:indexPath]) {
                self.searchListVC = [[TFJunYou_SearchListVC alloc] initWithLable:PUBLIC withSearchText:self.searchbar.text withUserArray:self.publicArray withGroupDictionary:nil withMsgDictionary:nil  isChatRecord:NO enterChatRecord:NO withPeople:nil];
                [self showListView:indexPath];
            }
            if ([self compareSectionSring:RECORD withIndexPath:indexPath]) {
                self.searchListVC = [[TFJunYou_SearchListVC alloc] initWithLable:RECORD withSearchText:self.searchbar.text withUserArray:self.recordArray  withGroupDictionary:nil withMsgDictionary:self.msgDictionary  isChatRecord:YES enterChatRecord:NO withPeople:nil];
                [self showListView:indexPath];
            }
            [self.searchbar resignFirstResponder];
            //        [self.cancelBtn setEnabled:YES];
        }
        BOOL saveSuccess = [[TFJunYou_UserObject sharedInstance] insertSearchRecord:self.searchbar.text withTable:self.tableName];
        if (!saveSuccess) {
            NSLog(@"保存搜索记录失败");
        }
        [_searchRecordArray removeAllObjects];
        _searchRecordArray = [[TFJunYou_UserObject sharedInstance] fetchAllSearchRecordWithTable:self.tableName];
    }
    if (tableView.tag == 2) {
        if (indexPath.row == _searchRecordArray.count + 1) {
            ...
            UIAlertAction *sureAction = [UIAlertAction actionWithTitle:Localized(@"JX_Confirm") style:UIAlertActionStyleDefault handler:^(UIAlertAction * _Nonnull action) {
                [_searchRecordArray removeAllObjects];
                [[TFJunYou_UserObject sharedInstance] deleteAllSearchRecordWithTable:self.tableName];
...
- (void)deleteCell:(TFJunYou_SearchRecordCell *)cell{
    ...
    [[TFJunYou_UserObject sharedInstance] deleteOneSearchRecord:msg withTable:self.tableName];
    [_searchRecordArray removeAllObjects];
    _searchRecordArray = [[TFJunYou_UserObject sharedInstance] fetchAllSearchRecordWithTable:self.tableName];
    [self.searchRecordView reloadData];
}
...
- (void)saveSearchRecord:(NSString *)searchRecord{
    BOOL saveSuccess = [[TFJunYou_UserObject sharedInstance] insertSearchRecord:searchRecord withTable:self.tableName];
    if (!saveSuccess) {
        NSLog(@"保存搜索记录失败");
    }
    [_searchRecordArray removeAllObjects];
...
//        self.searchListVC.searchbar.frame = CGRectMake(10, 5, TFJunYou__SCREEN_WIDTH-20, 33);
        [UIView animateWithDuration:0.3 animations:^{
            self.searchListVC.recordtableview.frame = CGRectMake(TFJunYou__SCREEN_WIDTH, 44, TFJunYou__SCREEN_WIDTH, TFJunYou__SCREEN_HEIGHT-SCREEN_VIEW);
...
//        self.searchListVC.searchbar.frame = CGRectMake(10, 5, TFJunYou__SCREEN_WIDTH-20, 33);
        [UIView animateWithDuration:0.3 animations:^{
            self.searchListVC.tableview.frame = CGRectMake(TFJunYou__SCREEN_WIDTH, 44, TFJunYou__SCREEN_WIDTH, TFJunYou__SCREEN_HEIGHT-SCREEN_VIEW);
        } completion:^(BOOL finished) {
            [self.searchListVC removeFromParentViewController];
            [self.searchListVC.view removeFromSuperview];
        }];
...
- (void)listDidAppear:(BOOL )hasRecord{
    if (hasRecord) {
//        self.searchListVC.searchbar.frame = CGRectMake(40, 5, TFJunYou__SCREEN_WIDTH-50, 33);
//        self.searchListVC.backbtn.frame = CGRectMake(10, 11, 20, 20);
        [UIView animateWithDuration:0.3 animations:^{
            self.searchListVC.recordtableview.frame = CGRectMake(0, 44, TFJunYou__SCREEN_WIDTH, TFJunYou__SCREEN_HEIGHT-SCREEN_VIEW);
        }];
    }else{
//        self.searchListVC.searchbar.frame = CGRectMake(40, 5, TFJunYou__SCREEN_WIDTH-50, 33);
//        self.searchListVC.backbtn.frame = CGRectMake(10, 11, 20, 20);
        [UIView animateWithDuration:0.3 animations:^{
            self.searchListVC.tableview.frame = CGRectMake(0, 44, TFJunYou__SCREEN_WIDTH, TFJunYou__SCREEN_HEIGHT-SCREEN_VIEW);
        }];
    }
}
