...
@interface TFJunYou_NewRoomVC ()<UITextFieldDelegate,TFJunYou_RoomObjectDelegate>

@property (nonatomic, assign) NSInteger roomNameLength;
@property (nonatomic, assign) NSInteger descLength;
@property (nonatomic, strong) TFJunYou_ImageView *GroupValidationBtn;
...
@implementation TFJunYou_NewRoomVC
@synthesize chatRoom;
...
#ifdef IS_MsgEncrypt
        iv = [self createButton:@"是否开启私密群组" drawTop:YES drawBottom:NO must:NO click:nil];
        iv.frame = CGRectMake(0, h, TFJunYou__SCREEN_WIDTH, HEIGHT);
        _secretSwitch = [[UISwitch alloc] init];
        _secretSwitch.frame = CGRectMake(TFJunYou__SCREEN_WIDTH-5-51,0,0,0);
...
//        _showGroupMembersSwitch.frame = CGRectMake(TFJunYou__SCREEN_WIDTH-5-51,0,0,0);
...
//        _sendCardSwitch.frame = CGRectMake(TFJunYou__SCREEN_WIDTH-5-51,0,0,0);
...
//        _readSwitch.frame = CGRectMake(TFJunYou__SCREEN_WIDTH-5-51,0,0,0);
...
//            _publicSwitch.frame = CGRectMake(TFJunYou__SCREEN_WIDTH-5-51,0,0,0);
...
//        self.GroupValidationBtn.frame = CGRectMake(0, h, TFJunYou__SCREEN_WIDTH, HEIGHT);
//        self.GroupValidationSwitch = [[UISwitch alloc] init];
//        self.GroupValidationSwitch.frame = CGRectMake(TFJunYou__SCREEN_WIDTH-5-51,0,0,0);
...
-(void)dealloc{
//    NSLog(@"TFJunYou_NewRoomVC.dealloc");
//    [_room release];
//    [super dealloc];
}
...
-(TFJunYou_ImageView*)createButton:(NSString*)title drawTop:(BOOL)drawTop drawBottom:(BOOL)drawBottom must:(BOOL)must click:(SEL)click{
    ...
    TFJunYou_Label* p = [[TFJunYou_Label alloc] initWithFrame:CGRectMake(15, 0, 230, HEIGHT)];
    p.text = title;
    p.font = g_factory.font16;
    p.backgroundColor = [UIColor clearColor];
...
- (void)textLong12:(UITextField *)textField
{
    ...
    if (length > 20) {
        ...
        [TFJunYou_MyTools showTipView:Localized(@"JX_CannotEnterMore")];
    }
...
-(UILabel*)createLabel:(UIView*)parent default:(NSString*)s{
    UILabel* p = [[UILabel alloc] initWithFrame:CGRectMake(TFJunYou__SCREEN_WIDTH/2,INSETS,TFJunYou__SCREEN_WIDTH/2 - 15,HEIGHT-INSETS*2)];
...
//    }
        ...
        if (!g_msgUtil.rsaPublicKey || g_msgUtil.rsaPublicKey.length <= 0) {
            ...
            if (_secretSwitch.isOn) {
                ...
                [TFJunYou_MyTools showTipView:@"不支持创建私密群组"];
        ...
        if (_roomName.text.length <= 0) {
            [TFJunYou_MyTools showTipView:@"请输入有效的群组名称"];
            return;
        }
        ...
        _chatRoom = [[TFJunYou_XMPP sharedInstance].roomPool createRoom:s title:_roomName.text];
...
-(void) didServerResultSucces:(TFJunYou_Connection*)aDownload dict:(NSDictionary*)dict array:(NSArray*)array1{
    ...
    if( [aDownload.action isEqualToString:act_roomAdd] ){
        ...
        _chatRoom = [[TFJunYou_XMPP sharedInstance].roomPool createRoom:_room.roomJid title:_roomName.text];
        _chatRoom.delegate = self;
...
//        _room.call = [NSString stringWithFormat:@"%@",[dict objectForKey:@"call"]];
        ...
        if (self.isAddressBook) {
            ...
            for (NSInteger i = 0; i < self.addressBookArr.count; i ++) {
                TFJunYou_AddressBook *ab = self.addressBookArr[i];
