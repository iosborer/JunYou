...
@implementation TFJunYou__SelectMenuView
...
- (instancetype)initWithTitle:(NSArray *)titleArr image:(NSArray *)images cellHeight:(int)height {
    ...
    if (self) {
        ...
        //每格高度heigth
        //大小
        self.frame = CGRectMake(0, 0, TFJunYou__SCREEN_WIDTH, TFJunYou__SCREEN_HEIGHT);
        ...
        if (images.count > 0) {  // 有图标
            point = CGPointMake(TFJunYou__SCREEN_WIDTH - (size.width+INSET_INT*3+IMAGE_W) - 10, TFJunYou__SCREEN_TOP);
            bigView.frame = CGRectMake(point.x, point.y+3 , size.width+INSET_INT*3+IMAGE_W, titleArr.count *height+topHeight);
        } else {  // 无图标
            point = CGPointMake(TFJunYou__SCREEN_WIDTH - (size.width + INSET_INT*2) - 10, TFJunYou__SCREEN_TOP);
