...
@interface TFJunYou_NearVC () <UICollectionViewDelegate,UICollectionViewDataSource,UICollectionViewDelegateFlowLayout,TFJunYou_ActionSheetVCDelegate>
{
...
@implementation TFJunYou_NearVC
...
- (void)viewDidLoad
{
    ...
    btn.frame = CGRectMake(TFJunYou__SCREEN_WIDTH-100, TFJunYou__SCREEN_TOP - 38, 30, 30);
    [self.tableHeader addSubview:btn];
    
    btn = [UIFactory createButtonWithRect:CGRectMake(TFJunYou__SCREEN_WIDTH-17-15, TFJunYou__SCREEN_TOP - 13-18, 17, 18) title:nil titleFont:nil titleColor:nil normal:@"near_screening" selected:nil selector:@selector(onScreening) target:self];
    btn.custom_acceptEventInterval = 1.0f;
    //        UIButton* btn = [UIFactory createButtonWithTitle:Localized(@"JXNearVC_AddFriends") titleFont:[UIFont systemFontOfSize:15] titleColor:[UIColor whiteColor] normal:nil highlight:nil];
    //        [btn addTarget:self action:@selector(onSearch) forControlEvents:UIControlEventTouchUpInside];
    [self.tableHeader addSubview:btn];
...
- (void) customView {
    ...
    _topSiftView.titleSelectedColor = THEMECOLOR;
    _topSiftView.titleNormalColor = HEXCOLOR(0x222222);
...
- (void)onScreening {
    TFJunYou_ActionSheetVC *actionVC = [[TFJunYou_ActionSheetVC alloc] initWithImages:@[] names:@[Localized(@"JX_OnlySeeFemale"),Localized(@"JX_OnlyLookAtMen"),Localized(@"JX_NoGender")]];
...
//    }
    ...
    if(indexPath.row == [self collectionView:_collectionView numberOfItemsInSection:indexPath.section]-1){
        ...
        if (_nearArray.count < TFJunYou__page_size) {    // 不足一页，不翻页
            flag = NO;
        }
...
//        if (!_mapVC) {
//            _mapVC = [[TFJunYou_LocMapVC alloc]initWithFrame:CGRectMake(0, 0, TFJunYou__SCREEN_WIDTH, self.tableBody.frame.size.height) andType:2];
...
//    BOOL isShowGoo = [g_default boolForKey:kUseGoogleMap];
    if (g_config.isChina) {
        ...
        if (!_goomapVC) {
            _goomapVC = [[TFJunYou_GooMapVC alloc] initWithFrame:CGRectMake(0, 0, TFJunYou__SCREEN_WIDTH, self.tableBody.frame.size.height) andType:2];
...
//        _refreshCount++;
//        [_table reloadData];
        _refreshFooter.hidden = [array1 count] < TFJunYou__page_size;
...
//-(void)buildMenu{
//    _tb = [TFJunYou_TopMenuView alloc];
//    _tb.items = [NSArray arrayWithObjects:@"最新",@"最热",@"附近",nil];
//    _tb.delegate = self;
//    _tb.onClick  = @selector(actionSegment:);
//    [_tb initWithFrame:CGRectMake(0, TFJunYou__SCREEN_TOP, TFJunYou__SCREEN_WIDTH, 44)];
//    [_tb selectOne:0];
...
//    [_topSiftView resetBottomLineIndex:0];
    ...
    TFJunYou_SearchUserVC* vc = [[TFJunYou_SearchUserVC alloc]init];
    vc.delegate  = self;
