...
@interface TFJunYou_AutoReplyAideVC ()
...
@implementation TFJunYou_AutoReplyAideVC
...
- (void)customView {
    ...
    _addBtn = [[UIButton alloc] initWithFrame:CGRectMake(TFJunYou__SCREEN_WIDTH - 65, 33, 50, 24)];
    ...
    UIView *lineView = [[UIView alloc] initWithFrame:CGRectMake(15, CGRectGetMaxY(imageView.frame) + 20, TFJunYou__SCREEN_WIDTH - 15, LINE_WH)];
    lineView.backgroundColor = THE_LINE_COLOR;
    [self.tableBody addSubview:lineView];
    
    label = [[UILabel alloc] initWithFrame:CGRectMake(15, CGRectGetMaxY(lineView.frame), TFJunYou__SCREEN_WIDTH - 30, 50)];
    ...
    lineView = [[UIView alloc] initWithFrame:CGRectMake(15, CGRectGetMaxY(label.frame), TFJunYou__SCREEN_WIDTH, LINE_WH)];
    lineView.backgroundColor = THE_LINE_COLOR;
    [self.tableBody addSubview:lineView];
    
    
    UILabel *deveLab = [[UILabel alloc] initWithFrame:CGRectMake(15, CGRectGetMaxY(lineView.frame)+20, TFJunYou__SCREEN_WIDTH-30, 20)];
    ...
    CGSize size = [tintLab.text boundingRectWithSize:CGSizeMake(TFJunYou__SCREEN_WIDTH-30, MAXFLOAT) options:NSStringDrawingUsesLineFragmentOrigin attributes:@{NSFontAttributeName:tintLab.font} context:nil].size;
    tintLab.frame = CGRectMake(15, CGRectGetMaxY(deveLab.frame), TFJunYou__SCREEN_WIDTH-30, size.height);
    
    if (self.model.type == 1  || self.model.type == 2) {
        _baseView = [[UIView alloc] initWithFrame:CGRectMake(0, CGRectGetMaxY(tintLab.frame)+20, TFJunYou__SCREEN_WIDTH, self.tableBody.frame.size.height-CGRectGetMaxY(tintLab.frame)-20)];
        [self.tableBody addSubview:_baseView];
        
        lineView = [[UIView alloc] initWithFrame:CGRectMake(0, 0, TFJunYou__SCREEN_WIDTH, 30)];
        lineView.backgroundColor = HEXCOLOR(0xF2F2F2);
        [_baseView addSubview:lineView];
        
        label = [[UILabel alloc] initWithFrame:CGRectMake(15, 0, TFJunYou__SCREEN_WIDTH - 15, 30)];
        label.font = [UIFont systemFontOfSize:14.0];
        label.numberOfLines = 0;
        label.textColor = [UIColor lightGrayColor];
        ...
        _manageBtn = [[UIButton alloc] initWithFrame:CGRectMake(TFJunYou__SCREEN_WIDTH - 55, 0, 45, 30)];
        ...
        _keyView = [[UIView alloc] initWithFrame:CGRectMake(0, CGRectGetMaxY(lineView.frame), TFJunYou__SCREEN_WIDTH, 0)];
        [_baseView addSubview:_keyView];
        
        _noKeyView = [[UIView alloc] initWithFrame:CGRectMake(0, (_baseView.frame.size.height-140)/2, TFJunYou__SCREEN_WIDTH, 140)];
        [_baseView addSubview:_noKeyView];
        
        imageView = [[UIImageView alloc] initWithFrame:CGRectMake((TFJunYou__SCREEN_WIDTH - 50) / 2, 0, 50, 50)];
        imageView.image = [UIImage imageNamed:@"ALOGO_120"];
        [_noKeyView addSubview:imageView];
        
        label = [[UILabel alloc] initWithFrame:CGRectMake(0, CGRectGetMaxY(imageView.frame), TFJunYou__SCREEN_WIDTH, 30)];
        label.font = [UIFont systemFontOfSize:14.0];
        ...
        UIButton *btn = [[UIButton alloc] initWithFrame:CGRectMake((TFJunYou__SCREEN_WIDTH - 150)/2, CGRectGetMaxY(label.frame) + 10, 150, 30)];
...
- (void)onAdd:(UIButton *)button {
    if (button.selected) {
        for (TFJunYou_GroupHeplerModel *hModel in self.groupHelperArr) {
...
- (void)keyManage {
    ...
    TFJunYou_ReplyAideKeyManageVC *vc = [[TFJunYou_ReplyAideKeyManageVC alloc] init];
    vc.keys = self.keys;
...
- (void)createKeys {
    ...
    for (NSInteger i = 0; i < _keys.count; i ++) {
        ...
        UILabel *label = [[UILabel alloc] initWithFrame:CGRectMake(15, y, TFJunYou__SCREEN_WIDTH - 15, 50)];
        label.font = [UIFont systemFontOfSize:16.0];
        label.text = [dic objectForKey:@"keyWord"];
        [_keyView addSubview:label];
        
        UIView *lineView = [[UIView alloc] initWithFrame:CGRectMake(15, CGRectGetMaxY(label.frame), TFJunYou__SCREEN_WIDTH - 15, LINE_WH)];
        lineView.backgroundColor = THE_LINE_COLOR;
    ...
    _keyView.frame = CGRectMake(_keyView.frame.origin.x, _keyView.frame.origin.y, TFJunYou__SCREEN_WIDTH, y);
