...
@interface TFJunYou_UserInfoVC ()<TFJunYou_ReportUserDelegate,UITextFieldDelegate,TFJunYou_SelectMenuViewDelegate>
...
@implementation TFJunYou_UserInfoVC
@synthesize user;
...
- (void)setupServers {
    ...
    if (![self.userId isEqualToString:MY_USER_ID] && ![self.userId isEqualToString:@"18938880001"]) {
        ...
        btn.frame = CGRectMake(TFJunYou__SCREEN_WIDTH-24-8, TFJunYou__SCREEN_TOP - 34, 24, 24);
        [self.tableHeader addSubview:btn];
    }

    
    _baseView = [[UIView alloc] initWithFrame:CGRectMake(0, 0, TFJunYou__SCREEN_WIDTH, 0)];
    ...
    _head = [[TFJunYou_ImageView alloc]initWithFrame:CGRectMake(TFJunYou__SCREEN_WIDTH/2-70/2, 40, 70, 70)];
    ...
    _serverName = [[UILabel alloc] initWithFrame:CGRectMake(0, CGRectGetMaxY(_head.frame)+15, TFJunYou__SCREEN_WIDTH, 16)];
    ...
    _tintLab = [[UILabel alloc] initWithFrame:CGRectMake(30, CGRectGetMaxY(_serverName.frame)+20, TFJunYou__SCREEN_WIDTH-60, 40)];
    _tintLab.text = @"说点什么~~";
    ...
    _btn = [[UIButton alloc] initWithFrame:CGRectMake(TFJunYou__SCREEN_WIDTH/2-50, CGRectGetMaxY(_tintLab.frame)+40, 100, 16)];
...
- (void)setServerDetail {
    ...
    CGSize size = [_tintLab.text boundingRectWithSize:CGSizeMake(TFJunYou__SCREEN_WIDTH-60, MAXFLOAT) options:NSStringDrawingUsesLineFragmentOrigin attributes:@{NSFontAttributeName:SYSFONT(15)} context:nil].size;
...
- (void)createViews {
    ...
    int Head_height = 132;
    UIView *headView = [[UIView alloc] initWithFrame:CGRectMake(0, 0, TFJunYou__SCREEN_WIDTH, Head_height)];
    headView.backgroundColor = [UIColor whiteColor];
    [self.tableBody addSubview:headView];
    
    _head = [[TFJunYou_ImageView alloc]initWithFrame:CGRectMake(15, (Head_height-83)/2, 83, 83)];
    _head.layer.cornerRadius = _head.frame.size.width/2;
    _head.layer.masksToBounds = YES;
    _head.didTouch = @selector(onHeadImage);
    _head.delegate = self;
...
//    [headView addSubview:_city];
    ...
    UIView *line = [[UIView alloc] initWithFrame:CGRectMake(0, Head_height-LINE_WH, TFJunYou__SCREEN_WIDTH, LINE_WH)];
...
//    }
    ...
    TFJunYou_UserObject *friend = [[TFJunYou_UserObject sharedInstance] getUserById:self.userId];
    ...
    if ((friend && [status intValue] != friend_status_none)  || [self.userId isEqualToString:g_myself.userId]) {
        ...
        // 生活圈
        iv = [self createButton:Localized(@"JX_LifeCircle") drawTop:NO drawBottom:YES must:NO click:@selector(onMyBlog) superView:self.tableBody];
        iv.frame = CGRectMake(0, h, TFJunYou__SCREEN_WIDTH, HEIGHT);
        h+=iv.frame.size.height;
        _lifeImgV = iv;
    }
...
//    h+=iv.frame.size.height;
    ...
    // 个性签名
    iv = [self createButton:@"个性签名" drawTop:YES drawBottom:NO must:NO click:nil superView:self.tableBody];
    iv.frame = CGRectMake(0, h, TFJunYou__SCREEN_WIDTH, HEIGHT);
    _desImgV = iv;
...
//    h+=iv.frame.size.height;
    ...
    _baseView = [[UIView alloc] initWithFrame:CGRectMake(0, h, TFJunYou__SCREEN_WIDTH, 0)];
    [self.tableBody addSubview:_baseView];
    h = 0;
...
#pragma mark 消息免打扰
   ...
   /* if ([g_myself.telephone rangeOfString:@"18938880001"].location != NSNotFound) {
        ...
        _tel = [[UILabel alloc] initWithFrame:CGRectMake(0,INSETS,TFJunYou__SCREEN_WIDTH - INSETS - 20 - 5,HEIGHT-INSETS*2)];
...
//        [iv addSubview:_tel];
    ...
    if (self.tableBody.frame.size.height < CGRectGetMaxY(_baseView.frame)+30) {
        self.tableBody.contentSize = CGSizeMake(TFJunYou__SCREEN_WIDTH, CGRectGetMaxY(_baseView.frame)+30);
    }
    
//    TFJunYou_UserObject *user = [[TFJunYou_UserObject sharedInstance] getUserById:_userId];
//    [self setUserInfo:user];
}

- (void)onCirclePermissions {
    TFJunYou_CirclePermissionsVC *vc = [[TFJunYou_CirclePermissionsVC alloc] init];
    vc.user = self.user;
...
- (void) setUserInfo:(TFJunYou_UserObject *)user {
...
- (void)updateSubviewFrameIsHide:(BOOL)isHide {
    ...
    _circleImgV.frame = CGRectMake(0, y-HEIGHT-8, TFJunYou__SCREEN_WIDTH, HEIGHT);
    
//    _lifeImgV.frame = CGRectMake(0, y, TFJunYou__SCREEN_WIDTH, HEIGHT);
    ...
    _birthdayImgV.frame = CGRectMake(0, y, TFJunYou__SCREEN_WIDTH, HEIGHT);
    if ([user.showLastLoginTime intValue] > 0 && [user.userType intValue] != 2){
        y += HEIGHT;
        _lastTImgV.frame = CGRectMake(0, y, TFJunYou__SCREEN_WIDTH, HEIGHT);
    }
    if (user.telephone.length > 0 && [user.userType intValue] != 2 && [g_config.regeditPhoneOrName intValue] == 0) {
        y += HEIGHT;
        _showNImgV.frame = CGRectMake(0, y, TFJunYou__SCREEN_WIDTH, HEIGHT);
    }
    if (user.userDescription.length > 0) {
        y += HEIGHT;
        CGSize size = [user.userDescription boundingRectWithSize:CGSizeMake(TFJunYou__SCREEN_WIDTH/2-15, MAXFLOAT) options:NSStringDrawingUsesLineFragmentOrigin attributes:@{NSFontAttributeName:SYSFONT(15)} context:nil].size;
        int h = HEIGHT;
        if (size.height > 20) {
            // 大于一行，就改变高度
        ...
        _desImgV.frame = CGRectMake(0, y, TFJunYou__SCREEN_WIDTH, h);
        y += h;
    }else {
        y += HEIGHT;
    }
    
    _baseView.frame = CGRectMake(0, y+INSETS, TFJunYou__SCREEN_WIDTH, _baseView.frame.size.height);
...
-(void)dealloc{
    NSLog(@"TFJunYou_UserInfoVC.dealloc");
...
//            _friendStatus = friend_status_see;//单向关注
        ...
        if ([self.user.userType intValue] == 2) {
            ...
            TFJunYou_ChatViewController *chatVC=[TFJunYou_ChatViewController alloc];
...
-(UILabel*)createLabel:(UIView*)parent default:(NSString*)s{
    UILabel* p = [[UILabel alloc] initWithFrame:CGRectMake(TFJunYou__SCREEN_WIDTH/2,(HEIGHT-15)/2,TFJunYou__SCREEN_WIDTH/2 - 15,15)];
...
//    NSLog(@"newReceipt");
    ...
    if([msg.type intValue] == XMPP_TYPE_BLACK){//拉黑
        ...
        _friendStatus = [user.status intValue];
        [[TFJunYou_XMPP sharedInstance].blackList addObject:user.userId];
        [user update];
        [self showAddFriend];
        [g_server showMsg:Localized(@"JXAlert_AddBlackList")];
        
        [g_notify postNotificationName:kXMPPNewFriendNotifaction object:nil];
//        [TFJunYou_MessageObject msgWithFriendStatus:user.userId status:_friendStatus];
//        [user notifyDelFriend];
    ...
    if([msg.type intValue] == XMPP_TYPE_NOBLACK){//取消拉黑
        ...
        _friendStatus = friend_status_friend;
        [self showAddFriend];
        if ([[TFJunYou_XMPP sharedInstance].blackList containsObject:user.userId]) {
            [[TFJunYou_XMPP sharedInstance].blackList removeObject:user.userId];
            [TFJunYou_MessageObject msgWithFriendStatus:user.userId status:friend_status_friend];
        }
...
-(void)showAddFriend{
    CGFloat inset = (TFJunYou__SCREEN_WIDTH-200)/3;
    _notFocusBtn.hidden = YES;
    switch (_friendStatus) {
        ...
        case friend_status_see:
            if([user.isBeenBlack boolValue])
                ...
                if ([self.user.userType intValue] == 2) {
                    ...
                    _btn.frame = CGRectMake(TFJunYou__SCREEN_WIDTH/2-50, _btn.frame.origin.y, _btn.frame.size.width, _btn.frame.size.height);
                }else {
        ...
        case friend_status_friend:
            if([user.isBeenBlack boolValue])
                ...
                if ([self.user.userType intValue] == 2 ) {
                    ...
                    if (_btn.hidden) {
                        _notFocusBtn.frame = CGRectMake(TFJunYou__SCREEN_WIDTH/2-_notFocusBtn.frame.size.width/2, _notFocusBtn.frame.origin.y, _notFocusBtn.frame.size.width, _notFocusBtn.frame.size.height);
                    }else {
                        _notFocusBtn.frame = CGRectMake(TFJunYou__SCREEN_WIDTH-inset-100, _notFocusBtn.frame.origin.y, _notFocusBtn.frame.size.width, _notFocusBtn.frame.size.height);
                    }
...
//    //模糊背景
//    _bgBlackAlpha = [[UIView alloc]initWithFrame:CGRectMake(0, 0, TFJunYou__SCREEN_WIDTH, TFJunYou__SCREEN_HEIGHT)];
...
////    [_bgBlackAlpha release];
    ...
    //自定义View
    _selectView = [[TFJunYou__SelectMenuView alloc] initWithTitle:_titleArr image:@[] cellHeight:CellHeight];
