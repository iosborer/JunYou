...
@interface TFJunYou_ProgressVC ()

@end

@implementation TFJunYou_ProgressVC
...
- (void)customView{
    //按钮
//    _comBtn = [[UIButton alloc]initWithFrame:CGRectMake(TFJunYou__SCREEN_WIDTH/2 - 50, 300, 100, 30)];
...
//    [_comBtn release];
    //本地与服务器好友数量
//    _dbCountLabel = [[UILabel alloc]initWithFrame:CGRectMake(TFJunYou__SCREEN_WIDTH/2 - 100, 160, 200, 30)];
...
//    [_dbCountLabel release];
    ...
    _sysCountLabel = [[UILabel alloc]initWithFrame:CGRectMake(TFJunYou__SCREEN_WIDTH/2 - 100, 200, 200, 30)];
...
-(void)createProgressView{
    //ProgressView
    UIView * centerView = [[UIView alloc]initWithFrame:CGRectMake(50, 50, TFJunYou__SCREEN_WIDTH - 100, 80)];
    centerView.backgroundColor = [UIColor whiteColor];
    centerView.layer.cornerRadius = 10;
    [self.tableBody addSubview:centerView];
//        [centerView release];
    
//    _titleLabel = [[UILabel alloc]initWithFrame:CGRectMake(0, 5, TFJunYou__SCREEN_WIDTH - 100, 25)];
...
//    
    ...
    _progressView = [[UIProgressView alloc]initWithFrame:CGRectMake(5, 40, TFJunYou__SCREEN_WIDTH - 110, 5)];
    [centerView addSubview:_progressView];
//    [_progressView release];
    
    
    _progressLabel = [[UILabel alloc]initWithFrame:CGRectMake(0, 50, TFJunYou__SCREEN_WIDTH - 100, 20)];
...
////        [TFJunYou_MyTools showTipView:Localized(@"JXAlert_SynchFirend")];
...
//    _titleLabel.text = @"正在同步好友";
    ...
    // 删除本地数据库好友 重新插入
//    [[TFJunYou_UserObject sharedInstance] deleteAllUser];
...
//        [user release];
    ...
    for (NSInteger i = 0; i < arr.count; i ++) {
        TFJunYou_UserObject *locUser = arr[i];
        //        BOOL flag = NO;
        //        for (NSInteger j = 0; j < array1.count; j ++) {
        //            NSDictionary * dict = array1[j];
        //            TFJunYou_UserObject * serverUser = [[TFJunYou_UserObject alloc]init];
        //            //数据转为一个好友对象
...
//    [[TFJunYou_XMPP sharedInstance] performSelector:@selector(login) withObject:nil afterDelay:1];//1秒后执行xmpp登录
