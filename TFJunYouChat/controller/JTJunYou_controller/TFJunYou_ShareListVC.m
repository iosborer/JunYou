...
@implementation TFJunYou_ShareListVC
...
- (void)customView{
    ...
    //底部选择项
    _listView = [[UIView alloc] initWithFrame:CGRectMake(0, TFJunYou__SCREEN_HEIGHT, TFJunYou__SCREEN_WIDTH, 0)];
...
- (void)showShareList{
    [UIView animateWithDuration:0.2 animations:^{
        self.view.alpha = 1;
        CGRect frame = _listView.frame;
        frame.origin.y = TFJunYou__SCREEN_HEIGHT - frame.size.height;
        _listView.frame = frame;
    }];
}
...
- (void)hideShareList{
    [UIView animateWithDuration:0.2 animations:^{
        self.view.alpha = 0;
        CGRect frame = _listView.frame;
        frame.origin.y = TFJunYou__SCREEN_HEIGHT;
        _listView.frame = frame;
...
- (void)dealloc {
    NSLog(@"TFJunYou_ShareListVC -- dealloc");
}
