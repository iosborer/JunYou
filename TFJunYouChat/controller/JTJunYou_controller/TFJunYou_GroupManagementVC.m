...
@interface TFJunYou_GroupManagementVC ()
...
@implementation TFJunYou_GroupManagementVC
...
- (void)onCopyRoom {
    TFJunYou_CopyRoomVC *vc = [[TFJunYou_CopyRoomVC alloc] init];
    vc.room = self.room;
...
- (UISwitch *)createSwitchWithParent:(UIView *)parent tag:(NSInteger)tag isOn:(BOOL)isOn{
    ...
    switchView.frame = CGRectMake(TFJunYou__SCREEN_WIDTH-INSETS-51,0,0,0);
...
- (UILabel *)createLabelWithParent:(UIView *)parent frameY:(CGFloat)framey text:(NSString *)text {
    CGSize size = [text boundingRectWithSize:CGSizeMake(TFJunYou__SCREEN_WIDTH - 20, MAXFLOAT) options:NSStringDrawingUsesLineFragmentOrigin attributes:@{NSFontAttributeName:[UIFont systemFontOfSize:13.0]} context:nil].size;
    UILabel *label = [[UILabel alloc] initWithFrame:CGRectMake(15, framey, TFJunYou__SCREEN_WIDTH - 20, size.height)];
...
-(void)specifyAdministrator{
    [self setManagerWithType:TFJunYou_SelUserTypeSpecifyAdmin];
}
//设置隐身人
- (void)specifyInvisibleMan {
    [self setManagerWithType:TFJunYou_SelUserTypeRoomInvisibleMan];
}

// 设置监控人
- (void)specifyMonitorPeople {
    [self setManagerWithType:TFJunYou_SelUserTypeRoomMonitorPeople];
}

- (void)setManagerWithType:(TFJunYou_SelUserType)type {
    ...
    selVC.delegate = self;
    if (type == TFJunYou_SelUserTypeSpecifyAdmin) {
        selVC.didSelect = @selector(specifyAdministratorDelegate:);
    }else if(type == TFJunYou_SelUserTypeRoomInvisibleMan) {
...
- (void)roomTransferAction {
    TFJunYou_SelFriendVC * selVC = [[TFJunYou_SelFriendVC alloc] init];
    selVC.room = _room;
    selVC.type = TFJunYou_SelUserTypeRoomTransfer;
    selVC.delegate = self;
