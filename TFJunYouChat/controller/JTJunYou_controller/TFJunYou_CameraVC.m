...
#import "GPUImage.h"
#import "TFJunYou_GPUImageBeautifyFilter.h"
...
@interface TFJunYou_CameraVC () <KKImageEditorDelegate,UINavigationControllerDelegate>
...
@property (nonatomic, strong) TFJunYou_GPUImageBeautifyFilter *beautifyFilter;
...
@implementation TFJunYou_CameraVC
...
//        [self dismissViewControllerAnimated:YES completion:nil];
    ...
    // 添加上下两个地方的透明模板
    UIView *topView = [[UIView alloc] initWithFrame:CGRectMake(0, 0, TFJunYou__SCREEN_WIDTH, THE_DEVICE_HAVE_HEAD ? 62 : 42)];
    UIView *botView = [[UIView alloc] initWithFrame:CGRectMake(0, TFJunYou__SCREEN_HEIGHT-90, TFJunYou__SCREEN_WIDTH, 90)];
    ...
    _photoCaptureButton.frame = CGRectMake(round(TFJunYou__SCREEN_WIDTH / 2.0 - 50.0 / 2.0), TFJunYou__SCREEN_HEIGHT - 70.0, 50.0, 50.0);
...
//    [_photoCaptureButton addTarget:self action:@selector(singlePress:) forControlEvents:UIControlEventTouchUpInside];
    ...
    UIButton *cancelBtn = [[UIButton alloc] initWithFrame:CGRectMake(15, TFJunYou__SCREEN_HEIGHT - 60.0, 30, 30)];
    ...
    UIButton *switchBtn = [[UIButton alloc] initWithFrame:CGRectMake(TFJunYou__SCREEN_WIDTH - 45, TFJunYou__SCREEN_HEIGHT - 60.0, 30, 30)];
...
//    });
    ...
    // 显示美颜调整界面
    _beautyBtn = [[UIButton alloc] initWithFrame:CGRectMake(TFJunYou__SCREEN_WIDTH - 30-18, THE_DEVICE_HAVE_HEAD ? 32 : 12, 18, 18)];
...
//    [self addPhotoStyle:primaryView];
    ...
    _iv = [[UIImageView alloc] initWithFrame:CGRectMake(0, 0, TFJunYou__SCREEN_WIDTH, TFJunYou__SCREEN_HEIGHT)];
    ...
    UIButton *cancelImageBtn = [self createButtonWithFrame:CGRectMake(20, TFJunYou__SCREEN_HEIGHT-20-80, 80, 80)  image:@"video_return" action:@selector(cancelImageBtnAction:)];
    [_iv addSubview:cancelImageBtn];
    
    UIButton *editBtn = [self createButtonWithFrame:CGRectMake(TFJunYou__SCREEN_WIDTH/2-40, TFJunYou__SCREEN_HEIGHT-20-80, 80, 80)  image:@"video_edit" action:@selector(editImageBtnAction:)];
    [_iv addSubview:editBtn];
    
    UIButton *confirmBtn = [self createButtonWithFrame:CGRectMake(TFJunYou__SCREEN_WIDTH-20-80, TFJunYou__SCREEN_HEIGHT-20-80, 80, 80) image:@"video_gou" action:@selector(confirmBtnAction:)];
...
//            }
            ...
            _player.type = TFJunYou_VideoTypePreview;
...
//                CGFloat scale = TFJunYou__SCREEN_WIDTH / TFJunYou__SCREEN_HEIGHT;
...
- (void) isVideoCustomView{
    _noticeLabel = [[UILabel alloc] initWithFrame:CGRectMake(45, 0, TFJunYou__SCREEN_WIDTH-45*2, 45)];
...
//    [self noticeLabelHidden:NO textType:1];
    ...
    //时间
    _timerNum = 0;
    _timeBGView = [[UIImageView alloc] initWithFrame:CGRectMake((TFJunYou__SCREEN_WIDTH-210)/2,  THE_DEVICE_HAVE_HEAD ? TFJunYou__SCREEN_TOP/2+5 : TFJunYou__SCREEN_TOP/2-15, 210, 2)];
//    _timeBGView = [[UIImageView alloc] initWithFrame:CGRectMake((TFJunYou__SCREEN_WIDTH-210)/2, (TFJunYou__SCREEN_HEIGHT-TFJunYou__SCREEN_WIDTH)/2-35, 210, 2)];
...
- (void) addPhotoStyle:(UIView *)parentView{
    _scrollView = [[UIScrollView alloc] initWithFrame:CGRectMake(0, 20, TFJunYou__SCREEN_WIDTH, 60)];
...
//                       ];
    //美颜
    TFJunYou_GPUImageBeautifyFilter *BeautifyFilter = [[TFJunYou_GPUImageBeautifyFilter alloc] init];
    //哈哈镜效果
...
- (void) selectPhotoStyle:(UIGestureRecognizer *)tap{
    ...
    if (_isRecording) {
        [TFJunYou_MyTools showTipView:Localized(@"JX_CannotSwitchDuringRecording")];
        return;
    }
    
    if (_beautyBtn.selected) {
        [TFJunYou_MyTools showTipView:Localized(@"JX_PleaseOpenBeauty")];
        return;
    }
...
//    
//    _beautifyFilter = [TFJunYou_GPUImageBeautifyFilter alloc];
//
...
//    _movieWriter = [[GPUImageMovieWriter alloc] initWithMovieURL:movieURL size:CGSizeMake(720, 720) fileType:AVFileTypeQuickTimeMovie outputSettings:nil];
    ...
    _movieWriter = [[GPUImageMovieWriter alloc] initWithMovieURL:movieURL size:CGSizeMake(((int)TFJunYou__SCREEN_WIDTH/16)*16, ((int)TFJunYou__SCREEN_HEIGHT/16)*16) fileType:AVFileTypeQuickTimeMovie outputSettings:nil];
...
- (void)showBaseView {
    ...
    self.bigView = [[UIView alloc] initWithFrame:CGRectMake(0, TFJunYou__SCREEN_HEIGHT-100-TFJunYou__SCREEN_BOTTOM, TFJunYou__SCREEN_WIDTH, 100+TFJunYou__SCREEN_BOTTOM)];
    ...
    UIView *line = [[UIView alloc] initWithFrame:CGRectMake(0, CGRectGetMaxY(_scrollView.frame)+20, TFJunYou__SCREEN_WIDTH, LINE_WH)];
    line.backgroundColor = THE_LINE_COLOR;
    [self.bigView addSubview:line];
    
    
    //滤镜
    self.filterBtn = [[UIButton alloc] initWithFrame:CGRectMake(0, CGRectGetMaxY(_scrollView.frame)+20, TFJunYou__SCREEN_WIDTH/2, TFJunYou__SCREEN_BOTTOM)];
    ...
    //美颜
    self.skinCareBtn = [[UIButton alloc] initWithFrame:CGRectMake(TFJunYou__SCREEN_WIDTH/2, self.filterBtn.frame.origin.y, TFJunYou__SCREEN_WIDTH/2, self.filterBtn.frame.size.height)];
...
{
    _bottomControlView = [[UIView alloc] initWithFrame:CGRectMake(0, 20, TFJunYou__SCREEN_WIDTH, 60)];
    ...
    UISlider *bilateralSld  = [[UISlider alloc] initWithFrame:CGRectMake(50, 0, TFJunYou__SCREEN_WIDTH-100, 30)
                               ];
    ...
    UISlider *brightnessSld  = [[UISlider alloc] initWithFrame:CGRectMake(50, 40, TFJunYou__SCREEN_WIDTH-100, 30)
                                ];
