//
//  TFJunYou_ScanQRViewController.m
//  TFJunYouChat
//
...
#define TOP (TFJunYou__SCREEN_HEIGHT-300)/2
#define LEFT (TFJunYou__SCREEN_WIDTH-300)/2
#define kScanRect CGRectMake(LEFT, TOP, 300, 300)

@interface TFJunYou_ScanQRViewController ()<AVCaptureMetadataOutputObjectsDelegate,RITLPhotosViewControllerDelegate,GCDAsyncSocketDelegate,TFJunYou_RoomObjectDelegate>
{
    int num;
    BOOL upOrdown;
    NSTimer * timer;
...
@property (nonatomic, strong) TFJunYou_ChatLogMoveActionVC *moveActionVC;
...
@implementation TFJunYou_ScanQRViewController
...
- (void)setupPhotoAlbum {
    ...
    moreBtn.frame = CGRectMake(TFJunYou__SCREEN_WIDTH - 32-15, TFJunYou__SCREEN_TOP - 30, 32, 15);
...
//    CGPathAddRect(path, nil, self.view.bounds);
    ...
    viewRect.origin.y += TFJunYou__SCREEN_TOP;
    viewRect.size.height -= TFJunYou__SCREEN_TOP;
...
- (void)setupCamera
{
    ...
    //设置扫描区域
    CGFloat top = TOP/TFJunYou__SCREEN_HEIGHT;
    CGFloat left = LEFT/TFJunYou__SCREEN_WIDTH;
    CGFloat width = 300/TFJunYou__SCREEN_WIDTH;
    CGFloat height = 300/TFJunYou__SCREEN_HEIGHT;
    ...
    if ([metadataObjects count] >0)
    {
        ...
        if ([action isEqualToString:@"sendChatHistory"]) {
            ...
            if (![userId isEqualToString:g_myself.userId]) {
                ...
                [TFJunYou_MyTools showTipView:Localized(@"JX_PleaseLoginSameAccountScan")];
                return;
            }
            ...
            if (isConnect) {
                ...
                self.moveActionVC = [[TFJunYou_ChatLogMoveActionVC alloc] init];
...
////    [self.output setRectOfInterest:CGRectMake(95/TFJunYou__SCREEN_HEIGHT, 40/TFJunYou__SCREEN_WIDTH, 240/TFJunYou__SCREEN_HEIGHT, 240/TFJunYou__SCREEN_WIDTH)];
//    //使用self.session，初始化预览层，self.session负责驱动input进行信息的采集，layer负责把图像渲染显示
//    self.previewLayer = [[AVCaptureVideoPreviewLayer alloc]initWithSession:self.session];
//    self.previewLayer.frame = CGRectMake(0, 0, TFJunYou__SCREEN_WIDTH , TFJunYou__SCREEN_HEIGHT);
...
//        _chatRoom = [g_xmpp.roomPool joinRoom:[dict objectForKey:@"jid"] title:[dict objectForKey:@"name"] isNew:YES];
    ...
    if ([aDownload.action isEqualToString:act_UserQrCodeLogin]) {
        ...
        if (self.isQRLoginAction) {
            [TFJunYou_MyTools showTipView:Localized(@"JX_SuccessfulLogin")];
        }else {
//            TFJunYou_WebLoginVC *vc = [[TFJunYou_WebLoginVC alloc] init];
//            vc.delegate = self;
//            vc.isQRLogin = YES;
//            [g_navigation pushViewController:vc animated:YES];
//            
