...
@interface TFJunYou_SearchListVC ()<UISearchBarDelegate,UITableViewDelegate,UITableViewDataSource>
...
@property (nonatomic,strong)TFJunYou_UserObject *chatUser;
...
//    [self.cancelBtn setEnabled:YES];
    ...
    if (!_enterChatRecord) {
        self.tableview = [[UITableView alloc] initWithFrame:CGRectMake(TFJunYou__SCREEN_WIDTH, 44, TFJunYou__SCREEN_WIDTH, TFJunYou__SCREEN_HEIGHT-SCREEN_VIEW)];
        self.tableview.tag = 1;
        self.tableview.dataSource = self;
        self.tableview.delegate = self;
        self.tableview.backgroundColor = HEXCOLOR(0xf2f2f2);
        UIView *view = [[UIView alloc] initWithFrame:CGRectMake(0, 0, TFJunYou__SCREEN_WIDTH, 0.1)];
    ...
    if (_isChatRecord) {
        self.recordtableview = [[UITableView alloc] initWithFrame:CGRectMake(TFJunYou__SCREEN_WIDTH, 44, TFJunYou__SCREEN_WIDTH, TFJunYou__SCREEN_HEIGHT-SCREEN_VIEW)];
...
- (void)updataSearchContent{
    ...
    if ([self.titleString isEqualToString:CONTACT]) {
        for (TFJunYou_UserObject *user in self.userArray) {
    ...
    if ([self.titleString isEqualToString:RECORD]) {
        for (TFJunYou_UserObject *user in self.userArray) {
            ...
            for (TFJunYou_MessageObject *msg in array) {
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    if (tableView.tag == 1) {
        if (indexPath.row == 0) {
            ...
            TFJunYou_UserObject *user = self.lastUserArray[indexPath.row-1];
    ...
    if (tableView.tag == 2){
        ...
        if (indexPath.row == 0) {
            TFJunYou_SearchShowCell *cell = [[TFJunYou_SearchShowCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:@"recordNameCell" withNewStyle:TFJunYou_SearchShowCellStyleUser];
            [g_server getHeadImageSmall:userId userName:nickName imageView:cell.headImgView];
            ...
            TFJunYou_SearchShowCell *cell = [tableView dequeueReusableCellWithIdentifier:@"searchRecordCell"];
            if (!cell) {
                cell = [[TFJunYou_SearchShowCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:@"searchRecordCell" withNewStyle:TFJunYou_SearchShowCellStyleRecord];
            }
            NSMutableArray *msgArray = [self.lastMsgDictionary objectForKey:userId];
            TFJunYou_MessageObject *msg = msgArray[indexPath.row-1];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    if (tableView.tag == 1) {
        ...
        if ([self.titleString isEqualToString:RECORD]) {
            ...
            [UIView animateWithDuration:0.3 animations:^{
                self.recordtableview.frame = CGRectMake(0, 44, TFJunYou__SCREEN_WIDTH, TFJunYou__SCREEN_HEIGHT-SCREEN_VIEW);
            }];
            _inChatRecord = YES;
            [self.tableview deselectRowAtIndexPath:indexPath animated:YES];
        }
    ...
    if (tableView.tag == 2) {
        TFJunYou_UserObject *user = self.chatUser;
...
//        [self.cancelBtn setEnabled:YES];
        [UIView animateWithDuration:0.3 animations:^{
            self.recordtableview.frame = CGRectMake(TFJunYou__SCREEN_WIDTH, 44, TFJunYou__SCREEN_WIDTH, TFJunYou__SCREEN_HEIGHT-SCREEN_VIEW);
        }];
    }else{
