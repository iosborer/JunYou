...
@interface TFJunYou_ActionSheetVC ()
...
@implementation TFJunYou_ActionSheetVC
...
- (instancetype)initWithImages:(NSArray *)images names:(NSArray *)names {
    ...
    if (self) {
        ...
        self.maxX = (TFJunYou__SCREEN_WIDTH-(size.width+INSET+IMAGWE_W))/2;
...
- (void)viewDidLoad {
    ...
    if (self.images.count > 0 || self.names.count > 0) {
        ...
        self.baseView.frame = CGRectMake(0, TFJunYou__SCREEN_HEIGHT, TFJunYou__SCREEN_WIDTH, TFJunYou__SCREEN_HEIGHT);
...
- (void)viewDidAppear:(BOOL)animated {
    ...
    [UIView animateWithDuration:.3f animations:^{
        ...
        self.baseView.frame = CGRectMake(0, 0, TFJunYou__SCREEN_WIDTH, TFJunYou__SCREEN_HEIGHT);
    }];
}

- (void)setupViews {
    // 计算取消按钮的高度
    CGFloat cH = THE_DEVICE_HAVE_HEAD ? TFJunYou__SCREEN_BOTTOM : HEIGHT;

    // 创建一个取消按钮
    [self createButtonWithFrame:CGRectMake(0, TFJunYou__SCREEN_HEIGHT-cH, TFJunYou__SCREEN_WIDTH, cH) index:10000];
    for (int i = 0; i < self.names.count; i++) {
        int h = HEIGHT*(i+1);
        // 创建成员按钮
        [self createButtonWithFrame:CGRectMake(0, TFJunYou__SCREEN_HEIGHT-cH - h, TFJunYou__SCREEN_WIDTH,(i == self.names.count-1) ? HEIGHT+12 : HEIGHT) index:i];
    }
}
...
- (void)createButtonWithFrame:(CGRect)frame index:(int)index {
    ...
    if (self.images.count > 0 && index !=10000 && index < self.images.count) {
        ...
        label.frame = CGRectMake(CGRectGetMaxX(imgV.frame)+INSET, (HEIGHT-20)/2, TFJunYou__SCREEN_WIDTH-CGRectGetMaxX(imgV.frame)-INSET, 20);
    }else {
        label.frame = CGRectMake(0, (HEIGHT-20)/2, TFJunYou__SCREEN_WIDTH, 20);
    ...
    UIView *line = [[UIView alloc] initWithFrame:CGRectMake(33, button.frame.size.height-LINE_WH, TFJunYou__SCREEN_WIDTH-66, LINE_WH)];
