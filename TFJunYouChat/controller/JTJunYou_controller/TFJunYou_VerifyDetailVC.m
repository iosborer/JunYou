...
#define HEIGHT 50
@interface TFJunYou_VerifyDetailVC ()
@property (nonatomic, strong) UIButton *confirmBtn;
@property (nonatomic, strong) NSArray *userIds;
@property (nonatomic, strong) NSArray *userNames;
@end

@implementation TFJunYou_VerifyDetailVC
...
- (void)customView {
    ...
    TFJunYou_ImageView *imageView = [[TFJunYou_ImageView alloc] initWithFrame:CGRectMake(0, 50, 80, 80)];
    imageView.center = CGPointMake(TFJunYou__SCREEN_WIDTH / 2, imageView.center.y);
    ...
    UILabel *name = [[UILabel alloc] initWithFrame:CGRectMake(0, CGRectGetMaxY(imageView.frame) + 10, TFJunYou__SCREEN_WIDTH, 20)];
    ...
    UILabel *tip = [[UILabel alloc] initWithFrame:CGRectMake(0, CGRectGetMaxY(name.frame) + 10, TFJunYou__SCREEN_WIDTH, 20)];
    ...
    if ([self.msg.fromUserId isEqualToString:[array firstObject]] && array.count == 1) {
        tip.text = [NSString stringWithFormat:@"%@申请加入群组",self.msg.fromUserName];
    }
    
    UILabel *detail = [[UILabel alloc] initWithFrame:CGRectMake(0, CGRectGetMaxY(tip.frame) + 10, TFJunYou__SCREEN_WIDTH, 20)];
    ...
    UIView *line = [[UIView alloc] initWithFrame:CGRectMake(20, CGRectGetMaxY(detail.frame) + 20, TFJunYou__SCREEN_WIDTH - 40, 1)];
    line.backgroundColor = [UIColor grayColor];
    [self.tableBody addSubview:line];
    
    UIView *view = [self createImages];
    view.frame = CGRectMake(0, CGRectGetMaxY(line.frame) + 20, TFJunYou__SCREEN_WIDTH, view.frame.size.height);
    
    _confirmBtn = [[UIButton alloc] initWithFrame:CGRectMake(0, CGRectGetMaxY(view.frame) + 20, WIDTH, 50)];
    _confirmBtn.center = CGPointMake(TFJunYou__SCREEN_WIDTH / 2, _confirmBtn.center.y);
...
- (UIView *) createImages {
    ...
    UIView *contentView = [[UIView alloc] initWithFrame:CGRectMake(0, 0, TFJunYou__SCREEN_WIDTH, 0)];
...
//    float widthInset = (screenWidth%52 +52)/(screenWidth/52.0);
    ...
    for (NSInteger i = 0; i < self.userIds.count; i ++) {
        ...
        TFJunYou_ImageView* headImageView = [[TFJunYou_ImageView alloc] init];
        ...
        if(headImageView){
            ...
            TFJunYou_Label* b = [[TFJunYou_Label alloc]initWithFrame:CGRectMake( headImageView.frame.origin.x, headImageView.frame.origin.y+headImageView.frame.size.height + 5, 52, 10)];
            b.text = userName;
...
//    [g_server addRoomMember:self.room.roomId userArray:self.userIds toView:self];//用接口即可
    [g_server addRoomMemberV1:self.room.roomId userArray:self.userIds fromUserId:self.msg.fromUserId toView:self];
}

-(void) didServerResultSucces:(TFJunYou_Connection*)aDownload dict:(NSDictionary*)dict array:(NSArray*)array1{
    [_wait stop];
    if( [aDownload.action isEqualToString:act_roomMemberSetV1] ){
        ...
        for(NSInteger i=[self.chatVC.array count]-1;i>=0;i--){
            TFJunYou_MessageObject *p=[self.chatVC.array objectAtIndex:i];
        ...
        [g_navigation popToViewController:[TFJunYou_ChatViewController class] animated:YES];
    }
