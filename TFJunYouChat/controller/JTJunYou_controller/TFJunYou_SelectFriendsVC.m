...
@interface TFJunYou_SelectFriendsVC ()<UITextFieldDelegate, UIAlertViewDelegate,TFJunYou_RoomObjectDelegate>
...
@implementation TFJunYou_SelectFriendsVC
...
- (void)viewDidLoad
{
    ...
    if (_type == TFJunYou_SelectFriendTypeGroupAT ||_type == TFJunYou_SelectFriendTypeSpecifyAdmin) {
...
-(void)getDataArrayByType{
    ...
    if (_type == TFJunYou_SelectFriendTypeGroupAT || _type == TFJunYou_SelectFriendTypeSelMembers) {
        if(_type == TFJunYou_SelectFriendTypeSelMembers){
        ...
        [_table reloadData];
    }else if(_type == TFJunYou_SelectFriendTypeSpecifyAdmin){
        self.title = Localized(@"JXRoomMemberVC_SetAdministrator");
        [self getRoomMembersArray];
        [_table reloadData];
    }else if (_type == TFJunYou_SelectFriendTypeCustomArray) {
        //        self.title
...
- (void) textFieldDidChange:(UITextField *)textField {
    ...
    if (_type == TFJunYou_SelectFriendTypeGroupAT || _type == TFJunYou_SelectFriendTypeSelMembers || _type == TFJunYou_SelectFriendTypeSpecifyAdmin) {
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (_type == TFJunYou_SelectFriendTypeGroupAT || _type == TFJunYou_SelectFriendTypeSpecifyAdmin ||  _type == TFJunYou_SelectFriendTypeSelMembers) {
        ...
        if (_type == TFJunYou_SelectFriendTypeGroupAT) {
            if(member.idStr){
                if (room.roomId != nil) {
...
//                    }
    ...
    }else if (_type == TFJunYou_SelectFriendTypeCustomArray || _type == TFJunYou_SelectFriendTypeDisAble) {
...
//                    btn.userInteractionEnabled = !b;
                    ...
                    if (b) {
                        if (_type == TFJunYou_SelectFriendTypeSelFriends) {
                            btn.selected = b;
                        }else {
                            btn.enabled = !b;
                        }
...
//        }
    ...
    cell.lbTitle.frame = CGRectMake(CGRectGetMaxX(cell.headImageView.frame)+15, 21.5, TFJunYou__SCREEN_WIDTH - 115 -CGRectGetMaxX(cell.headImageView.frame)-14, 18);
...
//    if (self.addressBookArr.count > 0) {
    ...
    if (self.type == 0 && checked && self.maxSize > 0 &&  _userIds.count  == self.maxSize) {
        ...
        [TFJunYou_MyTools showTipView:@"群成员已达上限"];
        return;
    }
        ...
        if ([user isKindOfClass:[TFJunYou_UserObject class]]) {
            TFJunYou_UserObject *userO = (TFJunYou_UserObject *)user;
            if (self.room.isSecretGroup) {
                if (!userO.publicKeyRSARoom || userO.publicKeyRSARoom.length <= 0) {
                    checkbox.selected = NO;
                    [TFJunYou_MyTools showTipView:@"此用户不能进入私密群组"];
                    return;
                }
            }
...
-(void)onAdd:(UIButton *)btn{
    ...
    if(_type == TFJunYou_SelectFriendTypeSelFriends || chatRoom || self.isForRoom){
...
//            chatRoom.delegate = self;
        ...
        if ((self.room.isNeedVerify && self.room.userId != [g_myself.userId longLongValue]) || _type == TFJunYou_SelectFriendTypeSelFriends) {
    ...
    if (_type == TFJunYou_SelectFriendTypeGroupAT)
        return;
    if (_type == TFJunYou_SelectFriendTypeSpecifyAdmin)
        return;
    if (_type == TFJunYou_SelectFriendTypeSelMembers){
    }
    if(self.delegate != nil && [self.delegate respondsToSelector:self.didSelect])
...
-(void) didServerResultSucces:(TFJunYou_Connection*)aDownload dict:(NSDictionary*)dict array:(NSArray*)array1{
    ...
    if( [aDownload.action isEqualToString:act_roomAdd] ){
        ...
        chatRoom = [[TFJunYou_XMPP sharedInstance].roomPool createRoom:room.roomJid title:room.name];
        chatRoom.delegate = self;
        [self insertRoom];
...
//        [g_server addRoomMember:room.roomId userArray:_userIds toView:self];//用接口即可
        ...
        if (_userIds.count > maxUser) {
            [TFJunYou_MyTools showTipView:@"群成员超过上限"];
            [_userIds removeAllObjects];
            [_userNames removeAllObjects];
            [self actionQuit];
        }else {
