...
@interface TFJunYou_SetLabelVC ()<UITextFieldDelegate,UITextViewDelegate,UIScrollViewDelegate>
...
@implementation TFJunYou_SetLabelVC

- (void)viewDidLoad {
    [super viewDidLoad];
    // Do any additional setup after loading the view.
    if (_array.count <= 0) {
        _array = [[TFJunYou_LabelObject sharedInstance] fetchLabelsWithUserId:self.user.userId];
    }
    if (_allArray.count <= 0) {
        _allArray = [[TFJunYou_LabelObject sharedInstance] fetchAllLabelsFromLocal];
    }
...
- (void)customView {
    TFJunYou_Label *p = [self createLabel:self.tableHeader default:Localized(@"JX_Confirm") selector:@selector(onSave)];
    ...
    p.frame = CGRectMake(TFJunYou__SCREEN_WIDTH - 51 - 15, TFJunYou__SCREEN_TOP - 8 - 29, 51, 29);

    _scrollView = [[UIScrollView alloc] initWithFrame:CGRectMake(0, 0, TFJunYou__SCREEN_WIDTH, 50)];
    _scrollView.backgroundColor = [UIColor whiteColor];
    [self.tableBody addSubview:_scrollView];
    
    _allScrollView = [[UIScrollView alloc] initWithFrame:CGRectMake(0, CGRectGetMaxY(_scrollView.frame), TFJunYou__SCREEN_WIDTH, self.tableBody.frame.size.height - CGRectGetMaxY(_scrollView.frame))];
...
-(TFJunYou_Label*)createLabel:(UIView*)parent default:(NSString*)s selector:(SEL)selector{
    TFJunYou_Label* p = [[TFJunYou_Label alloc] initWithFrame:CGRectMake(TFJunYou__SCREEN_WIDTH/2,INSETS,TFJunYou__SCREEN_WIDTH/2 -20,44-INSETS*2)];
...
- (void)createLabels {
   ...
   [_scrollView.subviews makeObjectsPerformSelector:@selector(removeFromSuperview)];
    ...
    for (NSInteger i = 0; i < _array.count; i ++) {
        TFJunYou_LabelObject *labelObj = _array[i];
        ...
        if (i != _array.count - 1) {
            TFJunYou_LabelObject *lastLabelObj = _array[i + 1];
            ...
            if ((x + lastSize.width + margin) > TFJunYou__SCREEN_WIDTH) {
                x = margin;
    ...
    if (CGRectGetMaxX(lastLabelBtn.frame) + margin + 50 + margin > TFJunYou__SCREEN_WIDTH) {
        _textField.frame = CGRectMake(margin, CGRectGetMaxY(lastLabelBtn.frame) + margin, TFJunYou__SCREEN_WIDTH - margin - margin, 36);
    }else {
        ...
        _textField.frame = CGRectMake(CGRectGetMaxX(lastLabelBtn.frame) + margin, y, TFJunYou__SCREEN_WIDTH - CGRectGetMaxX(lastLabelBtn.frame) - margin - margin, 36);
    }
    ...
    if ((CGRectGetMaxY(_textField.frame) + margin) > (margin * 4 + 36 * 3)) {
        ...
        _scrollView.contentSize = CGSizeMake(TFJunYou__SCREEN_WIDTH, CGRectGetMaxY(_textField.frame) + margin);
    }else {
        ...
        _scrollView.contentSize = CGSizeMake(TFJunYou__SCREEN_WIDTH, _scrollView.frame.size.height);
    }
...
- (void) createAllLabels {
    ...
    UILabel *allLabel = [[UILabel alloc] initWithFrame:CGRectMake(15, 10, TFJunYou__SCREEN_WIDTH, 15)];
    ...
    for (NSInteger i = 0; i < _allArray.count; i ++) {
        TFJunYou_LabelObject *labelObj = _allArray[i];
        
        BOOL flag = NO;
        for (TFJunYou_LabelObject *obj in _array) {
        ...
        if (i != _allArray.count - 1) {
            TFJunYou_LabelObject *lastLabelObj = _allArray[i + 1];
            CGSize lastSize = [lastLabelObj.groupName boundingRectWithSize:CGSizeMake(MAXFLOAT, MAXFLOAT) options:NSStringDrawingUsesLineFragmentOrigin attributes:@{NSFontAttributeName : [UIFont systemFontOfSize:16.0]} context:nil].size;
            lastSize = CGSizeMake(lastSize.width + 20, lastSize.height + 10);
            if ((x + lastSize.width + margin) > TFJunYou__SCREEN_WIDTH) {
                x = margin;
...
- (void)allLabelAction:(UIButton *)labelBtn {
    ...
    TFJunYou_LabelObject *allObj = _allArray[labelBtn.tag];
    TFJunYou_LabelObject *obj = nil;
    // 查找已选标签中是否有此标签
    for (TFJunYou_LabelObject *labelObj in _array) {
...
- (void)onSave {
    ...
    if (!flag) {
        NSMutableString *userIdListStr = [NSMutableString string];
        for (NSInteger i = 0; i < _array.count; i ++) {
            TFJunYou_LabelObject *obj = _array[i];
            if (i == 0) {
...
-(void) didServerResultSucces:(TFJunYou_Connection*)aDownload dict:(NSDictionary*)dict array:(NSArray*)array1{
    ...
    if ([aDownload.action isEqualToString:act_FriendGroupUpdateFriend]) {
        ...
        // 更新数据库
        for (TFJunYou_LabelObject *labelObj in _allArray) {
            [labelObj update];
        }
