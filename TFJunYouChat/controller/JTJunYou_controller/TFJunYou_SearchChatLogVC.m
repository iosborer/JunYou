...
@interface TFJunYou_SearchChatLogVC () <UITextFieldDelegate>
...
@implementation TFJunYou_SearchChatLogVC
...
- (void)createSelectView {
    ...
    if (!_selectView) {
        ...
        CGFloat itemW = 100;
        CGFloat itemH = 60;
        CGFloat marginX = (TFJunYou__SCREEN_WIDTH - (itemW * 3)) / 2;
...
- (void)itemAction:(UIButton *)btn {
    ...
    switch (btn.tag) {
        case 0:{
            TFJunYou_SearchImageLogVC *vc = [[TFJunYou_SearchImageLogVC alloc] init];
            vc.isImage = YES;
            vc.user = self.user;
    ...
    TFJunYou_SearchFileLogVC *vc = [[TFJunYou_SearchFileLogVC alloc] init];
    vc.type = type;
    vc.user = self.user;
    vc.isGroup = YES;
    [g_navigation pushViewController:vc animated:YES];
...
- (void) textFieldDidChange:(UITextField *)textField {
    ...
    NSArray * resultArray = [[TFJunYou_MessageObject sharedInstance] fetchSearchMessageWithUserId:self.user.userId String:textField.text];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    TFJunYou_MsgAndUserObject * dict = (TFJunYou_MsgAndUserObject*) [_array objectAtIndex:indexPath.row];
