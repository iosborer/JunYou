...
- (void)viewDidLoad {
    ...
    UIImageView *qrImageView = [[UIImageView alloc] initWithFrame:CGRectMake((TFJunYou__SCREEN_WIDTH-200)/2, 100, 200, 200)];
    qrImageView.image = qrImage;
    [self.tableBody addSubview:qrImageView];
    
    UILabel *label = [[UILabel alloc] initWithFrame:CGRectMake(0, CGRectGetMaxY(qrImageView.frame) + 20, TFJunYou__SCREEN_WIDTH, 50)];
...
- (void)socket:(GCDAsyncSocket *)sock didAcceptNewSocket:(GCDAsyncSocket *)newSocket {
    ...
    for (NSInteger i = 0; i < _selUserIdArray.count; i ++) {
        ...
        NSMutableArray *msgArr = [[TFJunYou_MessageObject sharedInstance] fetchAllMessageListWithUser:userId];
...
//            }
        ...
        for (NSInteger j = 0; j < msgArr.count; j ++) {
            TFJunYou_MessageObject *msg = msgArr[j];
