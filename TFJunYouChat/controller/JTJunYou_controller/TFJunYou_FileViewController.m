...
#import "TFJunYou_ShareFileTableViewCell.h"
...
@interface TFJunYou_FileViewController ()<UINavigationControllerDelegate,UIImagePickerControllerDelegate,QBImagePickerControllerDelegate>
...
@implementation TFJunYou_FileViewController
...
- (void)viewDidLoad {
    ...
    [_table registerClass:[TFJunYou_ShareFileTableViewCell class] forCellReuseIdentifier:NSStringFromClass([TFJunYou_ShareFileTableViewCell class])];
    [self getServerData];
...
-(void)customView{
    if (!_addFileButton){
        ...
        _addFileButton.frame = CGRectMake(TFJunYou__SCREEN_WIDTH-15-18, TFJunYou__SCREEN_TOP-15-18, 18, 18);
...
- (UITableViewCell *)tableView:(UITableView *)_tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    TFJunYou_ShareFileTableViewCell * cell = [_tableView dequeueReusableCellWithIdentifier:NSStringFromClass([TFJunYou_ShareFileTableViewCell class]) forIndexPath:indexPath];
    
    TFJunYou_ShareFileObject * fileObject = _dataArray[indexPath.row];
...
- (UITableViewCellEditingStyle)tableView:(UITableView *)tableView editingStyleForRowAtIndexPath:(NSIndexPath *)indexPath {
    TFJunYou_ShareFileObject *obj = _dataArray[indexPath.row];
    memberData * myMem = nil;
    for (memberData * member in _room.members) {
        if ([[NSString stringWithFormat:@"%ld",member.userId] isEqualToString:g_myself.userId]) {
            myMem = member;
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    TFJunYou_ShareFileObject * shareFile = _dataArray[indexPath.row];
    TFJunYou_FileDetailViewController * detailVC = [[TFJunYou_FileDetailViewController alloc] init];
...
-(void) didServerResultSucces:(TFJunYou_Connection*)aDownload dict:(NSDictionary*)dict array:(NSArray*)array1{
    ...
    if([aDownload.action isEqualToString:act_shareList]){
        ...
        for (NSDictionary * dict in array1) {
            TFJunYou_ShareFileObject * shareFile = [TFJunYou_ShareFileObject shareFileWithDict:dict];
...
//            NSURL * videoUrl = info[@"UIImagePickerControllerReferenceURL"];
    ...
    //    int imageHeight = chosedImage.size.height;
        [self dismissViewControllerAnimated:NO completion:^{
            NSString* file = [TFJunYou_FileInfo getUUIDFileName:@"jpg"];
...
-(void)addNewShareFile{
    ...
    if ([data.role integerValue] == 4) {
        [TFJunYou_MyTools showTipView:@"隐身人不能上传共享文件"];
        return;
    }
