...
//  TFJunYouChat
//
//  Created by lifengye on 2020/09/09.
//  Copyright © 2020 zengwOS. All rights reserved.
//

#import "TFJunYou_LocMapVC.h"
#import "TFJunYou_LocPerImageVC.h"
//#import "TFJunYou_NewResumeInfoVC.h"
...
@interface TFJunYou_LocMapVC ()<BMKMapViewDelegate,BMKLocationServiceDelegate>{
...
@implementation TFJunYou_LocMapVC
...
//    [_resetLoca release];
    ...
    //上方的滚动条
//    UIView * topBackground = [[UIView alloc]initWithFrame:CGRectMake(0, 0, TFJunYou__SCREEN_WIDTH, 40)];
//    topBackground.backgroundColor = [UIColor whiteColor];
//    _topScrolView = [[UIScrollView alloc]initWithFrame:CGRectMake(0, 0, TFJunYou__SCREEN_WIDTH, 40)];
...
//    [_topScrolView addSubview:_topMinView];
    //职位详情ScrollView
    _scrollBackground = [[UIView alloc]initWithFrame:CGRectMake(0, self.view.frame.size.height + 100, TFJunYou__SCREEN_WIDTH, 100)];
    _resumeScrolView = [[UIScrollView alloc]initWithFrame:CGRectMake(0, 0, TFJunYou__SCREEN_WIDTH, 100)];
    ...
    _resumeScrolView.contentSize = CGSizeMake(TFJunYou__SCREEN_WIDTH *3, 100);
...
//    [_resumeScrolView release];
    ...
    //创建ScrollView里面三个视图
    _jobDetailOne = [[UIView alloc]initWithFrame:CGRectMake(0, 0, TFJunYou__SCREEN_WIDTH, 100)];
    _jobDetailOne.tag = 1;
    [self creatJobDetailView:_jobDetailOne];
    [_resumeScrolView addSubview:_jobDetailOne];//创建视图里的所有控件
//    [_jobDetailOne release];
    _jobDetailTwo = [[UIView alloc]initWithFrame:CGRectMake(TFJunYou__SCREEN_WIDTH , 0, TFJunYou__SCREEN_WIDTH, 100)];
    _jobDetailTwo.tag = 2;
    [self creatJobDetailView:_jobDetailTwo];//创建视图里的所有控件
    [_resumeScrolView addSubview:_jobDetailTwo];
//    [_jobDetailTwo release];
    _jobDetailThree = [[UIView alloc]initWithFrame:CGRectMake(TFJunYou__SCREEN_WIDTH*2, 0, TFJunYou__SCREEN_WIDTH, 100)];
...
//    [_jobDetailThree release];
    ...
    //放大缩小
    _scaleView = [[UIView alloc]initWithFrame:CGRectMake(TFJunYou__SCREEN_WIDTH - 100, self.view.frame.size.height - 40, 90, 30)];
...
//    [enlargeButton release];
    ...
    //当前位置图标
//    UIImageView * pointImage = [[UIImageView alloc]initWithFrame:CGRectMake((TFJunYou__SCREEN_WIDTH - 20) /2, (TFJunYou__SCREEN_HEIGHT - 25 +64)/2, 20, 25)];
...
-(void)creatJobDetailView:(UIView*)jobView{
    //白色背景
    UIButton * detailButton = [[UIButton alloc]initWithFrame:CGRectMake(20, 0, TFJunYou__SCREEN_WIDTH-40, 90)];
...
//    [detailButton release];
    //灰色背景
//    UIView * grayView = [[UIView alloc]initWithFrame:CGRectMake(20, 50, TFJunYou__SCREEN_WIDTH - 40, 40)];
...
//    [grayView release];
    if (jobView.tag == 1) {
        //标题
        _titleLabelOne = [[UILabel alloc]initWithFrame:CGRectMake(30, 5, TFJunYou__SCREEN_WIDTH - 160, 20)];
        [jobView addSubview:_titleLabelOne];
//        [_titleLabelOne release];
        //付费方式
        _payLabelOne = [[UILabel alloc]initWithFrame:CGRectMake(TFJunYou__SCREEN_WIDTH - 180 + 30, 5, 110, 20)];
...
//        [_payLabelOne release];
        //详情
        _detailLabelOne = [[UILabel alloc]initWithFrame:CGRectMake(30, 28, TFJunYou__SCREEN_WIDTH - 100, 15)];
...
//        _identImageOne.image = [UIImage imageNamed:[TFJunYou_MyTools verImgName:@"ic_website"]];
...
//        [_integralLabelOne release];
        //距离
        _distanceLabelOne = [[UILabel alloc]initWithFrame:CGRectMake(TFJunYou__SCREEN_WIDTH - 180 + 30, 60, 110, 20)];
...
//        [_distanceLabelOne release];
    ...
    if (jobView.tag == 2) {
        //标题
        _titleLabelTwo = [[UILabel alloc]initWithFrame:CGRectMake(30, 5, TFJunYou__SCREEN_WIDTH - 160, 20)];
        [jobView addSubview:_titleLabelTwo];
//        [_titleLabelTwo release];
        //付费方式
        _payLabelTwo = [[UILabel alloc]initWithFrame:CGRectMake(TFJunYou__SCREEN_WIDTH - 180 + 30, 5, 110, 20)];
...
//        [_payLabelTwo release];
        //详情
        _detailLabelTwo = [[UILabel alloc]initWithFrame:CGRectMake(30, 28, TFJunYou__SCREEN_WIDTH - 100, 15)];
...
//        _identImageTwo.image = [UIImage imageNamed:[TFJunYou_MyTools verImgName:@"ic_website"]];
...
//        [_integralLabelTwo release];
        //距离
        _distanceLabelTwo = [[UILabel alloc]initWithFrame:CGRectMake(TFJunYou__SCREEN_WIDTH - 180 + 30, 60, 110, 20)];
...
//        [_distanceLabelTwo release];
    ...
    if (jobView.tag == 3) {
        //标题
        _titleLabelThree = [[UILabel alloc]initWithFrame:CGRectMake(30, 5, TFJunYou__SCREEN_WIDTH - 160, 20)];
        [jobView addSubview:_titleLabelThree];
//        [_titleLabelThree release];
        //付费方式
        _payLabelThree = [[UILabel alloc]initWithFrame:CGRectMake(TFJunYou__SCREEN_WIDTH - 180 + 30, 5, 110, 20)];
...
//        [_payLabelThree release];
        //详情
        _detailLabelThree = [[UILabel alloc]initWithFrame:CGRectMake(30, 28, TFJunYou__SCREEN_WIDTH - 100, 15)];
...
//        _identImageThree.image = [UIImage imageNamed:[TFJunYou_MyTools verImgName:@"ic_website"]];
...
//        [_integralLabelThree release];
        //距离
        _distanceLabelThree = [[UILabel alloc]initWithFrame:CGRectMake(TFJunYou__SCREEN_WIDTH - 180 + 30, 60, 110, 20)];
...
- (void)setData:(int)centerIndex{
    if (centerIndex == 0) {
        ...
        [_resumeScrolView setContentOffset:CGPointMake(TFJunYou__SCREEN_WIDTH, 0)];
        [self setOneData:centerIndex -1];
        [self setThreeData:centerIndex +1];
        
    }else{
        [self setThreeData:centerIndex];
        [_resumeScrolView setContentOffset:CGPointMake(TFJunYou__SCREEN_WIDTH *2, 0)];
        [self setTwoData:centerIndex -1];
    }
}
#pragma mark ------------------填充数据------------------
...
- (void)initBaiduMapView{
    _imageMapView = [[BMKMapView alloc]initWithFrame:CGRectMake(0, 0, TFJunYou__SCREEN_WIDTH, self.view.frame.size.height)];
...
- (void)scrollViewDidEndDecelerating:(UIScrollView *)scrollView{
    if (scrollView.tag == 12) {
        ...
        for ( int i = 0; i < [_allAnnotationView count]; i++) {
            TFJunYou_LocPerImageVC * annoView = [_allAnnotationView objectAtIndex:i];
...

{
    ...
    TFJunYou_LocPerImageVC *annotationView = [[TFJunYou_LocPerImageVC alloc] initWithAnnotation:annotation reuseIdentifier:AnnotationViewID];
...
-(void)mapView:(BMKMapView *)mapView didSelectAnnotationView:(BMKAnnotationView *)view{
    ...
    if (_resumeScrolView.hidden) {
        ...
        [UIView animateWithDuration:0.3 animations:^{
            ...
            _scrollBackground.frame = CGRectMake(0, self.view.frame.size.height - 100, TFJunYou__SCREEN_WIDTH, 100);
            _scaleView.frame = CGRectMake(TFJunYou__SCREEN_WIDTH -100, self.view.frame.size.height -145, 90, 30);
        }];
    }
...
-(void)dissmissScrollView{
    ...
    _scaleView.frame = CGRectMake(TFJunYou__SCREEN_WIDTH - 100,self.view.frame.size.height -40, 90, 30);
}
...
-(void) didServerResultSucces:(TFJunYou_Connection*)aDownload dict:(NSDictionary*)dict array:(NSArray*)array1{
    ...
    if([aDownload.action isEqualToString:act_nearbyUser]){
        ...
        if ([_nearbyData count] == 1) {
            _resumeScrolView.contentSize = CGSizeMake(TFJunYou__SCREEN_WIDTH, 100);
        }else if ([_nearbyData count] == 2){
            _resumeScrolView.contentSize = CGSizeMake(TFJunYou__SCREEN_WIDTH*2, 100);
        }else {
            _resumeScrolView.contentSize = CGSizeMake(TFJunYou__SCREEN_WIDTH*3, 100);
        }
        if (_resumeScrolView.hidden == NO) {
