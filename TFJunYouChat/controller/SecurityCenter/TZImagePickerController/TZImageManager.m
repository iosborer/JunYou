...
- (void)savePhotoWithImage:(UIImage *)image completion:(void (^)(NSError *error))completion {
    ...
    if (iOS9Later) { // 这里有坑... iOS8系统下这个方法保存图片会失败 原来是因为PHAssetResourceType是iOS9之后的...
        [[PHPhotoLibrary sharedPhotoLibrary] performChanges:^{
            ...
            dispatch_sync(dispatch_get_main_queue(), ^{
                if (success && completion) {
                    ...
                    // JLog(@"保存照片出错:%@",error.localizedDescription);
                    if (completion) {
                        completion(error);
                    }
                }
        ...
        [self.assetLibrary writeImageToSavedPhotosAlbum:image.CGImage orientation:[self orientationFromImage:image] completionBlock:^(NSURL *assetURL, NSError *error) {
            if (error) {
                // JLog(@"保存图片失败:%@",error.localizedDescription);
                if (completion) {
                    completion(error);
                }
            } else {
...
- (void)startExportVideoWithVideoAsset:(AVURLAsset *)videoAsset completion:(void (^)(NSString *outputPath))completion {
    ...
    if ([presets containsObject:AVAssetExportPreset640x480]) {
        ...
        // JLog(@"video outputPath = %@",outputPath);
        ...
        if ([supportedTypeArray containsObject:AVFileTypeMPEG4]) {
            ...
            // JLog(@"No supported file types 视频类型暂不支持导出");
            return;
        } else {
        ...
        [session exportAsynchronouslyWithCompletionHandler:^(void) {
            switch (session.status) {
                case AVAssetExportSessionStatusUnknown:
                    // JLog(@"AVAssetExportSessionStatusUnknown"); break;
                case AVAssetExportSessionStatusWaiting:
                    // JLog(@"AVAssetExportSessionStatusWaiting"); break;
                case AVAssetExportSessionStatusExporting:
                    // JLog(@"AVAssetExportSessionStatusExporting"); break;
                case AVAssetExportSessionStatusCompleted: {
                    // JLog(@"AVAssetExportSessionStatusCompleted");
                    dispatch_async(dispatch_get_main_queue(), ^{
                        if (completion) {
                ...
                case AVAssetExportSessionStatusFailed:
                    // JLog(@"AVAssetExportSessionStatusFailed"); break;
                default: break;
            }
        }];
    }
