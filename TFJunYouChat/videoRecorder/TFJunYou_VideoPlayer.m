...
@implementation TFJunYou_VideoPlayer
...
//        self.parent = value;
        self.isPlaying = NO;
        // 添加上下两个地方的透明模板
        _topView = [[UIView alloc] initWithFrame:CGRectMake(0, 0, TFJunYou__SCREEN_WIDTH, THE_DEVICE_HAVE_HEAD ? 62 : 42)];
        _botView = [[UIView alloc] initWithFrame:CGRectMake(0, TFJunYou__SCREEN_HEIGHT-90, TFJunYou__SCREEN_WIDTH, 90)];
...
//        [_parent bringSubviewToFront:_movieTimeControl];
        ...
        CGRect frame = THE_DEVICE_HAVE_HEAD ? CGRectMake(0, TFJunYou__SCREEN_TOP, TFJunYou__SCREEN_WIDTH, TFJunYou__SCREEN_HEIGHT-TFJunYou__SCREEN_TOP-TFJunYou__SCREEN_BOTTOM) : _firstBaseView.bounds;
//        CGRect frame = _firstBaseView.bounds;
        _videoFirst = [[UIImageView alloc] initWithFrame:frame];
        [_firstBaseView addSubview:_videoFirst];
        [TFJunYou_FileInfo getFullFirstImageFromVideo:_videoFile imageView:_videoFirst];
        
        _wait = [[TFJunYou_WaitView alloc] initWithParent:_firstBaseView];
        [_wait start];
...
- (void)firstViewAction:(UITapGestureRecognizer *)tap {
    if (self.type != TFJunYou_VideoTypePreview) {  // 如果不是预览界面
...
- (void)longPress:(UILongPressGestureRecognizer *)gestureRecognizer {
    ...
    TFJunYou_ActionSheetVC *actionVC = [[TFJunYou_ActionSheetVC alloc] initWithImages:@[] names:@[Localized(@"JX_SaveVideo")]];
...
- (void)dealloc {
    NSLog(@"TFJunYou_VideoPlayer.dealloc");
    self.parent = nil;
    self.videoFile = nil;
//    [_pauseBtn release];
...
//显示播放器
- (void)start{
    _player = [[TFJunYou_VideoPlayerVC alloc] init];
    ...
    if (_isPreview) {  // 预览界面处理
        ...
        _timeLab.hidden = YES;
        _exitBtn.hidden = YES;
        _disBtn = [self createButtonWithFrame:CGRectMake(20, TFJunYou__SCREEN_HEIGHT-20-80, 80, 80) image:@"video_return" action:@selector(exitVideoPlayer)];
        _sendBtn = [self createButtonWithFrame:CGRectMake(TFJunYou__SCREEN_WIDTH-20-80, TFJunYou__SCREEN_HEIGHT-20-80, 80, 80) image:@"video_gou" action:@selector(didButtonWithSendVideo)];
...
- (void)onClickVideo:(id)sender{
    if (self.type == TFJunYou_VideoTypePreview) {
        return;
    }
...
- (void)dismissVideoPlayer {
    if (self.type == TFJunYou_VideoTypeChat) {
        _topView.hidden = NO;
        _botView.hidden = NO;
    }
...
-(void)doPlayEnd{
    if (_isPreview || self.type == TFJunYou_VideoTypeWeibo) {
        [self switch];
        return;
    }
    self.isPlaying = NO;
...
- (void)setupView:(UIView *)view colors:(NSArray *)colors {
    ...
    gradientLayer.frame = CGRectMake(0, 0, TFJunYou__SCREEN_WIDTH, 100);  // 设置显示的frame
