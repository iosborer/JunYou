...
@implementation TFJunYou_ConvertMedia
...
- (id)init {
	...
	if (self) {
        ...
        videoHeight = TFJunYou__SCREEN_WIDTH;
        videoWidth  = 480;
        videoFrames = 15;
        [self initRotateSize];
	}
...
- (void)dealloc {
//    NSLog(@"TFJunYou_ConvertMedia.dealloc");
...
-(void)saveToImage:(CMSampleBufferRef)sampleBuffer newImage:(CGImageRef)newImage{
    ...
    if(m % saveVideoToImage == 0){
        ...
        if(![s isEqualToString:_lastSaveFile]){
            ...
            image = [image imageAtRect:CGRectMake(80, 0, TFJunYou__SCREEN_WIDTH, TFJunYou__SCREEN_WIDTH)];
