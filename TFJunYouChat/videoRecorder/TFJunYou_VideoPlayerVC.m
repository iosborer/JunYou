...
@interface TFJunYou_VideoPlayerVC ()

@end

@implementation TFJunYou_VideoPlayerVC
...
-(void)initScrubberTimer
{
    ...
    __weak TFJunYou_VideoPlayerVC *weakSelf = self;
...
- (void)syncPlayPauseButtons
{
    if ([self isPlaying])
    {
        NSLog(@"TFJunYou_VideoPlayerVC.播放");
    }
    else
    {
        NSLog(@"TFJunYou_VideoPlayerVC.暂停");
...
- (void)observeValueForKeyPath:(NSString *)keyPath ofObject:(id)object change:(NSDictionary *)change context:(void *)context
{
    ...
    else if (context == StreamPlayerViewControllerCurrentItemObservationContext)
    {
        [[NSOperationQueue mainQueue] addOperationWithBlock:^{//只留下主线程返回的进度数据
            ...
            [self->playerLayerView.playerLayer setPlayer:self->_player];
...
//        [kTimedMetadataKey retain];
        ...
        _wait = [[TFJunYou_WaitView alloc] initWithParent:playerLayerView];
        ...
        //进度条
//        movieTimeControl = [[UISlider alloc] initWithFrame:CGRectMake(100, TFJunYou__SCREEN_HEIGHT-50, TFJunYou__SCREEN_WIDTH-160, 10)];
...
- (void)dealloc
{
    NSLog(@"TFJunYou_VideoPlayerVC.dealloc");
...
-(void)setPlayStatus:(TFJunYou_Label *)sender{
