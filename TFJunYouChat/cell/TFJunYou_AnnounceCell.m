...
@interface TFJunYou_AnnounceCell ()
@property (nonatomic, strong) UIView *baseView;
@property (nonatomic, strong) UIView *line;

@end

@implementation TFJunYou_AnnounceCell

- (instancetype)initWithStyle:(UITableViewCellStyle)style reuseIdentifier:(NSString *)reuseIdentifier {
    self = [super initWithStyle:style reuseIdentifier:reuseIdentifier];
    if (self) {
        self.contentView.backgroundColor = HEXCOLOR(0xF2F2F2);
        self.baseView = [[UIView alloc] initWithFrame:CGRectMake(INSETS, INSETS, TFJunYou__SCREEN_WIDTH-INSETS*2, MAXFLOAT)];
...
- (void)setCellHeightWithText:(NSString *)text {
    CGSize size = [text boundingRectWithSize:CGSizeMake(TFJunYou__SCREEN_WIDTH-INSETS*2-15*2, MAXFLOAT) options:NSStringDrawingUsesLineFragmentOrigin attributes:@{NSFontAttributeName:g_factory.font16} context:nil].size;
    self.content.frame = CGRectMake(15, CGRectGetMaxY(self.line.frame)+20,size.width, size.height);
    self.baseView.frame = CGRectMake(INSETS, INSETS, TFJunYou__SCREEN_WIDTH-INSETS*2, 106+size.height);
}
