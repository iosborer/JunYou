...
@implementation TFJunYou_EmojiCell
- (void)awakeFromNib {
    [super awakeFromNib];
    // Initialization code
}
...
- (void)setUIFrame{
    ...
    }else if ([url.absoluteString rangeOfString:@".webp"].location != NSNotFound){
        NSData *data = [NSData dataWithContentsOfURL:url];
        UIImage *img = [UIImage imageWithData:data];
        _chatImage.image = img;
        [self setCellSubViewFrame:_chatImage.image.size];
        [self setBackgroundImage];
    }else {
...
- (void) setCellSubViewFrame:(CGSize)size {
    ...
    {
        if(w != 0 && h != 0){
//        self.bubbleBg.frame=CGRectMake(TFJunYou__SCREEN_WIDTH-HEAD_SIZE-INSETS*4-k+CHAT_WIDTH_ICON+10, INSETS, INSETS+k, n+INSETS-4);
...
//        self.bubbleBg.frame=CGRectMake(CGRectGetMaxX(self.headImage.frame) + INSETS-CHAT_WIDTH_ICON, INSETS2(self.msg.isGroup), k+INSETS-5, n+INSETS -4);
        ...
        /*
         //            如果超出屏幕宽度
         if(self.bubbleBg.frame.size.width > (TFJunYou__SCREEN_WIDTH - 80)){
         self.bubbleBg.frame = CGRectMake(self.bubbleBg.frame.origin.x, self.bubbleBg.frame.origin.y, TFJunYou__SCREEN_WIDTH - n, self.bubbleBg.frame.size.height);
         _chatImage.frame = CGRectMake(_chatImage.frame.origin.x, _chatImage.frame.origin.y, TFJunYou__SCREEN_WIDTH - n, _chatImage.frame.size.height);
         }
         */
    }
