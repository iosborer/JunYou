...
@implementation TFJunYou_RemindCell
...
//    _messageRemind.layer.borderColor = [[UIColor whiteColor] colorWithAlphaComponent:0.5].CGColor;
    ...
    _messageRemind.numberOfLines = 6.f;
...
//        _messageRemind.backgroundColor = HEXCOLOR(0xB5B5B5);
        ...
        CGSize size = [_messageRemind.text boundingRectWithSize:CGSizeMake(TFJunYou__SCREEN_WIDTH - 80-14, MAXFLOAT) options:NSStringDrawingUsesLineFragmentOrigin attributes:@{NSFontAttributeName:g_factory.font13} context:nil].size;
        ...
        int w = size.width;
        if (allSize.width+14 > TFJunYou__SCREEN_WIDTH-80) {
            // 多行
            w = TFJunYou__SCREEN_WIDTH-80-14;
            _baseView.frame = CGRectMake((TFJunYou__SCREEN_WIDTH-w-14)/2, _baseView.frame.origin.y, w+14, size.height+10);
        ...
        if (range.location != NSNotFound) {
            ...
            CGSize size = [str boundingRectWithSize:CGSizeMake(TFJunYou__SCREEN_WIDTH - 20, MAXFLOAT) options:NSStringDrawingUsesLineFragmentOrigin attributes:@{NSFontAttributeName:g_factory.font13} context:nil].size;
...
+ (float)getChatCellHeight:(TFJunYou_MessageObject *)msg {
    ...
    CGSize size = [str boundingRectWithSize:CGSizeMake(TFJunYou__SCREEN_WIDTH - 80-14, MAXFLOAT) options:NSStringDrawingUsesLineFragmentOrigin attributes:@{NSFontAttributeName:g_factory.font13} context:nil].size;
