...
@implementation TFJunYou_BaseChatCell
//没有xib的初始化
...
-(void)setCellData{
    ...
    NSLog(@"room.showMember----------%d", _room.showMember);
    if (!_room.showMember && [data1.role intValue] != 1 && [data1.role intValue] != 2 && self.nicknameLabel.text.length > 0) {
...
//    [_headImage release];
    ...
    //已读人数
    _readView = [[TFJunYou_Label alloc]initWithFrame:CGRectMake(0, 0, 50, 50)];
    ...
    _readNum = [[TFJunYou_Label alloc] initWithFrame:CGRectMake(_readView.frame.size.width - 30, 0, 30, 15)];
    ...
    //发送失败
    _sendFailed = [[TFJunYou_ImageView alloc]init];
...
-(void)isShowSendTime{
    ...
    if(self.msg.isShowTime && [self.msg.type intValue] != kWCMessageTypeRemind){//超过15分钟则显示时间
        if(_timeLabel == nil){
            ...
            _timeLabel.frame = CGRectMake(TFJunYou__SCREEN_WIDTH/2 -40-4, 10, 86+8, 19);
...
-(void)setHeaderImage{
    ...
    if([_msg.type intValue] != kWCMessageTypeRemind && ![_msg isPinbaMsg]){
        ...
        if(_msg.isMySend){//头像
            _headImage.frame = CGRectMake(TFJunYou__SCREEN_WIDTH-MAX_INSETS-HEAD_SIZE, INSETS, HEAD_SIZE, HEAD_SIZE);
            ...
            _nicknameLabel.frame = CGRectMake(TFJunYou__SCREEN_WIDTH-MAX_INSETS-HEAD_SIZE - 200 - 10, 6, 200, 12);
            if (_msg.isShowTime) {
                _headImage.frame = CGRectMake(TFJunYou__SCREEN_WIDTH-MAX_INSETS-HEAD_SIZE, INSETS + 40, HEAD_SIZE , HEAD_SIZE);
                _cerImgView.frame = CGRectMake(_headImage.frame.origin.x-CER_HEIGHT/2, _headImage.frame.origin.y-CER_HEIGHT/2, _headImage.frame.size.width+CER_HEIGHT, _headImage.frame.size.height+CER_HEIGHT);
                _nicknameLabel.frame = CGRectMake(TFJunYou__SCREEN_WIDTH-MAX_INSETS-HEAD_SIZE - 200 - 10, 6 + 40, 200, 12);
            }
        }else{
            if (self.isSelectMore) {
                if ([self.msg.type intValue] != kWCMessageTypeRemind) {
...
- (void)setBackgroundImage{
    ...
    // 阅后即焚标记(使用时才创建)
    if (!_burnImage) {
        _burnImage = [[TFJunYou_ImageView alloc] init];
    ...
    if(self.msg.isMySend){
        ...
        if (([_msg.type intValue] == kWCMessageTypeFile || [_msg.type intValue] == kWCMessageTypeImage || [_msg.type intValue] == kWCMessageTypeCustomFace || [_msg.type intValue] == kWCMessageTypeEmoji  || [_msg.type intValue] == kWCMessageTypeVideo) && [self.msg.isSend intValue] == transfer_status_yes) {
...
- (void)drawReadDelView:(BOOL)isSelected {
    ...
    if (([_msg.type intValue] == kWCMessageTypeText || [_msg.type intValue] == kWCMessageTypeImage|| [_msg.type intValue] == kWCMessageTypeCustomFace|| [_msg.type intValue] == kWCMessageTypeEmoji || [_msg.type intValue] == kWCMessageTypeVoice || [_msg.type intValue] == kWCMessageTypeVideo)) {
...
-(void)drawIsRead{
    ...
    if([self.msg.isRead boolValue]){
        if (_readImage == nil) {
            TFJunYou_ImageView * imageV = (TFJunYou_ImageView *) [self viewWithTag:131];
            imageV.hidden = YES;
        }else{
            _readImage.hidden = YES;
        }
...
-(void)drawIsSend{
    ...
    if(n){
        ...
        if (([_msg.type intValue] == kWCMessageTypeFile || [_msg.type intValue] == kWCMessageTypeImage || [_msg.type intValue] == kWCMessageTypeCustomFace || [_msg.type intValue] == kWCMessageTypeEmoji || [_msg.type intValue] == kWCMessageTypeVideo) && [self.msg.isRead intValue] == 0 && [self.msg.isSend intValue] == transfer_status_yes) {
            // 进度条发送时可能会出现进度条一直存在， 这里处理， 收到回执就隐藏进度条
            [self sendMessageToUser];
        }
...
-(void)drawSendOrReadImage{
    ...
    if([self.msg.isSend intValue] == transfer_status_yes){
        ...
        if ([self.msg.type intValue] == kWCMessageTypeGif || [self.msg.type intValue] == kWCMessageTypeShake) {
            _readImage.frame = CGRectMake(TFJunYou__SCREEN_WIDTH-HEAD_SIZE-imageItemHeight-INSETS*2 + 40 - 20 - INSETS * .5-10, 20, _readImage.frame.size.width, _readImage.frame.size.height);
            _burnImage.frame = CGRectMake(TFJunYou__SCREEN_WIDTH-HEAD_SIZE-imageItemHeight-INSETS*2 + 40 - 20 - INSETS * .5+2, 20+18, _burnImage.frame.size.width, _burnImage.frame.size.height);
            if (self.msg.isShowTime) {
...
//            _readImage.image = [UIImage imageNamed:@"read"];
        ...
        _readImage.hidden = NO;
        if (([_msg.type intValue] == kWCMessageTypeText || [_msg.type intValue] == kWCMessageTypeImage || [_msg.type intValue] == kWCMessageTypeCustomFace || [_msg.type intValue] == kWCMessageTypeEmoji || [_msg.type intValue] == kWCMessageTypeVoice || [_msg.type intValue] == kWCMessageTypeVideo || [_msg.type intValue] == kWCMessageTypeReply)) {
            _burnImage.hidden = ![_msg.isReadDel boolValue];
        }else {
            _burnImage.hidden = YES;
        }
...
//            }
    ...
    if ([self.msg.type intValue] == kWCMessageTypeImage || [self.msg.type intValue] == kWCMessageTypeCustomFace || [self.msg.type intValue] == kWCMessageTypeEmoji || [self.msg.type intValue] == kWCMessageTypeVideo || [self.msg.type intValue] == kWCMessageTypeFile || [self.msg.type intValue] == kWCMessageTypeVoice || [self.msg.type intValue] == kWCMessageTypeText || [self.msg.type intValue] == kWCMessageTypeReply) {
        if (([self.msg.type intValue] == kWCMessageTypeImage || [self.msg.type intValue] == kWCMessageTypeCustomFace || [self.msg.type intValue] == kWCMessageTypeEmoji) && ![self.msg.isReadDel boolValue]) {
            if ([self.msg.type intValue] == kWCMessageTypeImage) {
       ...
       // 收藏  除了11 和 12 都能收藏
        if (![self.msg.isReadDel boolValue]) {
            ...
            if([self.msg.type intValue] == kWCMessageTypeCustomFace || [self.msg.type intValue] == kWCMessageTypeEmoji) {
...
- (void)shareAction {
    if ([self.chatCellDelegate respondsToSelector:@selector(chatCell:shareWXIndexNum:)]) {
        [self.chatCellDelegate chatCell:self shareWXIndexNum:self.indexNum];
    }
}
...
- (void)favoritAction{
    ...
    }else if (msgType == kWCMessageTypeCustomFace || msgType == kWCMessageTypeEmoji) {
...
-(void)dealloc{
    NSLog(@"TFJunYou_BaseChatCell.delloc");
//    [super dealloc];
//    [_bubbleBg release];
    _bubbleBg = nil;
}
