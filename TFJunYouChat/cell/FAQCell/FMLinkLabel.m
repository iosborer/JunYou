...
- (void)setUp{
    ...
    self.textView.frame = CGRectMake(-5, -8, self.bounds.size.width + 5,  self.bounds.size.height + 9);
...
- (void)addClickText:(NSString *)text attributeds:(NSDictionary *)attributeds transmitBody:(id)transmitBody countInt:(int)countInt countArr:(NSArray *)countArr clickItemBlock:(FMLinkLabelClickItemBlock)clickBlock{
    // 根据现有的文本生成可变的富文本
    ...
    if (countInt==countArr.count - 1) {
        self.textView.textContainerInset = UIEdgeInsetsMake(-40, 0, 0, 0);
    }
...
- (void)setAttributedText:(NSAttributedString *)attributedText{
    ...
    self.textView.frame = CGRectMake(-5, -8, size.width + 5, size.height + 8);
