...
@interface TFJunYou_GoogleMapVC () <GMSMapViewDelegate,UIScrollViewDelegate,CLLocationManagerDelegate,UITableViewDelegate,UITableViewDataSource,TFJunYou_ActionSheetVCDelegate>
...
@implementation TFJunYou_GoogleMapVC

- (instancetype)init {
    if (self = [super init]) {
        self.title = Localized(@"JXUserInfoVC_Loation");
        self.heightHeader = _locationType ==TFJunYou_GooLocationTypeShowStaticLocation ? 0 : TFJunYou__SCREEN_TOP;
        self.heightFooter = 0;
        self.isGotoBack = YES;
        [self createHeadAndFoot];
        _nearMarkArray = [[NSMutableArray alloc] init];
        if (_locationType ==TFJunYou_GooLocationTypeShowStaticLocation) {
...
//            [self.tableHeader addSubview:btn];
            ...
            if(self.isSend){
                ...
                _sendButton.frame = CGRectMake(TFJunYou__SCREEN_WIDTH-60, TFJunYou__SCREEN_TOP - 34, 60, 24);
    ...
    [[GMSGeocoder geocoder]reverseGeocodeCoordinate:position.target completionHandler:^(GMSReverseGeocodeResponse * response, NSError * error) {
        if (response.results) {
            ...
            if (![_address isEqualToString:@""]) {
                TFJunYou_PlaceMarkModel *place = [[TFJunYou_PlaceMarkModel alloc] init];
...
-(void)mapView:(GMSMapView *)mapView idleAtCameraPosition:(GMSCameraPosition *)position{
    if (_locationType == TFJunYou_GooLocationTypeCurrentLocation) {
...
- (nullable UIImage *)snapshotToImage:(CGSize)size {
    ...
    [_gooMapView drawViewHierarchyInRect:CGRectMake(0, -(map_center.y - size.height)-TFJunYou__SCREEN_TOP, _gooMapView.bounds.size.width, _gooMapView.bounds.size.height) afterScreenUpdates:YES];
...
- (void)customView{
    if (_locationType == TFJunYou_GooLocationTypeShowStaticLocation) {
        ...
        //周边地点列表
        _nearMarkTableView = [[UITableView alloc] initWithFrame:CGRectMake(0, TFJunYou__SCREEN_HEIGHT - 100, TFJunYou__SCREEN_WIDTH, 100) style:UITableViewStylePlain];
