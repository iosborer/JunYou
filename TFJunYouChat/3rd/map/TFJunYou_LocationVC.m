...
@interface TFJunYou_LocationVC()<TFJunYou_ActionSheetVCDelegate> {
    
}

@property (nonatomic, strong) TFJunYou_PlaceMarkModel*model;

@property (nonatomic, strong) TFJunYou_NearMarkCell *lastCell;

@end
@implementation TFJunYou_LocationVC
...
- (id)init
{
    ...
    if (self) {
        ...
        self.heightHeader = _locationType ==TFJunYou_LocationTypeShowStaticLocation ? 0 : TFJunYou__SCREEN_TOP;
        self.heightFooter = 0;
...
- (void)viewDidLoad {
    ...
    [self customView];
    self.tableBody.contentSize = CGSizeMake(0, TFJunYou__SCREEN_HEIGHT - TFJunYou__SCREEN_TOP);
...
-(void)viewDidAppear:(BOOL)animated{
    ...
    if (_locationType == TFJunYou_LocationTypeShowStaticLocation) {
        TFJunYou_MapData * staticData = [_locations firstObject];
...
- (void)customView{
    if (_locationType == TFJunYou_LocationTypeShowStaticLocation) {
        UIButton *backBtn = [[UIButton alloc] initWithFrame:CGRectMake(15, TFJunYou__SCREEN_TOP - 38, 31, 31)];
        ...
        UIView *baseView = [[UIView alloc] initWithFrame:CGRectMake(0, TFJunYou__SCREEN_HEIGHT-100, TFJunYou__SCREEN_WIDTH, 100)];
        baseView.backgroundColor = [UIColor whiteColor];
        [self.view addSubview:baseView];
        
        UILabel *adrLabel = [[UILabel alloc] initWithFrame:CGRectMake(20, 40, TFJunYou__SCREEN_WIDTH-20*3-60, 20)];
        ...
        //复位
        UIButton * resetLoca = [[UIButton alloc]initWithFrame:CGRectMake(TFJunYou__SCREEN_WIDTH -80+30-15, TFJunYou__SCREEN_HEIGHT -100-50, 30, 30)];
        ...
        if(self.isSend){
            ...
            _sendButton.frame = CGRectMake(TFJunYou__SCREEN_WIDTH - 15-32, TFJunYou__SCREEN_TOP - 30, 32, 15);
...
//            });
        ...
        //复位
        UIButton * resetLoca = [[UIButton alloc]initWithFrame:CGRectMake(TFJunYou__SCREEN_WIDTH -50, TFJunYou__SCREEN_HEIGHT -50, 30, 30)];
        [resetLoca setImage:[UIImage imageNamed:@"ic_greeting_checked"] forState:UIControlStateNormal];
        [resetLoca addTarget:self action:@selector(resetLocation) forControlEvents:UIControlEventTouchUpInside];
        [self.view addSubview:resetLoca];
        //当前位置图标
        UIImageView * pointImage = [[UIImageView alloc]initWithFrame:CGRectMake((TFJunYou__SCREEN_WIDTH - 30) /2, (TFJunYou__SCREEN_HEIGHT -64)/2 - 50 +10 +64, 30, 50)];
        ...
        //周边地点列表
        _nearMarkTableView = [[UITableView alloc] initWithFrame:CGRectMake(0, TFJunYou__SCREEN_HEIGHT - 200, TFJunYou__SCREEN_WIDTH, 200) style:UITableViewStylePlain];
...
-(void)addPointAnnotations:(NSArray *)locArray{
    ...
    for (TFJunYou_MapData * mapData in locArray) {
...
-(void)makeMapCenter:(TFJunYou_MapData *)centerData{
...
//    [_address release];
    ...
    if (error==BMK_SEARCH_NO_ERROR) {
        ...
        {
            [_nearMarkArray addObject:[TFJunYou_PlaceMarkModel modelByBMKPoiInfo:poiInfo]];
        }
        _selIndex = 0;
        if (!_nearMarkArray || _nearMarkArray.count <= 0) {
            return;
        }
        TFJunYou_PlaceMarkModel*model = [_nearMarkArray objectAtIndex:0];
        NSLog(@"%@",model);
...
-(void)dealloc{
    NSLog(@"TFJunYou_LocationVC dealloc");
...
-(void)onSelect{
    ...
    TFJunYou_MapData * data = [[TFJunYou_MapData alloc]init];
...
//    UIImage *image = [_baiduMapView takeSnapshot:CGRectMake(0, (CGRectGetHeight(_baiduMapView.frame)-TFJunYou__SCREEN_WIDTH/2.2)/2, TFJunYou__SCREEN_WIDTH-100, TFJunYou__SCREEN_WIDTH/2.2-100)];
...
//    [g_server saveImageToFile:logoImage file:filePath isOriginal:NO];
    UIImage *image = [self snapshotToImage:CGSizeMake(TFJunYou__SCREEN_WIDTH, TFJunYou__SCREEN_WIDTH/2.2)];
    UIImage *logoImage = [self addImageLogo:image text:[UIImage imageNamed:@"position"]];
    NSString* filePath = [TFJunYou_FileInfo getUUIDFileName:@"jpg"];
    [g_server saveImageToFile:logoImage file:filePath isOriginal:NO];
...
- (nullable UIImage *)snapshotToImage:(CGSize)size {
    ...
    [_baiduMapView drawViewHierarchyInRect:CGRectMake(0, -(map_center.y - size.height)-TFJunYou__SCREEN_TOP, _baiduMapView.bounds.size.width, _baiduMapView.bounds.size.height) afterScreenUpdates:YES];
...
-(void)moreBtnAction{
    TFJunYou_ActionSheetVC *actionVC = [[TFJunYou_ActionSheetVC alloc] initWithImages:nil names:@[Localized(@"JX_MapApple")]];
...
////        alertSheet.popoverPresentationController.sourceRect = CGRectMake(0, TFJunYou__SCREEN_HEIGHT, TFJunYou__SCREEN_WIDTH, TFJunYou__SCREEN_HEIGHT);//可选，我这里加这句代码是为了调整到合适的位置
////    }
...
{
    TFJunYou_NearMarkCell *cell = [tableView dequeueReusableCellWithIdentifier:NSStringFromClass([TFJunYou_NearMarkCell class])];
    if (cell == nil) {
        cell = [[TFJunYou_NearMarkCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:NSStringFromClass([TFJunYou_NearMarkCell class])];
    }
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    TFJunYou_PlaceMarkModel*model = [_nearMarkArray objectAtIndex:indexPath.row];
//    _locationTF.text = model.address;
//    
    self.model = model;
    
    TFJunYou_NearMarkCell *cell = [tableView cellForRowAtIndexPath:indexPath];
