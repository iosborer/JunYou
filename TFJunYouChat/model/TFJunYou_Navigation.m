...
@end
@implementation TFJunYou_Navigation
static TFJunYou_Navigation *navigation;

+(TFJunYou_Navigation*)sharedInstance{
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        navigation=[[TFJunYou_Navigation alloc]init];
    });
    return navigation;
}
-(instancetype)init{
    self = [super init];
    if(self){
        _navigationView = [[UIView alloc] initWithFrame:CGRectMake(0, 0, TFJunYou__SCREEN_WIDTH, TFJunYou__SCREEN_HEIGHT)];
...
-(void)createHeaderView{
    ...
    UIImageView* iv = [[UIImageView alloc] initWithFrame:CGRectMake(0, 0, TFJunYou__SCREEN_WIDTH, TFJunYou__SCREEN_TOP)];
    ...
    TFJunYou_Label* p = [[TFJunYou_Label alloc]initWithFrame:CGRectMake(40, TFJunYou__SCREEN_TOP - 32, TFJunYou__SCREEN_WIDTH-40*2, 20)];
    ...
    UIButton *btn = [[UIButton alloc] initWithFrame:CGRectMake(8, TFJunYou__SCREEN_TOP - 38, 31, 31)];
...
- (void)applicationWillResignActive {
    ...
    // 页面复位
    vc.view.frame = CGRectMake(0, 0, TFJunYou__SCREEN_WIDTH, vc.view.frame.size.height);
    self.lastVC.view.frame = CGRectMake(-TFJunYou__SCREEN_WIDTH, 0, self.lastVC.view.frame.size.width, self.lastVC.view.frame.size.height);
...
- (void)screenEdgePanGestureRecognizer:(UIViewController *)viewController{
    ...
    if ([viewController isKindOfClass:[TFJunYou_admobViewController class]]) {
        TFJunYou_admobViewController *vc = (TFJunYou_admobViewController *)viewController;
        [vc.tableBody.panGestureRecognizer requireGestureRecognizerToFail:screenPan];
    }else if([viewController isKindOfClass:[TFJunYou_TableViewController class]]) {
        TFJunYou_TableViewController *vc = (TFJunYou_TableViewController *)viewController;
...
-(void)screenPanAction:(UIScreenEdgePanGestureRecognizer *)screenPan
{
    ...
    vc.view.frame = CGRectMake(p.x, 0, TFJunYou__SCREEN_WIDTH, TFJunYou__SCREEN_HEIGHT);
    self.lastVC.view.frame = CGRectMake(p.x - TFJunYou__SCREEN_WIDTH, 0, self.lastVC.view.frame.size.width, self.lastVC.view.frame.size.height);
    self.isReset = YES;
    ...
    if (screenPan.state == UIGestureRecognizerStateEnded) {
        ...
        if (p.x > TFJunYou__SCREEN_WIDTH/2 || velocity.x > 1000) {
            [UIView animateWithDuration:0.3 animations:^{
                // 滑动结束更改当前view与上一个view的frame
                vc.view.frame = CGRectMake(TFJunYou__SCREEN_WIDTH, 0, TFJunYou__SCREEN_WIDTH, vc.view.frame.size.height);
                ...
                if([vc isKindOfClass:[TFJunYou_TableViewController class]]) {
                    TFJunYou_TableViewController *tableVC = (TFJunYou_TableViewController *)vc;
                    [tableVC actionQuit];
                    [tableVC.footer removeFromSuperview];
                    [tableVC.header removeFromSuperview];
                    tableVC.header = nil;
                    tableVC.footer = nil;
                }else if([vc isKindOfClass:[TFJunYou_admobViewController class]]){
                    TFJunYou_admobViewController *admobVC = (TFJunYou_admobViewController *)vc;
                    [admobVC actionQuit];
                }
...
- (void)pushViewController:(UIViewController *)viewController animated:(BOOL)animated{
    ...
    if (animated) { // 有动画
        ...
        viewController.view.frame = CGRectMake(TFJunYou__SCREEN_WIDTH, 0, TFJunYou__SCREEN_WIDTH, TFJunYou__SCREEN_HEIGHT);
        //移动viewController到屏幕上并添加动画
        [UIView animateWithDuration:.3 animations:^{
            viewController.view.frame = CGRectMake(0, 0, TFJunYou__SCREEN_WIDTH, TFJunYou__SCREEN_HEIGHT);
        }];
    } else { // 没有动画
...
- (void)dismissViewController:(UIViewController *)viewController animated:(BOOL)animated{
    ...
    if (vc == viewController) {
        if (self.subViews.count >= 2) {
            ...
            if (animated) { //有动画
                // 先将上一个view添加到左侧屏幕外边
                lastVC.view.frame = CGRectMake(-TFJunYou__SCREEN_WIDTH, lastVC.view.frame.origin.y, lastVC.view.frame.size.width, lastVC.view.frame.size.height);
...
- (void)popToViewController:(Class)viewController animated:(BOOL)animated{
    ...
    if (animated) { // 有动画
        // 先将上一个view添加到左侧屏幕外边
        vc.view.frame = CGRectMake(-TFJunYou__SCREEN_WIDTH, vc.view.frame.origin.y, vc.view.frame.size.width, vc.view.frame.size.height);
