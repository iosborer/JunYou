//
//  TFJunYou_AddressBook.m
//  TFJunYouChat
//
//  Created by lifengye on 2020/4/14.
...
@interface TFJunYou_AddressBook ()
...
@implementation TFJunYou_AddressBook
static TFJunYou_AddressBook *shared;

+(TFJunYou_AddressBook*)sharedInstance{
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        shared=[[TFJunYou_AddressBook alloc]init];
    });
    return shared;
}
...
- (NSDictionary *) getMyAddressBook {
    ...
    for (int i = 0; i < count; i++) {
        ...
        if ([TFJunYou_MyTools isChineseLanguage:lang]) {
...
- (NSMutableArray *)fetchAllAddressBook {
    ...
    while ([rs next]) {
        TFJunYou_AddressBook *task=[[TFJunYou_AddressBook alloc] init];
...
-(void)addressBookFromDataset:(TFJunYou_AddressBook*)obj rs:(FMResultSet*)rs{
...
-(NSMutableArray *)doFetchUnread {
    ...
    FMResultSet *rs=[db executeQuery:sql];
    while ([rs next]) {
        TFJunYou_AddressBook *obj=[[TFJunYou_AddressBook alloc] init];
