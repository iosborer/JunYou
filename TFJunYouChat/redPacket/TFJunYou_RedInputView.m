...
@interface TFJunYou_RedInputView () <UITextFieldDelegate> {
    CGFloat _greetY;
    CGFloat _countY;
    CGFloat _moneyY;
    CGFloat _sendY;
...
@end
@implementation TFJunYou_RedInputView
...
-(void)layoutSubviews{
    ...
    if(_isRoom){
        ...
        _line.frame = CGRectMake(15, RowHeight-LINE_WH, TFJunYou__SCREEN_WIDTH-15, LINE_WH);
    }
    ...
    _allMoneyLab.frame = CGRectMake(0, CGRectGetMinY(_sendButton.frame)-22 - 26, TFJunYou__SCREEN_WIDTH, 22);
    if (_type == 3) {
        ...
        _noticeTitle.frame = CGRectMake(15, CGRectGetMaxY(_greetView.frame)+12, TFJunYou__SCREEN_WIDTH-15, 13);
    }else{
        _greetTitle.hidden = YES;
        _noticeTitle.frame = CGRectMake(15, CGRectGetMaxY(_moneyView.frame)+12, TFJunYou__SCREEN_WIDTH-15, 13);
        _greetTextField.frame = CGRectMake(0, 0, CGRectGetWidth(_greetView.frame), _type == 1 || (_type == 2 && _isRoom)  ? RowMaxHeight : RowHeight);
    }
    [self viewLocalized];
}
-(void)viewLocalized{
    _countTitle.text = Localized(@"JXRed_numberPackets");
    _moneyTitle.text = Localized(@"JXRed_totalAmount");
    _countUnit.text = Localized(@"JXRed_A");
    _moneyUnit.text = Localized(@"JX_ChinaMoney");
    [_sendButton setTitle:Localized(@"JXRed_send") forState:UIControlStateNormal];
    [_sendButton setTitle:Localized(@"JXRed_send") forState:UIControlStateHighlighted];
    _moneyTextField.placeholder = Localized(@"JXRed_inputAmount");
    _countTextField.placeholder = Localized(@"JXRed_inputNumPackets");
    switch (_type) {
        case 1:{
            if (_isRoom) {
                _noticeTitle.text = Localized(@"JXRed_sameAmount");
            }
            _greetTextField.placeholder = Localized(@"JXRed_greetOlace");
            break;
        }
        case 2:{
            _noticeTitle.text = Localized(@"JXRed_ARandomAmount");
            _greetTextField.placeholder = Localized(@"JXRed_greetOlace");
            break;
        }
        case 3:{
            _noticeTitle.text = Localized(@"JXRed_NoticeOrder");
            _greetTextField.placeholder = Localized(@"JXRed_orderPlace");
            _greetTitle.text = Localized(@"JXRed_setOrder");
            break;
        }
        default:
            break;
