...
- (void)setup {
    ...
    [UIView animateWithDuration:kImageToolAnimationDuration
                     animations:^{
        self->_textMenuView.transform = CGAffineTransformIdentity;
                     }];
...
- (void)showTextEditView:(NSString *)text{
    if (!_textEditView) {
        _textEditView = [[UITextView alloc] initWithFrame:CGRectMake(0, TFJunYou__SCREEN_TOP, [UIScreen mainScreen].bounds.size.width,800)];
...
- (void)createColorButtonWithArray:(NSArray *)colorArr {
    ...
    self.colorView.frame = CGRectMake(self.colorView.frame.origin.x, TFJunYou__SCREEN_HEIGHT-TFJunYou__SCREEN_TOP-self.keyBoardH-44, TFJunYou__SCREEN_WIDTH, 44);
