...
- (void)initMenuView{
    if (_menuView == nil) {
        _menuView = [[UIView alloc] initWithFrame:CGRectMake(0, self.view.height - TFJunYou__SCREEN_BOTTOM, self.view.width, TFJunYou__SCREEN_BOTTOM)];
...
- (void)initNavigationBar{
    NSString *title;
    if ([self.delegate isKindOfClass:[TFJunYou_ChatViewController class]] || [self.delegate isKindOfClass:[TFJunYou_CameraVC class]]) {
...
- (UIView *)setupViewWithTop:(BOOL)isTop {
    ...
    if (isTop) {  // 设置显示的fram
        gradientLayer.frame = CGRectMake(0, 0, TFJunYou__SCREEN_WIDTH, TFJunYou__SCREEN_TOP);
    }else {
        gradientLayer.frame = CGRectMake(0, 0, TFJunYou__SCREEN_WIDTH, TFJunYou__SCREEN_BOTTOM);
    }
    if (isTop) { // 设置渐变颜色
