...
@property (nonatomic, strong) UIButton *watherButton;
@end
...
-(void)viewWillAppear:(BOOL)animated {
    ...
    [self.view bringSubviewToFront:self.watherButton];//懒加载，将切换城市按钮拿到最上层
}
...
-(UIButton *)watherButton {
    if (!_watherButton) {
        _watherButton = [UIButton buttonWithType:UIButtonTypeCustom];
        _watherButton.titleLabel.font=[UIFont systemFontOfSize:20 weight:UIFontWeightMedium];
        _watherButton.frame = CGRectMake(kScreenWidth-74-10, kScreenHeight-100, 64, 44);
        [_watherButton setTitle:@"跳过" forState:UIControlStateNormal];
//        [_watherButton setImage:[UIImage imageNamed:@"location_hardware"] forState:UIControlStateNormal];
        [self.view addSubview:_watherButton];
        [_watherButton addTarget:self action:@selector(pushMainVc) forControlEvents:UIControlEventTouchUpInside];
        
    }
    
    return _watherButton;
}
...
//    UIButton *watherButton = [UIButton buttonWithType:UIButtonTypeCustom];
//    watherButton.frame = CGRectMake(kScreenWidth-44-40, kScreenHeight-100, 64, 44);
//    //watherButton.imageEdgeInsets = UIEdgeInsetsMake(8, 8, 8, 8);
//    //[watherButton setImage:[UIImage imageNamed:@"shoushou"] forState:UIControlStateNormal];
//    watherButton.titleLabel.font=[UIFont systemFontOfSize:20 weight:UIFontWeightMedium];
//    [watherButton setTitle:@"跳过" forState:UIControlStateNormal];
//    [self.view addSubview:watherButton];
//    [watherButton sizeToFit];
//    [watherButton addTarget:self action:@selector(pushMainVc) forControlEvents:UIControlEventTouchUpInside];
//    self.watherButton=watherButton;
  //  [self ImageSpring];
}
...
- (void)ImageSpring {
    ...
    [UIView animateWithDuration:0.5 animations:^{
        ...
        self->_watherButton.transform=CGAffineTransformMake(1, 0, 0, 1, 0, 62);
    }];
    
    [UIView animateWithDuration:0.8 delay:1.0 options:UIViewAnimationOptionCurveEaseInOut animations:^{
 
        self->_watherButton.transform=CGAffineTransformMake(1, 0, 0, 1, 0, 0);
...
- (void)createBackgroundView {
    self.backgroudView = [[UIImageView alloc] initWithImage:[UIImage imageNamed:@"bg_rain_night.jpg"]];
...
- (void)sendRequestToServer:(NSString *)cityName {
    ...
    NSString *url = [NSString stringWithFormat:@"http://dejujia.com/v3/weather/daily.json?key=osoydf7ademn8ybv&location=%@&language=zh-Hans&start=0&days=3",cityName];
