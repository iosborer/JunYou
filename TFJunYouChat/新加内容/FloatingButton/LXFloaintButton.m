...
- (void)doMoveAction:(UIPanGestureRecognizer *)recognizer {
    ...
    //下边界的限制
    newCenter.y = MIN(self.parentView.bounds.size.height - self.safeInsets.bottom- recognizer.view.frame.size.height/2, newCenter.y);
    
    //左边界的限制
    newCenter.x = MAX(recognizer.view.frame.size.width/2, newCenter.x);
    
     //右边界的限制
    newCenter.x = MIN(self.parentView.bounds.size.width - recognizer.view.frame.size.width/2,newCenter.x);
